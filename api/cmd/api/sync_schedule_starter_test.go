...
func TestSyncScheduleStarterAvailabilityAndProviderMapping(t *testing.T) {
	...
	if len(targets) != 9 || targets[0] != syncschedule.TargetUGC || targets[4] != syncschedule.TargetMegarama || targets[5] != syncschedule.TargetCineville || targets[6] != syncschedule.TargetMK2 || targets[7] != syncschedule.TargetCinewest || targets[8] != syncschedule.TargetMetadataRefresh {
		t.Fatalf("targets=%v", targets)
	}
