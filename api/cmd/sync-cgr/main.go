...
type dependencies struct {
	...
	newClient    func(cgr.ClientConfig) (cgr.Getter, error)
...
func productionDependencies() dependencies {
	return dependencies{
		getenv:    os.Getenv,
		newClient: func(config cgr.ClientConfig) (cgr.Getter, error) { return cgr.NewClient(config) },
...
func main() {
	...
	if err := runtimeconfig.LoadDotEnv(); err != nil {
		logger.Error("process_start_failed", "component", "sync_cgr", "error_code", "configuration_error")
		os.Exit(2)
	}
...
func runWithDependencies(ctx context.Context, args []string, now func() time.Time, stdout, stderr io.Writer, deps dependencies) int {
	...
	flags := flag.NewFlagSet("sync-cgr", flag.ContinueOnError)
	...
	flags.StringVar(&proxyFile, "proxy-file", "", "optional proxy file")
	if flags.Parse(args) != nil || flags.NArg() != 0 {
		return 2
	}
	fromDate, parseErr := time.ParseInLocation("2006-01-02", from, location)
	if parseErr != nil || fromDate.Format("2006-01-02") != from {
		logCLIError(logger, "configuration_failed", "configuration_error")
		return 2
	}
	...
	timing, err := runtimeconfig.Load(runtimeconfig.CGRTiming, deps.getenv, overrides)
	if err != nil {
		logCLIError(logger, "configuration_failed", "configuration_error")
		return 2
	}
	...
	client, err := deps.newClient(cgr.ClientConfig{Proxies: proxies, Timeout: timing.Sync.RequestTimeout})
	if err != nil {
		logCLIError(logger, "configuration_failed", "configuration_error")
		return 2
	}
	...
	executor, err := deps.newExecutor(synccontrol.ProductionExecutorOptions{Writer: services.writer, NewCGR: func() (cgr.Getter, error) { return client, nil }, Enrich: enrich, Now: now, Logger: logger, OperationTimeout: fullConfig.Sync.OperationTimeout})
	if err != nil {
		logCLIError(logger, "sync_command_failed", "configuration_error")
		return 1
	}
	outcomes, err := executor.Run(ctx, synccontrol.TargetCGR, synccontrol.Window{From: from})
	if err != nil {
...
func loadProxies(path string) ([]syncproxy.Proxy, error) {
	if path == "" {
		return nil, nil
	}
...
func logCLIError(logger *slog.Logger, event, code string) {
	logger.Error(event, "component", "sync_cgr", "error_code", code)
}
