...
ALTER TABLE schedule_snapshot ADD CONSTRAINT schedule_snapshot_provider_check CHECK (provider IN ('ugc', 'kinepolis', 'pathe', 'combined'));

ALTER TABLE provider_snapshots DROP CONSTRAINT provider_snapshots_provider_check;
ALTER TABLE provider_snapshots ADD CONSTRAINT provider_snapshots_provider_check CHECK (provider IN ('ugc', 'kinepolis', 'pathe'));

ALTER TABLE theaters DROP CONSTRAINT theaters_provider_check;
ALTER TABLE theaters ADD CONSTRAINT theaters_provider_check CHECK (provider IN ('ugc', 'kinepolis', 'pathe'));
...
ALTER TABLE theaters ADD CONSTRAINT theaters_provider_identity_check CHECK (
    ...
    (provider IN ('kinepolis', 'pathe') AND provider_id ~ '^[A-Za-z0-9][A-Za-z0-9_-]{0,127}$')
);

ALTER TABLE movies DROP CONSTRAINT movies_provider_check;
ALTER TABLE movies ADD CONSTRAINT movies_provider_check CHECK (provider IN ('ugc', 'kinepolis', 'pathe'));
...
ALTER TABLE showtimes ADD CONSTRAINT showtimes_provider_check CHECK (provider IN ('ugc', 'kinepolis', 'pathe'));
...
ALTER TABLE showtimes ADD CONSTRAINT showtimes_provider_identity_check CHECK (
    ...
    (provider = 'pathe' AND provider_showing_id ~ '^S[1-9][0-9]*$')
);
ALTER TABLE showtimes DROP CONSTRAINT showtimes_format_check;
ALTER TABLE showtimes ADD CONSTRAINT showtimes_format_check CHECK (format IN ('2D', '3D', 'IMAX', 'DOLBY', 'SCREENX', 'LASER_ULTRA', '4DX', 'ICE'));

ALTER TABLE movie_matches DROP CONSTRAINT movie_matches_source_provider_check;
ALTER TABLE movie_matches ADD CONSTRAINT movie_matches_source_provider_check CHECK (source_provider IN ('ugc', 'kinepolis', 'pathe'));
...
ALTER TABLE movie_matches ADD CONSTRAINT movie_matches_source_movie_id_check CHECK (
    ...
    (source_provider IN ('kinepolis', 'pathe') AND source_movie_id ~ '^[A-Za-z0-9][A-Za-z0-9_-]{0,127}$')
);

ALTER TABLE local_movie_groups DROP CONSTRAINT local_movie_groups_primary_source_provider_check;
ALTER TABLE local_movie_groups ADD CONSTRAINT local_movie_groups_primary_source_provider_check CHECK (primary_source_provider IN ('ugc', 'kinepolis', 'pathe'));
...
ALTER TABLE local_movie_groups ADD CONSTRAINT local_movie_groups_check CHECK (
    ...
    (primary_source_provider IN ('kinepolis', 'pathe') AND primary_source_movie_id ~ '^[A-Za-z0-9][A-Za-z0-9_-]{0,127}$')
);

ALTER TABLE local_movie_group_members DROP CONSTRAINT local_movie_group_members_source_provider_check;
ALTER TABLE local_movie_group_members ADD CONSTRAINT local_movie_group_members_source_provider_check CHECK (source_provider IN ('ugc', 'kinepolis', 'pathe'));
...
ALTER TABLE sync_runs ADD CONSTRAINT sync_runs_target_check CHECK (target IN ('all', 'ugc', 'kinepolis', 'pathe'));
...
ALTER TABLE sync_runs ADD CONSTRAINT sync_runs_trigger_occurrence_check CHECK (
    ...
    (trigger_source = 'scheduled' AND target IN ('ugc', 'kinepolis', 'pathe') AND schedule_revision > 0 AND scheduled_for IS NOT NULL AND schedule_attempt BETWEEN 0 AND 2)
);

ALTER TABLE sync_schedules DROP CONSTRAINT sync_schedules_provider_check;
ALTER TABLE sync_schedules ADD CONSTRAINT sync_schedules_provider_check CHECK (provider IN ('ugc', 'kinepolis', 'pathe'));

ALTER TABLE public_movies DROP CONSTRAINT public_movies_identity_anchor_provider_check;
ALTER TABLE public_movies ADD CONSTRAINT public_movies_identity_anchor_provider_check CHECK (identity_anchor_provider IN ('ugc', 'kinepolis', 'pathe'));
...
ALTER TABLE public_movies ADD CONSTRAINT public_movies_check1 CHECK (
    ...
    (identity_anchor_provider IN ('kinepolis', 'pathe') AND identity_anchor_source_movie_id ~ '^[A-Za-z0-9][A-Za-z0-9_-]{0,127}$')
);

ALTER TABLE public_movie_sources DROP CONSTRAINT public_movie_sources_source_provider_check;
ALTER TABLE public_movie_sources ADD CONSTRAINT public_movie_sources_source_provider_check CHECK (source_provider IN ('ugc', 'kinepolis', 'pathe'));
...
ALTER TABLE movie_slug_aliases ADD CONSTRAINT movie_slug_aliases_check CHECK (
    (alias_kind = 'source' AND source_provider IN ('ugc', 'kinepolis', 'pathe') AND source_movie_id IS NOT NULL) OR
