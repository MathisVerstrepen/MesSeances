...
This document describes the PostgreSQL schema after applying migrations `001_initial.sql` through [047_account_avatar_webp.sql](047_account_avatar_webp.sql), including the required offline media-conversion completion described below. The SQL files are the source of truth. Update this document when adding a migration; this is the resulting schema, not a migration-by-migration changelog or a report of a deployed database.
...
- Providers are `ugc`, `kinepolis`, `pathe`, `cgr`, `megarama`, `cineville`, `mk2`, `cinewest`, `grandecran`, and `noecinemas`, unless a table explicitly allows another value. Provider fields are strings with checks, not PostgreSQL enums. TMDB upcoming ingestion is a scheduler target, not a cinema provider.
...
These rules apply to provider theater IDs, provider movie IDs (including source IDs and identity anchors), and showing IDs. `theater_locations.provider_theater_id` only has a nonblank check except for Cineville, MK2, Cinewest, Grand Ecran, and Noé Cinémas, which additionally use their identity helpers.
...
| `noecinemas` | `^[A-Z0-9]{5}$` | `^([1-9][0-9]{0,111}\|c[A-Za-z0-9_-]{1,111})$`; numeric or opaque event, at most 112 characters | `^[A-Z0-9]{5}-[a-f0-9]{64}$`; cinema component matches theater; digest of theater ID, NUL, exact opaque source session ID |
...
| --- | --- |
| `accounts` | `id bigint identity` PK; normalized ASCII `email text` UNIQUE, 3-254 bytes; nullable `email_verified_at` and `verification_source` (`email`, `google`) present together; `created_at`; nullable `pending_kind` (`email`, `google`); positive `auth_revision bigint` default 1; `authority_event bigint` from sequence. Verification cannot predate creation; absent pending kind requires verified email. Nullable `avatar_path text`; `avatar_source text` default `none`, restricted to `none`, `removed`, `google`, `upload`; nonnegative `avatar_revision bigint` default 0. Sources none/removed require NULL path; google/upload require a path and positive revision. Paths match `^[a-f0-9]{32}\.webp$` and are unique when present; see conversion constraint state below. |
...
Migration 047 separates source/nullability/revision invariants (`accounts_avatar_check`) from WebP filename validation (`accounts_avatar_webp_check`). The latter starts NOT VALID to retain pre-existing PNG rows but still rejects every new insert/update with a non-WebP path. Fresh databases without such references validate it immediately. Databases containing PNG references require offline `messeances-api migrate-account-avatars` with all writers stopped. The command converts files, CAS-updates references and increments avatar revisions while preserving sources, then validates the constraint. Its validated/enforced state on the actual accounts relation is the runtime completion marker; ledger version 047 alone does not prove conversion complete. Enabled API fails closed before workers/listening while validation is pending. Do not edit 046, drop the CHECK, run old writers during conversion or reset invalid references to bypass errors. See the accounts runbook for tagged builds, paired backups, resumable failure handling and execution authorization.
