...
Migration 047 separates source/nullability/revision invariants (`accounts_avatar_check`) from WebP filename validation (`accounts_avatar_webp_check`). Migration [048](048_validate_account_avatar_webp.sql) unconditionally validates the latter, so the final schema has an ordinary enforced, validated CHECK, not a runtime conversion marker. Existing development PNG references are unsupported and cause transactional migration failure without deleting accounts/files or advancing the ledger. No converter or legacy stored-PNG support remains. Preserve 046/047 history and resolve disposable development data only with explicit operator authorization. PNG upload input remains supported and is normalized to WebP; it is distinct from legacy stored PNG files.
