...
func TestEmbeddedMigrations(t *testing.T) {
	...
	if len(items) != 6 || items[0].version != 1 || items[0].name != "001_initial.sql" || items[1].version != 2 || items[1].name != "002_movie_enrichment.sql" || items[2].version != 3 || items[2].name != "003_admin_match_review.sql" || items[3].version != 4 || items[3].name != "004_movie_backdrop.sql" || items[4].version != 5 || items[4].name != "005_multi_provider.sql" || items[5].version != 6 || items[5].name != "006_repair_multi_provider_schema.sql" {
