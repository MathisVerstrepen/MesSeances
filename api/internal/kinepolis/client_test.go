...
func TestClientRotatesBlockedProxiesAndStopsWhenExhausted(t *testing.T) {
	...
	client.clients[0].Transport = roundTripFunc(func(*http.Request) (*http.Response, error) {
		first++
		return testResponse(http.StatusForbidden, "blocked"), nil
	})
	...
	for index := range client.clients {
		client.clients[index].Transport = roundTripFunc(func(*http.Request) (*http.Response, error) { return testResponse(http.StatusForbidden, "blocked"), nil })
	}
