...
func TestMK2ManagerScheduledStatus(t *testing.T) {
	...
	if status.State != StateSucceeded || status.Providers["mk2"].State != ProviderSucceeded || status.Providers["ugc"].State != ProviderNotRequested || len(status.Providers) != 10 || status.Occurrence == nil || status.Occurrence.Provider != TargetMK2 {
