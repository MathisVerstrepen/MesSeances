...
func TestProductionExecutorPublishesTargetAllOnce(t *testing.T) {
	...
	if err != nil || writes != 1 || enrichments != 10 || outcomes[TargetNoeCinemas].Sync.Version != 11 || outcomes[TargetCinewest].Sync.Version != 11 || outcomes[TargetGrandEcran].Sync.Version != 11 || outcomes[TargetMK2].Sync.Version != 11 || outcomes[TargetCineville].Sync.Version != 11 || outcomes[TargetMegarama].Sync.Version != 11 || outcomes[TargetUGC].Sync.Version != 11 || outcomes[TargetKinepolis].Sync.Version != 11 || outcomes[TargetPathe].Sync.Version != 11 || outcomes[TargetCGR].Sync.Version != 11 || outcomes[TargetUGC].Sync.Through != "2027-01-10" || outcomes[TargetKinepolis].Sync.Through != "2026-11-20" || outcomes[TargetPathe].Sync.Through != "2026-12-15" || outcomes[TargetCGR].Sync.Through != "2026-10-30" || outcomes[TargetPathe].Sync.Requests != 17 || outcomes[TargetCGR].Sync.Requests != 8 {
		t.Fatalf("outcomes=%+v writes=%d enrichments=%d err=%v", outcomes, writes, enrichments, err)
	}
	// Failure of Grand Ecran cannot publish the already-prepared snapshots.
	executor.syncGrandEcran = func(context.Context, grandecran.Getter, grandecran.SyncOptions) (schedule.Dataset, grandecran.SyncSummary, error) {
		return schedule.Dataset{}, grandecran.SyncSummary{}, errors.New("synthetic acquisition failure")
	}
	_, err = executor.Run(t.Context(), TargetAll, window)
	var runErr *RunError
	if !errors.As(err, &runErr) || runErr.Provider != TargetGrandEcran || writes != 1 || enrichments != 10 {
		t.Fatalf("late provider failure published partial data: writes=%d enrichments=%d err=%v", writes, enrichments, err)
	}
}
...
type countedCinevilleFetcher struct{ requests int }

func (countedCinevilleFetcher) Fetch(context.Context) ([]byte, error) { return nil, nil }
func (countedCinevilleFetcher) FetchCinema(context.Context, string, string) ([]byte, error) {
	return nil, nil
}
func (c countedCinevilleFetcher) RequestCount() int { return c.requests }

func cinevilleDataset(t *testing.T, window Window) schedule.Dataset {
	t.Helper()
	data := validDataset(t, schedule.ProviderUGC, window)
	data.Provider = schedule.ProviderCineville
	data.Theaters[0] = schedule.TheaterRecord{Provider: schedule.ProviderCineville, ID: "cineville-639", ProviderID: "639", Slug: "cineville-639", Name: "Katorza", City: "Quimper", PostalCode: "29000", AvailableDates: []string{window.From}, AcceptedPasses: []string{}}
	r := &data.Showtimes[0]
	r.Provider, r.ID, r.ProviderShowingID, r.TheaterID = schedule.ProviderCineville, "cineville-showing-639-1", "639-1", "cineville-639"
	r.Movie = schedule.MovieRecord{Provider: schedule.ProviderCineville, ProviderID: "-693091020261", Slug: "cineville-film--693091020261", Title: "Event"}
	r.EndTime, r.Room, r.BookingURL = r.StartTime, "1", "https://www.cineville.fr/vad/639/1/9"
	return data
}
func configureCinevilleTestExecutor(t *testing.T, executor *ProductionExecutor, window Window) {
	t.Helper()
	executor.newCineville = func() (cineville.Fetcher, error) { return countedCinevilleFetcher{requests: 27}, nil }
	executor.syncCineville = func(_ context.Context, _ cineville.Fetcher, options cineville.SyncOptions) (schedule.Dataset, cineville.SyncSummary, error) {
		if options.From != window.From || options.Now.IsZero() {
			t.Fatal("Cineville sync options")
		}
		return cinevilleDataset(t, window), cineville.SyncSummary{Requests: 25, Cinemas: 1, Movies: 1, Showtimes: 1}, nil
	}
}
