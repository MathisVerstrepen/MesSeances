...
func (f executorFunc) Run(ctx context.Context, target Target, window Window) (map[Target]ProviderOutcome, error) {
	...
	if target == TargetAll {
		return map[Target]ProviderOutcome{TargetUGC: outcome, TargetKinepolis: outcome, TargetPathe: outcome, TargetCGR: outcome, TargetMegarama: outcome, TargetCineville: outcome, TargetMK2: outcome, TargetCinewest: outcome, TargetGrandEcran: outcome, TargetNoeCinemas: outcome}, nil
	}
...
func TestManagerOrdersAllAndRejectsOverlap(t *testing.T) {
	...
	if accepted.ID != "1" || accepted.State != StateRunning || accepted.Providers["ugc"].State != ProviderPending || accepted.Providers["kinepolis"].State != ProviderPending || accepted.Providers["pathe"].State != ProviderPending || accepted.Providers["cgr"].State != ProviderPending || accepted.Providers["megarama"].State != ProviderPending || accepted.Providers["cineville"].State != ProviderPending || accepted.Providers["mk2"].State != ProviderPending || accepted.Providers["cinewest"].State != ProviderPending || accepted.Providers["grandecran"].State != ProviderPending || accepted.Providers["noecinemas"].State != ProviderPending || len(accepted.Providers) != 10 || accepted.StartedAt.Location() != time.UTC {
...
func TestManagerFailurePanicCancellationAndTargets(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			status := waitForTerminal(t, manager)
			if status.Providers["ugc"].State != test.wantUGC || status.Providers["kinepolis"].State != test.wantKin || status.Providers["pathe"].State != test.wantPathe || status.Providers["cgr"].State != test.wantCGR || len(status.Providers) != 10 || status.FinishedAt == nil {
				t.Fatalf("status=%+v", status)
			}
...
func TestManagerTargetAllUsesLatestProviderEnd(t *testing.T) {
	manager, err := newTestManager(context.Background(), func() time.Time {
		...
		return map[Target]ProviderOutcome{
			...
			TargetNoeCinemas: {Sync: SyncOutcome{Through: "2027-06-28"}},
		}, nil
	}))
	if err != nil {
		t.Fatal(err)
	...
	status := waitForTerminal(t, manager)
	if status.State != StateSucceeded || status.Through != "2027-07-01" {
		t.Fatalf("status=%+v", status)
	}
}
...
func TestManagerMarksEveryProviderFailedWhenEveryPublicationFails(t *testing.T) {
	...
	status := waitForTerminal(t, manager)
	for _, provider := range []string{string(TargetUGC), string(TargetKinepolis), string(TargetPathe), string(TargetCGR), string(TargetMegarama), string(TargetCineville), string(TargetMK2), string(TargetCinewest), string(TargetGrandEcran), string(TargetNoeCinemas)} {
