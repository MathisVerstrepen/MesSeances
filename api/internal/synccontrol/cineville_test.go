...
func TestCinevilleManagerStatusCloning(t *testing.T) {
	...
	if status.State != StateSucceeded || status.Providers["cineville"].State != ProviderSucceeded || status.Providers["ugc"].State != ProviderNotRequested || len(status.Providers) != 6 {
