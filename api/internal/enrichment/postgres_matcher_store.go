...
func (s *PostgresStore) Metadata(ctx context.Context, provider string, movieID int64, locale string) (Metadata, bool, error) {
	...
	var overview, releaseDate, poster, backdrop, trailerYouTubeKey *string
	err := s.pool.QueryRow(ctx, `SELECT provider, provider_movie_id, locale, provider_title, localized_title, overview, release_date::text, poster_url, backdrop_url, trailer_youtube_key, runtime_minutes, genres, fetched_at, refresh_after
FROM movie_metadata_cache WHERE provider=$1 AND provider_movie_id=$2 AND locale=$3`, provider, movieID, locale).Scan(&metadata.Provider, &metadata.ProviderMovieID, &metadata.Locale, &metadata.ProviderTitle, &metadata.LocalizedTitle, &overview, &releaseDate, &poster, &backdrop, &trailerYouTubeKey, &metadata.RuntimeMinutes, &metadata.Genres, &metadata.FetchedAt, &metadata.RefreshAfter)
	if errors.Is(err, pgx.ErrNoRows) {
	...
	if trailerYouTubeKey != nil {
		metadata.TrailerYouTubeKey = *trailerYouTubeKey
	}
