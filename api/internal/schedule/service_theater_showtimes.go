...
func (s *Service) TheaterShowtimes(query TheaterShowtimesQuery) (TheaterShowtimes, error) {
	...
	for _, showingPosition := range view.theaterDate[theaterDateKey{theaterID: theater.ID, date: date}] {
		...
		result.Showtimes = append(result.Showtimes, TimelineShowtime{Showtime: showtime, StartOffsetMinutes: int(showtime.StartTime.Sub(windowStart) / time.Minute), DurationMinutes: record.Movie.RuntimeMinutes, PosterURL: poster, BackdropURL: backdrop})
	}
