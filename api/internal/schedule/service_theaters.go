...
func (s *Service) Theaters(query TheaterCatalogQuery) []TheaterCatalogItem {
	...
	if chain != "" && !strings.EqualFold(chain, string(ProviderUGC)) && !strings.EqualFold(chain, string(ProviderKinepolis)) && !strings.EqualFold(chain, string(ProviderPathe)) && !strings.EqualFold(chain, string(ProviderCGR)) && !strings.EqualFold(chain, string(ProviderMegarama)) && !strings.EqualFold(chain, string(ProviderCineville)) && !strings.EqualFold(chain, string(ProviderMK2)) && !strings.EqualFold(chain, string(ProviderCinewest)) && !strings.EqualFold(chain, string(ProviderGrandEcran)) && !strings.EqualFold(chain, string(ProviderNoeCinemas)) {
