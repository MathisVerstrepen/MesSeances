...
func (s *PostgresStore) List(ctx context.Context) ([]Schedule, error) {
	rows, err := s.pool.Query(ctx, `SELECT `+scheduleColumns+` FROM sync_schedules
		ORDER BY CASE target WHEN 'ugc' THEN 1 WHEN 'kinepolis' THEN 2 WHEN 'pathe' THEN 3 WHEN 'cgr' THEN 4 WHEN 'megarama' THEN 5 WHEN 'cineville' THEN 6 WHEN 'tmdb_metadata_refresh' THEN 7 WHEN 'tmdb_upcoming_movies' THEN 8 ELSE 9 END,id`)
	if err != nil {
