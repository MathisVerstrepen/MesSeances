...
var errSnapshotChanged = errors.New("grandecran snapshot changed")
...
func Sync(ctx context.Context, getter Getter, options SyncOptions) (data schedule.Dataset, summary SyncSummary, err error) {
	...
	if err == nil {
		if validation := schedule.ValidateDataset(data, true); validation != nil {
			err = fmt.Errorf("%w: invalid Grand Ecran dataset", schedule.ErrDatasetValidation)
		}
	}
...
func discoverCinemas(ctx context.Context, getter Getter) ([]cinema, error) {
	body, err := getter.Get(ctx, OperationCinemas, CinemasURL)
	if err == nil {
		return parseCinemas(body)
	}
	if fatalRequest(err) || ctx.Err() != nil {
		return nil, err
	}
	html, err := getter.Get(ctx, OperationDiscovery, APIBaseURL+"/")
	if err != nil {
		return nil, err
	}
	root := ""
	for _, candidate := range assetRoot.FindAllString(string(html), -1) {
		if root != "" && root != candidate {
			return nil, fmt.Errorf("conflicting asset roots")
		}
		root = candidate
	}
	if root == "" {
		return nil, fmt.Errorf("missing current asset root")
	}
	body, err = getter.Get(ctx, OperationCinemas, root+strings.TrimPrefix(queryPath, "/"))
	if err != nil {
		var r *RequestError
...
func syncSnapshot(ctx context.Context, g Getter, o SyncOptions) (schedule.Dataset, SyncSummary, error) {
	...
	cinemas, err := discoverCinemas(ctx, g)
	if err != nil {
		return schedule.Dataset{}, summary, err
	}
	...
	data := schedule.Dataset{SchemaVersion: schedule.SchemaVersion, Provider: schedule.ProviderGrandEcran, Scope: schedule.ScopeAll, Timezone: schedule.Timezone, GeneratedAt: o.Now.UTC(), Window: schedule.Window{From: o.From, Through: o.From}, Theaters: []schedule.TheaterRecord{}, Showtimes: []schedule.ShowtimeRecord{}}
	type job struct {
		cinema  cinema
	...
	for i, c := range cinemas {
		...
		id := "grandecran-" + c.ID
		data.Theaters = append(data.Theaters, schedule.TheaterRecord{Provider: schedule.ProviderGrandEcran, ProviderID: c.ID, ID: id, Slug: id, Name: c.Name, Address: l.Address, City: l.City, PostalCode: l.Zip, AvailableDates: dates, AcceptedPasses: []string{}})
		if through != "" {
	...
	if err := schedule.ValidateDataset(data, true); err != nil {
		return schedule.Dataset{}, summary, fmt.Errorf("%w: invalid Grand Ecran snapshot", schedule.ErrDatasetValidation)
	}
	return data, summary, nil
}
