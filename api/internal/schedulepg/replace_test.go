...
func TestReplaceRejectsDuplicateAndOversizedProviderBatchesBeforeDatabaseAccess(t *testing.T) {
	...
	if _, err := store.Replace(context.Background(), []Dataset{testDataset(), kinepolisTestDataset(), pathe, testDataset(), pathe, pathe}); err == nil || err.Error() != "invalid schedule replacement batch" {
