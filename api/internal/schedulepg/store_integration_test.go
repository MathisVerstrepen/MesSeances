...
('ugc','99',NULL,NULL,'ign','Lyon',0.60,$3,'ambiguous',$2)`, matchedHash, now, geocoding.AddressHash("Lyon", "69000", "Lyon")); err != nil {
	...
	t.Run("enrichment publication is durable and visible", func(t *testing.T) {
		...
		metadata := enrichment.Metadata{Provider: enrichment.ProviderTMDB, ProviderMovieID: 42, Locale: enrichment.LocaleFrench, ProviderTitle: "Film A", LocalizedTitle: "Film A", Overview: "Résumé", ReleaseDate: "2026-01-02", PosterURL: "https://image.tmdb.org/t/p/w500/a.jpg", BackdropURL: "https://image.tmdb.org/t/p/w780/a.jpg", TrailerVFYouTubeKey: "FRoff123456", TrailerVOYouTubeKey: "ENoff123456", RuntimeMinutes: 100, Genres: []string{"Drame"}, FetchedAt: now, RefreshAfter: now.Add(30 * 24 * time.Hour)}
		if err := enrichment.NewPostgresStore(pool).Publish(ctx, match, metadata); err != nil {
			t.Fatal(err)
		}
		loaded, revision, err := store.Load(ctx)
		if err != nil || revision.EnrichmentVersion != 1 || len(loaded.PublicMovies) != 4 || loaded.PublicMovies[0].TMDBID != 42 || loaded.PublicMovies[0].BackdropURL != metadata.BackdropURL || loaded.PublicMovies[0].TrailerVFYouTubeKey != metadata.TrailerVFYouTubeKey || loaded.PublicMovies[0].TrailerVOYouTubeKey != metadata.TrailerVOYouTubeKey {
