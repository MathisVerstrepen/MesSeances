...
func loadTMDBMetadata(ctx context.Context, tx pgx.Tx, components []*component) (map[int64]tmdbMetadata, error) {
	...
	rows, err := tx.Query(ctx, `SELECT provider_movie_id, localized_title, runtime_minutes, poster_url,
       backdrop_url, trailer_youtube_key, overview, release_date, genres
FROM movie_metadata_cache WHERE provider='tmdb' AND locale='fr-FR' AND provider_movie_id=ANY($1)`, ids)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&id, &item.title, &item.runtime, &item.poster, &item.backdrop, &item.trailerYouTubeKey, &item.overview, &item.releaseDate, &item.genres); err != nil {
			return nil, fmt.Errorf("read canonical TMDB metadata failed")
		}
		result[id] = item
	}
...
func chooseMetadata(component *component, tmdb tmdbMetadata) metadata {
	...
	result.poster, result.backdrop, result.trailerYouTubeKey, result.overview, result.releaseDate = tmdb.poster, tmdb.backdrop, tmdb.trailerYouTubeKey, nonblank(tmdb.overview), tmdb.releaseDate
...
func persistAssignments(ctx context.Context, tx pgx.Tx, components []*component, movies map[int64]publicMovie, retainedOrphans map[int64]bool) error {
	...
	for id, movie := range movies {
		if movie.redirectTo == 0 && movie.confirmedTMDB > 0 && movie.confirmedTMDB != desiredTMDB[id] {
			if _, err := tx.Exec(ctx, "UPDATE public_movies SET confirmed_tmdb_id=NULL, trailer_youtube_key=NULL WHERE id=$1", id); err != nil {
...
WHERE source_provider=$1 AND source_movie_id=$2`, member.key.provider, member.key.id, component.publicID); err != nil {
    ...
    title=$2, runtime_minutes=$3, poster_url=$4, backdrop_url=$5, trailer_youtube_key=$6, overview=$7,
    release_date=$8, genres=$9, confirmed_tmdb_id=$10,
    updated_at=CASE WHEN title IS DISTINCT FROM $2::varchar
        ...
        OR trailer_youtube_key IS DISTINCT FROM $6::varchar
        OR overview IS DISTINCT FROM $7::varchar
        OR release_date IS DISTINCT FROM $8::date
        OR genres IS DISTINCT FROM $9::text[]
        OR confirmed_tmdb_id IS DISTINCT FROM $10::bigint
...
WHERE id=$1 AND redirect_to_id IS NULL`, component.publicID, component.metadata.title, component.metadata.runtime,
			component.metadata.poster, component.metadata.backdrop, component.metadata.trailerYouTubeKey, component.metadata.overview, component.metadata.releaseDate,
	...
	for oldID, targets := range oldTargets {
		...
		if _, err := tx.Exec(ctx, `UPDATE public_movies SET redirect_to_id=$2, confirmed_tmdb_id=NULL, trailer_youtube_key=NULL,
