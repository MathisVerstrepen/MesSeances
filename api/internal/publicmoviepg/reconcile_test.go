...
FROM public_movie_sources`).Scan(&ugcID, &kinepolisID); err != nil || ugcID <= 0 || kinepolisID <= 0 || ugcID == kinepolisID {
    ...
    (provider,provider_movie_id,locale,provider_title,localized_title,overview,runtime_minutes,genres,fetched_at,refresh_after)
VALUES ('tmdb',42,'fr-FR','Original','Canonique TMDB','Résumé TMDB',91,ARRAY['Action'],$1,$2);
