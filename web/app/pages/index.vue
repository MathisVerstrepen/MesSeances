...
const homepagePosterSizes = [
  ...
  (desktopWidth) =>
    `(max-width: 639px) calc((100vw - 3.25rem) / 2), (max-width: 767px) calc((100vw - 4.25rem) / 2), ${desktopWidth}`,
)
...
const pageDescription =
  'Découvrez les films actuellement à l’affiche et trouvez rapidement la séance de cinéma qui correspond à votre emploi du temps.'
const robots = computed(() =>
  Object.keys(route.query).length === 0 && !errorMessage.value
    ? 'index,follow'
    : 'noindex,follow',
)
...
<template>
  <main class="home-page overflow-hidden bg-[#f8f7f2]">
    <section
      class="hero-intro border-b-2 border-ink bg-surface"
      aria-labelledby="home-title"
    >
      <div
        class="mx-auto grid max-w-[1440px] gap-10 px-4 pb-10 pt-12 sm:px-6 sm:pb-14 sm:pt-16 lg:grid-cols-[minmax(0,1.45fr)_minmax(300px,.55fr)] lg:items-end lg:px-10 lg:pb-16 lg:pt-20"
      >
        <div class="min-w-0">
          <h1
            id="home-title"
            class="text-balance text-[clamp(4.6rem,15vw,13rem)] font-black uppercase leading-[0.72] tracking-[-0.085em] text-ink"
          >
        ...
        <div class="max-w-md lg:justify-self-end lg:pb-2">
          <p
            class="text-balance text-2xl font-black leading-[0.95] tracking-[-0.04em] text-ink sm:text-3xl"
          >
            Le bon film.<br>Le bon cinéma.<br>
            <span class="inline-block bg-highlight px-1">Au bon moment.</span>
          </p>
          <div
            class="mt-7 flex flex-col gap-3 sm:flex-row lg:flex-col xl:flex-row"
          >
            <NuxtLink
              to="/recherche"
              class="inline-flex min-h-12 items-center justify-center gap-[0.6rem] rounded-[0.35rem] bg-ink px-4 py-3 font-mono text-[0.72rem] font-extrabold uppercase tracking-[0.08em] text-white [transition:background-color_160ms_ease,color_160ms_ease,transform_160ms_ease] hover:-translate-y-0.5 hover:bg-primary motion-reduce:hover:translate-y-0"
            >
    ...
    <section
      class="relative min-h-[43rem] border-b-2 border-ink bg-[#f8f7f2] bg-[linear-gradient(rgba(39,39,42,0.09)_1px,transparent_1px),linear-gradient(90deg,rgba(39,39,42,0.09)_1px,transparent_1px)] bg-[size:24px_24px] max-md:min-h-0"
      ...
      <div class="mx-auto max-w-[1440px] px-4 py-8 sm:px-6 lg:px-10 lg:py-10">
        <div class="relative z-10 flex items-center justify-between gap-4">
          <h2
            ...
            class="font-mono text-xs font-bold uppercase tracking-[0.18em] text-ink"
          >
            À l’affiche maintenant
          </h2>
          <NuxtLink
            to="/films"
            class="inline-flex shrink-0 items-center gap-[0.4rem] border-b-2 border-current pb-[0.2rem] text-xs font-bold uppercase tracking-[0.12em]"
          >
        ...
        <ul
          v-if="pending"
          class="relative min-h-[35rem] max-md:grid max-md:min-h-0 max-md:grid-cols-2 max-md:gap-x-5 max-md:gap-y-8 max-md:px-0 max-md:pt-12 max-md:pb-6"
          aria-hidden="true"
        >
          <li
            v-for="index in 6"
            :key="index"
            :class="homepagePosterClasses[index - 1]"
          >
            <div class="block rounded-[0.4rem]">
              <div
                class="poster-frame--skeleton aspect-[2/3] overflow-hidden rounded-[0.4rem] border-2 border-ink bg-[#d8d5cc] shadow-[7px_7px_0_rgba(39,39,42,0.95)]"
              />
              <div
                class="poster-label--skeleton mt-[0.65rem] ml-[0.35rem] h-7 rounded-full bg-[#d8d5cc]"
              />
            </div>
          </li>
        </ul>
        ...
        <EditorialStatePanel
          ...
          size="compact"
          shadow="large"
          class="canvas-state relative z-[1] my-24 mx-auto max-w-xl rounded-[0.4rem] font-bold"
        >
          <template #icon
            ><AlertTriangle
              :size="32"
        ...
        >
          <li
            ...
            :key="movie.slug"
            :class="homepagePosterClasses[index]"
          >
            <NuxtLink
              :to="`/film/${movie.slug}`"
              class="group block rounded-[0.4rem] text-ink [transition:transform_180ms_ease] hover:-translate-y-1 hover:rotate-1 motion-reduce:hover:translate-y-0 motion-reduce:hover:rotate-0"
              :aria-label="`${movie.title}, ${movie.runtime_minutes} minutes`"
            >
              <div
                class="aspect-[2/3] overflow-hidden rounded-[0.4rem] border-2 border-ink bg-[#e8e6de] shadow-[7px_7px_0_rgba(39,39,42,0.95)]"
              >
                <PosterImage
              ...
              <span
                class="mt-[0.65rem] ml-[0.35rem] flex items-center justify-between gap-2 rounded-full border-2 border-ink bg-highlight px-[0.55rem] py-[0.35rem] text-[0.68rem] leading-none font-black"
              >
    ...
    <section
      class="border-b-2 border-ink bg-[#ffcf3f]"
    ...
    >
      ...
      <ul class="mx-auto grid max-w-[1440px] md:grid-cols-3">
        <li
          ...
          :key="shortcut.to"
          class="border-ink md:border-r-2 md:last:border-r-0"
        >
          <NuxtLink
            :to="shortcut.to"
            class="shortcut group flex h-full min-h-64 flex-col justify-between border-b-2 border-ink p-6 transition-colors hover:bg-highlight md:border-b-0 lg:p-10"
          >
            <div class="flex items-start justify-between gap-4">
              <span
                class="font-mono text-[11px] font-bold uppercase tracking-[0.16em]"
                >{{
                  shortcut.eyebrow
                }}</span
              >
            ...
            <div class="mt-12">
              <h3
                class="text-3xl font-black leading-none tracking-[-0.04em] sm:text-4xl"
              >
                {{ shortcut.title }}
              </h3>
              <p class="mt-4 max-w-sm text-sm font-medium leading-6">
                {{ shortcut.description }}
              </p>
              <span
                class="mt-6 inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.12em]"
              >
                Explorer
                <ArrowRight
                  :size="16"
