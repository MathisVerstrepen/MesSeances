...
const geocodingControlsDisabled = computed(
  () =>
    geocodingInitialPending.value ||
    geocodingStarting.value ||
    geocodingStatus.value === null ||
    geocodingJob.value?.state === 'running',
)
...
function draftFor(
  item: AdminTheaterLocation,
): AdminTheaterLocationCoordinateDraft {
  const key = locationKey(item)
  return (
    drafts.value[key] ?? (drafts.value[key] = { latitude: '', longitude: '' })
  )
}
...
async function loadLocations(background = false): Promise<boolean> {
  ...
  try {
    ...
    if (isMounted && !background && currentRequest === requestId)
      pending.value = false
  }
}
...
function clearDraftError(
  key: string,
  field: keyof AdminTheaterLocationCoordinateErrors,
) {
...
async function loadGeocodingStatus(fromPolling = false) {
  ...
  if (!fromPolling && geocodingStatus.value === null)
    geocodingInitialPending.value = true
...
async function handleMutationError(cause: unknown, key: string) {
  if (getApiErrorStatus(cause) === 409) {
    setItemError(
      key,
      'Cette localisation a changé ou la suggestion n’est plus disponible. La liste a été actualisée. Vérifiez les données, puis réessayez.',
    )
...
async function acceptSuggestion(item: AdminTheaterLocation) {
  ...
  try {
    await api.adminAcceptTheaterLocationSuggestion(
      item.provider,
...
async function saveManualCoordinates(item: AdminTheaterLocation) {
  ...
  const validation = parseAdminTheaterLocationCoordinates(
    drafts.value[key] ?? { latitude: '', longitude: '' },
  )
  ...
  try {
    await api.adminSetManualTheaterLocation(
      item.provider,
      item.provider_theater_id,
      {
        expected_updated_at: item.updated_at,
  ...
  <main class="mx-auto max-w-5xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
    ...
    >
      <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        >
          <LoaderCircle
            v-if="geocodingStarting || geocodingJob?.state === 'running'"
            :size="17"
            class="animate-spin"
            aria-hidden="true"
          />
      ...
      >
        ...
        <div class="flex-1">
          ...
          <button
            ...
            :disabled="geocodingRequestPending || geocodingStarting"
            @click="loadGeocodingStatus()"
          >
      ...
      <div
        v-if="geocodingInitialPending"
        class="mt-4 flex min-h-11 items-center gap-2 text-sm text-muted"
        role="status"
        aria-live="polite"
      >
        <LoaderCircle
      ...
      <div
        v-else-if="geocodingJob?.state === 'running'"
      ...
      >
        ...
        <span class="text-muted"
          >Démarré le {{ formatDateTime(geocodingJob.started_at) }}</span
        >
      </div>
      ...
      <div v-else-if="geocodingJob" class="mt-4">
        ...
        >
          ...
          >
          <span v-if="geocodingJob.finished_at" class="text-muted"
            >Terminé le {{ formatDateTime(geocodingJob.finished_at) }}</span
          >
          <span
        ...
        <dl
          v-if="geocodingJob.summary"
          class="mt-4 grid grid-cols-2 gap-x-5 gap-y-3 text-sm sm:grid-cols-4 lg:grid-cols-7"
        >
          <div>
            <dt class="text-muted">Sélectionnés</dt>
            <dd class="mt-0.5 font-semibold tabular-nums text-ink">
              {{ geocodingJob.summary.selected }}
            </dd>
          </div>
          <div>
            <dt class="text-muted">Ignorés</dt>
            <dd class="mt-0.5 font-semibold tabular-nums text-ink">
              {{ geocodingJob.summary.skipped }}
            </dd>
          </div>
          <div>
            <dt class="text-muted">Correspondances</dt>
            <dd class="mt-0.5 font-semibold tabular-nums text-ink">
              {{ geocodingJob.summary.matched }}
            </dd>
          </div>
          <div>
            <dt class="text-muted">Ambigus</dt>
            <dd class="mt-0.5 font-semibold tabular-nums text-ink">
              {{ geocodingJob.summary.ambiguous }}
            </dd>
          </div>
          <div>
            <dt class="text-muted">Introuvables</dt>
            <dd class="mt-0.5 font-semibold tabular-nums text-ink">
              {{ geocodingJob.summary.not_found }}
            </dd>
          </div>
          <div>
            <dt class="text-muted">Échecs</dt>
            <dd class="mt-0.5 font-semibold tabular-nums text-ink">
              {{ geocodingJob.summary.failed }}
            </dd>
          </div>
          <div>
            <dt class="text-muted">Enregistrés</dt>
            <dd class="mt-0.5 font-semibold tabular-nums text-ink">
              {{ geocodingJob.summary.written }}
            </dd>
          </div>
        </dl>
      </div>

      <p v-else-if="!geocodingError" class="mt-4 text-sm text-muted">
        Aucun géocodage enregistré.
      </p>
    </section>
    ...
    <template v-else-if="result">
      ...
      <ul
        ...
        aria-label="Localisations de cinémas à traiter"
      >
        <li
          ...
          <article :aria-labelledby="`theater-${domKey(item)}`">
            ...
            >
              ...
              <template v-if="item.suggestion">
                ...
                >
                  <div>
                    ...
                    <dd class="font-semibold tabular-nums text-ink">
                      {{ formatScore(item.suggestion.score) }}
                    </dd>
                  </div>
                  <div v-if="item.suggestion.latitude !== null">
                    <dt class="text-muted">Latitude</dt>
                    <dd class="font-semibold tabular-nums text-ink">
                      {{ item.suggestion.latitude }}
                    </dd>
                  </div>
                  <div v-if="item.suggestion.longitude !== null">
                    <dt class="text-muted">Longitude</dt>
                    <dd class="font-semibold tabular-nums text-ink">
                      {{ item.suggestion.longitude }}
                    </dd>
                  </div>
                  <div v-if="item.suggestion.postal_code">
                    <dt class="text-muted">Code postal</dt>
                    <dd class="font-semibold text-ink">
                      {{ item.suggestion.postal_code }}
                    </dd>
                  </div>
                  <div v-if="item.suggestion.city">
                    <dt class="text-muted">Ville</dt>
                    <dd class="font-semibold text-ink">
                      {{ item.suggestion.city }}
                    </dd>
                  </div>
                  <div v-if="item.suggestion.type">
                ...
                >
                  <LoaderCircle
                    v-if="pendingActions[locationKey(item)] === 'suggestion'"
                    :size="17"
                    class="animate-spin"
                    aria-hidden="true"
                  />
                ...
                <p
                  v-if="!item.can_accept_suggestion"
                  class="mt-2 text-sm text-muted"
                >
                  Cette suggestion ne contient pas de coordonnées acceptables.
                </p>
              </template>
            ...
            <form
              class="pt-4"
              novalidate
              @submit.prevent="saveManualCoordinates(item)"
            >
              <fieldset :disabled="Boolean(pendingActions[locationKey(item)])">
                ...
                <div class="mt-3 grid gap-4 sm:grid-cols-2">
                  <div>
                    ...
                    <input
                      ...
                      autocomplete="off"
                      spellcheck="false"
                      :aria-invalid="draftErrors[locationKey(item)]?.latitude ? 'true' : undefined"
                      :aria-describedby="draftErrors[locationKey(item)]?.latitude ? `latitude-error-${domKey(item)}` : undefined"
                      @input="clearDraftError(locationKey(item), 'latitude')"
                    >
                    <p
                  ...
                  <div>
                    ...
                    <input
                      ...
                      autocomplete="off"
                      spellcheck="false"
                      :aria-invalid="draftErrors[locationKey(item)]?.longitude ? 'true' : undefined"
                      :aria-describedby="draftErrors[locationKey(item)]?.longitude ? `longitude-error-${domKey(item)}` : undefined"
                      @input="clearDraftError(locationKey(item), 'longitude')"
                    >
                    <p
                ...
                >
                  <LoaderCircle
                    v-if="pendingActions[locationKey(item)] === 'manual'"
                    :size="17"
                    class="animate-spin"
                    aria-hidden="true"
                  />
