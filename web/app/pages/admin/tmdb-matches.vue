...
const MATCH_TABS = [
  ...
  value: AdminPendingMatchesFilter
  label: string
  title: string
}>
...
const activeMutationKind = ref<
  'candidate' | 'manual' | 'reject' | 'correction' | ''
>('')
...
const rejectedPage = computed(
  () => Math.floor(rejectedOffset.value / PAGE_SIZE) + 1,
)
const matchedPage = computed(
  () => Math.floor(matchedOffset.value / PAGE_SIZE) + 1,
)
const groupsPage = computed(
  () => Math.floor(groupsOffset.value / PAGE_SIZE) + 1,
)
const canGoNext = computed(
  () => (result.value?.items.length ?? 0) === PAGE_SIZE,
)
const canRejectedGoNext = computed(
  () => (rejectedResult.value?.items.length ?? 0) === PAGE_SIZE,
)
const canMatchedGoNext = computed(
  () => (matchedResult.value?.items.length ?? 0) === PAGE_SIZE,
)
const canGroupsGoNext = computed(
...
const canMerge = computed(
  () =>
    ...
    Boolean(
      primarySourceKey.value && selectedSources.value[primarySourceKey.value],
    ),
)
const metadataRefreshJob = computed(
  () => metadataRefreshStatus.value?.job ?? null,
)
const metadataRefreshRunning = computed(
  () => metadataRefreshJob.value?.state === 'running',
)
const metadataRefreshLoading = computed(
  () => metadataRefreshPending.value || metadataRefreshRunning.value,
)
const anyMutation = computed(() =>
  Boolean(
    activeMutation.value ||
      ...
      rerunPending.value ||
      metadataRefreshPending.value ||
      metadataRefreshStatusPending.value ||
      metadataRefreshRunning.value,
  ),
)
...
const activeMatchSection = computed(() => {
  if (activeTab.value === 'rejected') {
    return {
      ...
      items: adminPendingMatchesForFilter(
...
function sameSource(
  left: AdminLocalMovieSource | null,
  right: AdminLocalMovieSource,
): boolean {
  return (
    left?.source_provider === right.source_provider &&
    left.source_movie_id === right.source_movie_id
  )
}
...
async function loadMatches(background = false) {
  ...
  try {
    const response = await api.adminPendingMatches(
      'unresolved',
      PAGE_SIZE,
      offset.value,
    )
    ...
    for (const match of response.items) {
      ...
      if (
        ...
        match.candidates.some((candidate) => candidate.id === selectedCandidate)
      ) {
    ...
    if (!background && currentRequest === matchesRequestId)
      pending.value = false
  }
}
...
async function loadRejectedMatches(background = false) {
  ...
  try {
    ...
    if (scrollRejectedAfterLoad) {
      ...
      document
        .getElementById('rejected-matches-title')
    ...
    if (!background && currentRequest === rejectedRequestId)
      rejectedPending.value = false
  }
}
...
async function loadMatchedMatches(background = false) {
  ...
  try {
    ...
    if (scrollMatchedAfterLoad) {
      ...
      document
        .getElementById('tmdb-match-tabs')
        ?.scrollIntoView({ behavior: 'smooth' })
    }
  } catch (error) {
    ...
    if (!background && currentRequest === matchedRequestId)
      matchedPending.value = false
  }
}
...
async function loadGroups() {
  ...
  try {
    const response = await api.adminLocalMovieGroups(
      PAGE_SIZE,
      groupsOffset.value,
    )
...
function adminQuery(
  ...
  return mergeOwnedQuery(route.query, OWNED_QUERY_KEYS, {
    ...
    rejected_page:
      nextRejectedPage === 1 ? undefined : String(nextRejectedPage),
...
function hydrateRoute() {
  ...
  const requestedSearch = adminTMDBMatchedSearch(
    singularQueryValue(route.query.q),
  )
  const requestedPage =
    positiveSafeInteger(singularQueryValue(route.query.page)) ?? 1
  const requestedRejectedPage =
    positiveSafeInteger(singularQueryValue(route.query.rejected_page)) ?? 1
  const requestedMatchedPage =
    positiveSafeInteger(singularQueryValue(route.query.matched_page)) ?? 1
  const requestedGroupsPage =
    positiveSafeInteger(singularQueryValue(route.query.groups_page)) ?? 1
  ...
  const safeRejectedPage = Number.isSafeInteger(nextRejectedOffset)
    ? requestedRejectedPage
    : 1
  const safeMatchedPage = Number.isSafeInteger(nextMatchedOffset)
    ? requestedMatchedPage
    : 1
  const safeGroupsPage = Number.isSafeInteger(nextGroupsOffset)
    ? requestedGroupsPage
    : 1
...
async function applyRoute() {
  ...
  if (
    matchedPage.value !== lastLoadMatchedPage ||
    matchedSearch.value !== lastLoadMatchedSearch
  ) {
...
async function refreshResources() {
  ...
  const nextPage =
    !matchesError.value && offset.value > 0 && result.value?.items.length === 0
      ? page.value - 1
      : page.value
  const nextRejectedPage =
    !rejectedError.value &&
    rejectedOffset.value > 0 &&
    rejectedResult.value?.items.length === 0
      ? rejectedPage.value - 1
      : rejectedPage.value
  const nextMatchedPage =
    !matchedError.value &&
    matchedOffset.value > 0 &&
    matchedResult.value?.items.length === 0
      ? matchedPage.value - 1
      : matchedPage.value
  const nextGroupsPage =
    !groupsError.value &&
    groupsOffset.value > 0 &&
    groupsResult.value?.items.length === 0
      ? groupsPage.value - 1
      : groupsPage.value
  if (
    nextPage !== page.value ||
...
async function refreshAfterDecision() {
  ...
  if (
    ...
    await router.replace({
      query: adminQuery(nextPage, nextRejectedPage, nextMatchedPage),
    })
  }
}
...
function applyTMDBMetadataRefreshStatus(
  response: AdminTMDBMetadataRefreshResponse,
) {
  ...
  observedRunningMetadataRefreshStartedAt = presentation.running
    ? (nextJob?.started_at ?? null)
    : null
...
async function loadTMDBMetadataRefreshStatus(
  fromPolling = false,
): Promise<AdminTMDBMetadataRefreshResponse | null> {
...
async function refreshTMDBMetadata() {
  ...
  try {
    ...
    if (getApiErrorStatus(error) === 409) {
      ...
      if (response && response.job?.state !== 'running')
        metadataRefreshError.value = conflictMessage
      return
    }
...
async function approveWithTmdbId(
  ...
  errorMessage.value = ''
  try {
    await api.adminApproveMatch(
      match.source_provider,
      match.source_movie_id,
      tmdbId,
    )
...
function replacementTmdbId(match: AdminPendingMatch): number | null {
  ...
  return adminReplacementTMDBId(
    replacementTmdbIds.value[sourceKey(match)],
  ...
  try {
    await api.adminCreateLocalMovieGroup({
      members: selectedSourceList.value.map(
        ({ source_provider, source_movie_id }) => ({
          source_provider,
          source_movie_id,
        }),
      ),
      primary: {
        ...
        source_movie_id: primary.source_movie_id,
      },
    })
    clearMergeSelection()
    await refreshResources()
...
async function addSelectedSourcesToGroup(group: AdminLocalMovieGroup) {
  ...
  try {
    ...
    if (getApiErrorStatus(error) === 404) {
      errorMessage.value =
        'Ce regroupement n’existe plus. Les listes ont été actualisées.'
      await refreshResources()
    } else {
      await handleMutationError(error)
    }
...
async function unmerge(group: AdminLocalMovieGroup) {
  ...
  try {
    ...
    if (getApiErrorStatus(error) === 404) {
      errorMessage.value =
        'Ce regroupement avait déjà été supprimé. Les listes ont été actualisées.'
      await refreshResources()
    } else {
      await handleMutationError(error)
    }
...
function changeRejectedPage(nextOffset: number) {
  if (
    rejectedPending.value ||
    anyMutation.value ||
    nextOffset < 0 ||
    nextOffset === rejectedOffset.value
  )
    return
  scrollRejectedAfterLoad = true
  router.push({
    query: adminQuery(page.value, Math.floor(nextOffset / PAGE_SIZE) + 1),
  })
}

function changeMatchedPage(nextOffset: number) {
  if (
    matchedPending.value ||
    anyMutation.value ||
    nextOffset < 0 ||
    nextOffset === matchedOffset.value
  )
    return
  ...
  router.push({
    query: adminQuery(
      page.value,
      rejectedPage.value,
      Math.floor(nextOffset / PAGE_SIZE) + 1,
    ),
  })
}
...
function selectAdjacentTab(event: KeyboardEvent, index: number) {
  ...
  if (event.key === 'ArrowRight') nextIndex = (index + 1) % MATCH_TABS.length
    ...
    nextIndex = (index - 1 + MATCH_TABS.length) % MATCH_TABS.length
  ...
  >
    ...
    >
      ...
      <p class="sr-only" role="status" aria-live="polite">
        {{
          metadataRefreshLoading ? 'Actualisation des métadonnées TMDB en cours pour tous les films associés.' : ''
        }}
      </p>
      <div
      ...
      >
        <p class="font-semibold">
          {{
            metadataRefreshSummary.failed > 0 ? 'Actualisation terminée avec des erreurs.' : 'Actualisation terminée.'
          }}
        </p>
        <p class="mt-1">
          {{ metadataRefreshSummary.processed }} traité(s),
          {{ metadataRefreshSummary.updated }} mis à jour,
          {{ metadataRefreshSummary.unchanged }} inchangé(s),
          {{ metadataRefreshSummary.failed }} en erreur.
        </p>
      </div>
    </section>
    ...
    <div class="mt-7">
      ...
      >
        <div class="flex flex-wrap items-center justify-between gap-3">
          ...
          <div class="flex flex-wrap items-center gap-3">
            ...
            <button
              ...
              :disabled="activeMatchSection.pending || anyMutation"
              @click="activeMatchSection.load()"
            >
              <RefreshCw
                :size="16"
                :class="activeMatchSection.pending ? 'animate-spin' : ''"
                aria-hidden="true"
              />
              Actualiser
            </button>
        ...
        <label
          ...
          for="matched-search"
          class="mt-4 block max-w-xl text-sm font-semibold text-ink"
        >
        ...
        <template v-if="activeTab === 'unresolved'">
          <p class="sr-only" role="status" aria-live="polite">
            {{
              rerunPending ? 'Relance TMDB en cours pour tous les films non résolus.' : ''
            }}
          </p>
          <div
            v-if="rerunError"
          ...
          >
            <p class="font-semibold">
              {{
                rerunSummary.failed > 0 ? 'Relance terminée avec des erreurs.' : 'Relance terminée.'
              }}
            </p>
            <p class="mt-1">
              {{ rerunSummary.processed }} traité(s),
              {{ rerunSummary.matched }} associé(s),
              {{ rerunSummary.review_required }} à vérifier,
              {{ rerunSummary.unmatched }} sans correspondance,
              {{ rerunSummary.reused }} réutilisé(s),
              {{ rerunSummary.failed }} en erreur.
            </p>
          </div>
        </template>
        ...
        <template v-else-if="activeMatchSection.result">
          ...
          <ul
            ...
            :aria-label="activeMatchSection.title"
          >
            <li
            ...
            >
              ...
              <div
                ...
                :class="match.status === 'rejected' ? '' : 'lg:grid-cols-[14rem_minmax(0,1fr)] lg:gap-6'"
              >
                <section
                  class="min-w-0"
                  ...
                  <div class="flex min-w-0 gap-3">
                    ...
                    >
                      <PosterImage
                        ...
                        sizes="(min-width: 1024px) 80px, (min-width: 640px) 96px, 80px"
                        :reset-key="matchesPosterVersion"
                        class="h-full w-full"
                        image-class="h-full w-full object-cover"
                        fallback-class="gap-1 px-2 text-center text-[11px] font-medium leading-tight text-muted"
                        :fallback-icon-size="24"
                      />
                    </div>
                    <div class="min-w-0 flex-1">
                      <h3
                        :id="`source-title-${domKey(match)}`"
                        class="line-clamp-3 text-sm font-semibold leading-snug text-ink"
                      >
                        {{ match.source_title }}
                      </h3>
                      <dl class="mt-2 space-y-1 text-xs text-muted">
                        <div>
                          <dt class="sr-only">Durée source</dt>
                          <dd>{{ match.source_runtime_minutes }} min</dd>
                        </div>
                        <div class="break-all">
                          <dt class="inline">
                            <BrandedText
                              :text="`ID ${providerLabel(match.source_provider)} :`"
                            />
                          </dt>
                          <dd class="inline">{{ match.source_movie_id }}</dd>
                        </div>
                      </dl>
                      <div class="mt-3 flex flex-col items-start gap-2">
                        <a
                          v-if="match.source_detail_url"
                          :href="match.source_detail_url"
                          target="_blank"
                          rel="noopener noreferrer"
                          class="inline-flex items-center gap-1 text-xs font-semibold text-accent hover:underline"
                          :aria-label="`Voir sur ${providerLabel(match.source_provider)}, ouverture dans un nouvel onglet`"
                          ><BrandedText
                            :text="`Voir sur ${providerLabel(match.source_provider)}`"
                            decorative
                          /><ExternalLink :size="13" aria-hidden="true" /></a
                        >
                        <a
                          v-if="adminTMDBSearchURL(match.source_title)"
                          :href="adminTMDBSearchURL(match.source_title)"
                          target="_blank"
                          rel="noopener noreferrer"
                          class="inline-flex items-center gap-1 text-xs font-semibold text-accent hover:underline"
                          aria-label="TMDB, ouverture dans un nouvel onglet"
                          >TMDB
                          <ExternalLink
                            :size="13"
                            class="shrink-0"
                ...
                <section
                  v-if="match.status === 'matched' && match.current_match"
                  class="min-w-0"
                  ...
                  <div class="flex min-w-0 gap-3">
                    ...
                    >
                      <PosterImage
                        ...
                        :alt="`Affiche TMDB de ${match.current_match.title}`"
                    ...
                    <div class="min-w-0 flex-1">
                      <h3
                        :id="`current-tmdb-title-${domKey(match)}`"
                        class="line-clamp-2 text-sm font-semibold leading-snug text-ink"
                      >
                        {{ match.current_match.title }}
                      </h3>
                      <p
                        class="mt-1 line-clamp-2 text-xs leading-snug text-muted"
                      >
                        Titre original :
                        {{
                          match.current_match.original_title || 'non renseigné'
                        }}
                      </p>
                      <dl class="mt-2 space-y-1 text-xs text-muted">
                        <div>
                          <dt class="inline">ID TMDB :</dt>
                          <dd class="inline">{{ match.current_match.id }}</dd>
                        </div>
                        <div>
                          <dt class="sr-only">Durée TMDB</dt>
                          <dd>{{ candidateRuntime(match.current_match) }}</dd>
                        </div>
                        <div>
                          <dt class="inline">Score :</dt>
                          <dd class="inline">
                            {{ candidateScore(match.current_match) }}
                          </dd>
                        </div>
                      </dl>
                      <a
                        v-if="match.current_match.detail_url"
                        :href="match.current_match.detail_url"
                        target="_blank"
                        rel="noopener noreferrer"
                        class="mt-3 inline-flex items-center gap-1 text-xs font-semibold text-accent hover:underline"
                        >Voir sur TMDB
                        <ExternalLink :size="13" aria-hidden="true" /></a
                      >
                    </div>
                ...
                <fieldset
                  v-else-if="match.status !== 'rejected'"
                  class="min-w-0"
                  :disabled="anyMutation"
                >
                  <legend
                  ...
                  <div
                    v-if="match.candidates.length"
                    class="grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-5"
                  >
                    <div
                    ...
                    >
                      <label
                        ...
                        class="flex min-w-0 cursor-pointer items-start gap-2.5"
                      >
                        <input
                          :id="`candidate-${domKey(match)}-${candidate.id}`"
                          v-model="selectedCandidates[sourceKey(match)]"
                          type="radio"
                        ...
                        <span class="min-w-0 flex-1"
                          ...
                          ><span
                            ...
                            >Titre original :
                            {{
                              candidate.original_title || 'non renseigné'
                            }}</span
                          ><span class="mt-2 block space-y-1 text-xs text-muted"
                            ><span class="block"
                              >ID TMDB : {{ candidate.id }}</span
                            ><span class="block">{{
                              candidateRuntime(candidate)
                            }}</span><span class="block"
                              >Score : {{ candidateScore(candidate) }}</span
                            ></span
                          ></span
                        >
                      </label>
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        class="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-accent hover:underline"
                        >Voir sur TMDB
                        <ExternalLink :size="13" aria-hidden="true" /></a
                      >
                    </div>
              ...
              >
                ...
                >
                  <LoaderCircle
                    v-if="activeMutation === sourceKey(match) && activeMutationKind === 'candidate'"
                    :size="17"
                    class="animate-spin"
                    aria-hidden="true"
                  /><Check v-else :size="17" aria-hidden="true" />Valider le
                  candidat
                </button>
                <form
                  class="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-end"
                  @submit.prevent="assignManual(match)"
                >
                  <div class="min-w-0 flex-1">
                    <label
                      :for="`manual-tmdb-${domKey(match)}`"
                      class="mb-1.5 block text-sm font-semibold text-ink"
                      >Identifiant TMDB</label
                    ><input
                  ...
                  >
                    <LoaderCircle
                      v-if="activeMutation === sourceKey(match) && activeMutationKind === 'manual'"
                      :size="17"
                      class="animate-spin"
                      aria-hidden="true"
                    /><Check v-else :size="17" aria-hidden="true" />Associer cet
                    identifiant TMDB
                  </button>
                </form>
                <div
              ...
              >
                <form
                  ...
                  class="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-end"
                  @submit.prevent="requestCorrection(match)"
                >
                  <div class="min-w-0 flex-1">
                    <label
                      :for="`replacement-tmdb-${domKey(match)}`"
                      class="mb-1.5 block text-sm font-semibold text-ink"
                      >Nouvel identifiant TMDB</label
                    ><input
                ...
                >
                  ...
                  <div class="flex flex-wrap gap-2">
                    ...
                    >
                      <LoaderCircle
                        v-if="activeMutation === sourceKey(match) && activeMutationKind === 'correction'"
                        :size="17"
                        class="animate-spin"
                        aria-hidden="true"
                      /><Check v-else :size="17" aria-hidden="true" />Confirmer
                    </button>
                    <button
                      type="button"
        ...
        >
          ...
          <span
            ...
            aria-live="polite"
            >Page {{ activeMatchSection.page }}</span
          >
          <button
            type="button"
            class="min-h-11 rounded-md border border-line bg-surface px-4 text-sm font-semibold text-ink disabled:opacity-50"
    ...
    >
      <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        >
          <RefreshCw
            :size="16"
            :class="groupsPending ? 'animate-spin' : ''"
            aria-hidden="true"
          />
          Actualiser
        </button>
      ...
      <ul
        ...
        aria-label="Regroupements locaux actifs"
      >
        <li
        ...
        >
          ...
          >
            <div class="min-w-0 text-xs">
              <h3 class="break-all text-sm font-semibold text-ink">
                {{ group.local_movie_id }}
              </h3>
              <p class="mt-0.5 text-muted">
                Primaire : {{ providerLabel(group.primary.source_provider) }} ·
                {{ group.primary.source_movie_id }}
              </p>
              <p v-if="group.metadata_source" class="mt-0.5 text-muted">
                Métadonnées :
                {{ providerLabel(group.metadata_source.source_provider) }} ·
            ...
            <div class="flex flex-wrap items-center justify-end gap-1.5">
              ...
              <template v-if="unmergeConfirmation === group.local_movie_id">
                ...
                >
                  <LoaderCircle
                    v-if="unmergePending === group.local_movie_id"
                    :size="14"
            ...
            >
              ...
              >
                <PosterImage
                  ...
                  :alt="`Affiche de ${member.source_title ?? member.source_movie_id}`"
                  sizes="32px"
                  :reset-key="groupsPosterVersion"
                  class="h-full w-full"
                  image-class="h-full w-full object-cover"
              ...
              <div class="min-w-0 flex-1 text-xs leading-tight">
                <p class="truncate font-semibold text-ink">
                  {{ member.source_title ?? 'Source indisponible' }}
                </p>
                <p class="mt-0.5 truncate text-muted">
                  {{ providerLabel(member.source_provider) }} ·
                  {{ member.source_movie_id
                  }}<span v-if="member.source_runtime_minutes">
                    · {{ member.source_runtime_minutes }} min</span
                  >
                </p>
              </div>
              <div
                class="shrink-0 text-right text-[11px] font-semibold leading-tight"
              >
      ...
      <nav
        v-if="!groupsPending && !groupsError && (groupsOffset > 0 || canGroupsGoNext)"
        class="mt-5 grid grid-cols-2 items-center justify-center gap-4 sm:flex"
        aria-label="Pagination des regroupements locaux"
      >
        <button
          type="button"
          class="min-h-11 rounded-md border border-line bg-surface px-4 text-sm font-semibold text-ink disabled:opacity-50"
