...
async function fetchCity() {
  try {
    return {
      ...
      detail: await api.city(slug.value),
      errorMessage: '',
    }
  } catch (error) {
    if (isNotFoundError(error))
      return { kind: 'not-found' as const, detail: null, errorMessage: '' }
    return {
...
async function fetchCatalog(currentDetail: CityDetailResponse) {
  if (currentDetail.theaters.length === 0)
    return { catalog: null, errorMessage: '' }
  try {
    return {
...
const initial = await useAsyncData(
  `city:${slug.value}:${appliedSearch.value}:${sort.value}:${page.value}`,
  async () => {
    const city = await fetchCity()
    if (city.kind !== 'success' || !city.detail)
      return { city, catalog: null, catalogErrorMessage: '' }
    const movieCatalog = await fetchCatalog(city.detail)
    return {
      city,
      catalog: movieCatalog.catalog,
      catalogErrorMessage: movieCatalog.errorMessage,
    }
  },
)
...
if (import.meta.server && initialState?.city.kind !== 'success') {
  ...
  if (event)
    setResponseStatus(
      event,
      initialState?.city.kind === 'not-found' ? 404 : 502,
    )
}
...
function changeSort(nextSort: MovieSort) {
  ...
  void router.replace({
    query: cityCatalogQuery(appliedSearch.value, nextSort, 1),
  })
}
...
onMounted(async () => {
  ...
  if (!queriesEqual(route.query, canonicalQuery))
    await router.replace({ query: canonicalQuery })
  else if (catalog.value && page.value > totalPages.value)
    await router.replace({
      query: cityCatalogQuery(
        appliedSearch.value,
        sort.value,
        totalPages.value,
      ),
...
const canonicalUrl = computed(() =>
  absoluteSiteUrl(
    ...
    `/ville/${encodeURIComponent(slug.value)}/cinemas`,
  ),
)
const pageTitle = computed(() =>
  detail.value
    ? `Cinémas à ${detail.value.city.name} - MesSeances`
    : 'Cinémas par ville - MesSeances',
)
const pageDescription = computed(() =>
  detail.value
    ? cityDescription(
        detail.value.city.name,
        detail.value.theaters.length,
        detail.value.movies.length,
      )
  ...
  const graph: JsonLdNode[] = [
    {
      ...
      itemListElement: [
        ...
        },
        {
          ...
          position: 2,
          name: 'Cinémas',
          item: absoluteSiteUrl(config.public.siteUrl, '/cinemas'),
        },
        {
          '@type': 'ListItem',
          position: 3,
...
useHead(() => ({
  ...
  script: cityJsonLd.value
    ? [
        {
          key: 'city-jsonld',
          type: 'application/ld+json',
          innerHTML: cityJsonLd.value,
        },
      ]
    : [],
}))
...
<template>
  ...
  >
    <EditorialStatePanel
      ...
      size="standard"
      shadow="large"
      class="city-state mx-auto max-w-3xl font-bold"
      ><template #icon
        ><LoaderCircle
          :size="34"
          class="animate-spin"
        ...
        /></template
      >
      <p>Chargement de la ville…</p></EditorialStatePanel
    >
    <EditorialStatePanel
      v-else-if="notFound"
      semantic="alert"
      ...
      shadow="large"
      class="city-state mx-auto max-w-3xl font-bold"
      ><template #icon><MapPin :size="36" aria-hidden="true" /></template
      ><template #heading
        ><h1 class="text-2xl font-black">Ville introuvable</h1></template
      >
      <p>Cette ville n’est pas disponible dans la programmation actuelle.</p>
      <template #actions
        ><NuxtLink
          to="/cinemas"
    ...
    <template v-else-if="detail">
      ...
      <header class="border-2 border-ink bg-surface shadow-[8px_8px_0_#27272a]">
        <div class="grid lg:grid-cols-[minmax(0,1.55fr)_minmax(17rem,0.65fr)]">
          <div class="min-w-0 p-5 sm:p-8 lg:p-10">
            <div class="min-w-0">
              ...
              >
                {{ detail.city.name }}<span class="text-primary">.</span>
              </h1>
            </div>
          ...
          <dl
            class="grid grid-cols-2 border-t-2 border-ink lg:grid-cols-1 lg:border-l-2 lg:border-t-0"
          >
            <div class="min-w-0 p-5 sm:p-6">
              ...
              <dd class="mt-2 pl-8 text-2xl font-black leading-none">
                {{ detail.theaters.length }}
              </dd>
            </div>
            <div
        ...
        <div
          class="border-t-2 border-ink bg-[#f1efe8] px-5 py-4 sm:px-8 sm:py-5 lg:px-10"
        >
          <p
            class="max-w-4xl text-sm font-semibold leading-6 sm:text-base sm:leading-7"
          >
            {{ pageDescription }}
          </p>
        </div>
      ...
      <section class="mt-12" aria-labelledby="city-cinemas-heading">
        ...
        <EditorialStatePanel
          v-if="detail.theaters.length === 0"
          size="standard"
          shadow="large"
          class="city-state mx-auto mt-8 max-w-3xl font-bold"
          ...
          >
          <p>
            La programmation actuelle ne contient aucun cinéma dans cette ville.
          </p></EditorialStatePanel
        >
        <ul v-else class="mt-8 grid gap-5 md:grid-cols-2">
          <li
            v-for="theater in detail.theaters"
            :key="theater.id"
            class="border-2 border-ink bg-surface p-5 shadow-[6px_6px_0_#27272a]"
          >
            <h3 class="text-2xl font-black tracking-[-0.04em]">
              <NuxtLink
            ...
            >
              <span v-if="theater.address">{{ theater.address }}<br></span>
              {{ theater.postal_code }} {{ theater.city }}
            </p>
          </li>
        </ul>
      </section>
      ...
      <section class="mt-14" aria-labelledby="city-films-heading">
        ...
        <MovieCatalogControls
          v-if="detail.theaters.length"
          class="mt-8 border-2 border-ink bg-[#ffcf3f] p-4 shadow-[7px_7px_0_#27272a] sm:p-6"
          :search="appliedSearch"
          :sort="sort"
          :pending="catalogPending"
          input-id="city-film-search"
          @search="submitSearch"
          @sort="changeSort"
        />
        ...
        <EditorialStatePanel
          ...
          >
          <p>
            Aucun film n’est programmé dans cette ville sur la période actuelle.
          </p></EditorialStatePanel
        >
        <EditorialStatePanel
        ...
        <EditorialStatePanel
          ...
          <p>
            {{
              appliedSearch ? 'Aucun film ne correspond à cette recherche.' : 'Aucun film n’est programmé dans cette ville sur la période actuelle.'
            }}
          </p>
          <template v-if="appliedSearch" #actions
            ><button
              type="button"
        ...
        <template v-else>
          <div
            class="mt-8 flex items-end justify-between gap-4 border-b-2 border-ink pb-4"
          >
            <h3 class="text-2xl font-black tracking-[-0.04em]">
              {{
            ...
            <p
              class="shrink-0 font-mono text-[11px] font-bold uppercase tracking-[0.14em]"
            >
              {{ catalog.total }} film{{ catalog.total > 1 ? 's' : '' }}
            </p>
          </div>
          <ul
            class="mt-8 grid grid-cols-2 gap-x-4 gap-y-8 sm:grid-cols-3 sm:gap-x-6 lg:grid-cols-4 xl:grid-cols-6"
          ...
          >
            ...
            >
              <MovieCatalogCard
                ...
                :poster-reset-key="posterVersion"
              />
            </li>
          </ul>
