...
export const ADMIN_MOVIE_ROUTE_KEYS = [
  'q',
  'runtime_min',
  'runtime_max',
  'release_date_from',
...
export type AdminMovieDraftOperation =
  | { kind: 'override'; value: AdminMovieDraftValue }
  | { kind: 'restore' }
...
export function adminMovieDraftFingerprint(
  draft: AdminMovieDraft | undefined,
): string {
  if (!draft) return '[]'
  return JSON.stringify(
    adminMovieFields.flatMap((field) => {
      ...
      return operation
        ? [
            [
              field,
              operation.kind,
              operation.kind === 'override' ? operation.value : null,
            ],
          ]
        : []
    }),
...
export function isAdminMovieDraftDirty(
  draft: AdminMovieDraft | undefined,
): boolean {
  return adminMovieDraftFingerprint(draft) !== '[]'
}

export function adminMovieFieldValue(
  item: AdminMovieItem,
  draft: AdminMovieDraft | undefined,
  field: AdminMovieField,
): AdminMovieDraftValue {
  const operation = draft?.operations[field]
...
export function isAdminMovieFieldOverridden(
  item: AdminMovieItem,
  draft: AdminMovieDraft | undefined,
  field: AdminMovieField,
): boolean {
...
export function stageAdminMovieOverride(
  item: AdminMovieItem,
  draft: AdminMovieDraft | undefined,
  field: AdminMovieField,
  value: AdminMovieDraftValue,
): AdminMovieDraft {
  const next = cloneDraft(draft)
  next.expected_updated_at ??= item.updated_at
  const normalized = normalizeAdminMovieDraftValue(field, value)
  if (
    item.overridden_fields.includes(field) &&
    valuesEqual(normalized, item.values[field])
  ) {
...
export function stageAdminMovieRestore(
  item: AdminMovieItem,
  draft: AdminMovieDraft | undefined,
  field: AdminMovieField,
): AdminMovieDraft {
  const next = cloneDraft(draft)
  next.expected_updated_at ??= item.updated_at
  if (item.overridden_fields.includes(field))
    next.operations[field] = { kind: 'restore' }
  else delete next.operations[field]
  return next
}

export function validateAdminMovieDraft(
  item: AdminMovieItem,
  draft: AdminMovieDraft | undefined,
): AdminMovieValidationErrors {
  const errors: AdminMovieValidationErrors = {}
  const value = (field: AdminMovieField) =>
    adminMovieFieldValue(item, draft, field)
  const title = value('title')
  if (!isStringValue(title) || title.trim() === '')
    errors.title = 'Indiquez un titre.'
  else if (runeLength(title.trim()) > 1024)
    errors.title = 'Le titre ne peut pas dépasser 1 024 caractères.'

  const runtime = value('runtime_minutes')
  if (
    !isNumberValue(runtime) ||
    !Number.isInteger(runtime) ||
    runtime < 0 ||
    runtime > maxRuntime
  )
    errors.runtime_minutes = 'Utilisez une durée entière positive ou nulle.'

  const releaseDate = value('release_date')
  if (
    releaseDate !== null &&
    (!isStringValue(releaseDate) || !isCalendarDate(releaseDate))
  )
    errors.release_date = 'Utilisez une date valide au format AAAA-MM-JJ.'
  ...
  if (!Array.isArray(genres)) errors.genres = 'Les genres sont invalides.'
    ...
    genres.some(
      (genre) => genre.trim() === '' || runeLength(genre.trim()) > 256,
    )
  )
    errors.genres = 'Chaque genre doit contenir de 1 à 256 caractères.'
  ...
  validateTrailerKey(
    ...
    value('trailer_vf_youtube_key'),
  )
  validateTrailerKey(
    errors,
    'trailer_vo_youtube_key',
    value('trailer_vo_youtube_key'),
  )

  const vf = value('trailer_vf_youtube_key')
  const vo = value('trailer_vo_youtube_key')
  if (isStringValue(vf) && vf !== '' && vf === vo) {
    errors.trailer_vo_youtube_key =
      'Les bandes-annonces VF et VO doivent être différentes.'
  }
  return errors
}

export function buildAdminMoviePatch(
  item: AdminMovieItem,
  draft: AdminMovieDraft | undefined,
): AdminMoviePatchRequest | null {
  if (
    !isAdminMovieDraftDirty(draft) ||
    Object.keys(validateAdminMovieDraft(item, draft)).length > 0
  )
    return null
  ...
  for (const field of adminMovieFields) {
    ...
    if (operation.kind === 'restore') restore.push(field)
      ...
      Object.assign(overrides, {
        [field]: normalizeAdminMovieDraftValue(field, operation.value),
      })
  }
  const request: AdminMoviePatchRequest = {
    expected_updated_at: draft?.expected_updated_at ?? item.updated_at,
  }
...
export function parseAdminMovieRouteQuery(
  query: Readonly<AdminMovieRouteQueryInput>,
): AdminMovieRouteState {
  ...
  if (releaseDateFrom && releaseDateTo && releaseDateFrom > releaseDateTo)
    releaseDateTo = undefined
  const overrideStatus =
    enumValue(queryValue(query.override_status), adminMovieOverrideStatuses) ??
    'all'
  const overrideField =
    overrideStatus === 'automatic'
      ? undefined
      : enumValue(queryValue(query.override_field), adminMovieFields)
  const sort = enumValue(queryValue(query.sort), adminMovieSorts) ?? 'title'
  const direction =
    enumValue(queryValue(query.direction), adminMovieDirections) ?? 'asc'
...
export function adminMovieRouteQuery(
  state: AdminMovieRouteState,
): AdminMovieOwnedRouteQuery {
  return {
    q: state.q || undefined,
    runtime_min:
      state.runtime_min === undefined ? undefined : String(state.runtime_min),
    runtime_max:
      state.runtime_max === undefined ? undefined : String(state.runtime_max),
    release_date_from: state.release_date_from,
    release_date_to: state.release_date_to,
    genre: state.genre || undefined,
    override_status:
      state.override_status === 'all' ? undefined : state.override_status,
...
export function adminMovieRouteStateFromGrid(
  state: AdminMovieRouteState,
  sortModel: readonly AdminMovieGridSort[],
  filterModel: Readonly<AdminMovieGridFilterModel>,
): AdminMovieRouteState {
  ...
  const runtimeMin =
    runtime?.type === 'inRange' ? numberFilterValue(runtime.filter) : undefined
  const runtimeMax =
    runtime?.type === 'inRange'
      ? numberFilterValue(runtime.filterTo)
      : undefined
  const releaseDateFrom =
    releaseDate?.type === 'inRange'
      ? calendarDate(dateFilterValue(releaseDate.dateFrom))
      : undefined
  const releaseDateTo =
    releaseDate?.type === 'inRange'
      ? calendarDate(dateFilterValue(releaseDate.dateTo))
      : undefined
  return {
    ...state,
    ...
    genre:
      genre?.type === 'contains'
        ? boundedTrimmed(String(genre.filter ?? ''), 256)
        : '',
    sort:
      sortEntry && isAdminMovieSort(sortEntry.colId)
        ? sortEntry.colId
        : 'title',
...
export function adminMovieGridFilterModel(
  state: AdminMovieRouteState,
  ...
  if (state.runtime_min !== undefined || state.runtime_max !== undefined) {
    model.runtime_minutes = {
      type: 'inRange',
      filter: state.runtime_min ?? 0,
      filterTo: state.runtime_max ?? maxRuntime,
    }
  }
  if (state.release_date_from || state.release_date_to) {
    model.release_date = {
      type: 'inRange',
      dateFrom: state.release_date_from ?? '0001-01-01',
      dateTo: state.release_date_to ?? '9999-12-31',
    }
  }
...
export function adminMovieQueryFromGrid(
  state: AdminMovieRouteState,
  request: AdminMovieGridRequest,
): AdminMoviesQuery {
  const translated = adminMovieRouteStateFromGrid(
    state,
    request.sortModel ?? [],
    request.filterModel ?? {},
  )
  const start = Math.max(
    0,
    request.startRow ?? (state.page - 1) * ADMIN_MOVIE_PAGE_SIZE,
  )
  const end = Math.max(
    start + 1,
    request.endRow ?? start + ADMIN_MOVIE_PAGE_SIZE,
  )
  ...
  if (translated.runtime_min !== undefined)
    query.runtime_min = translated.runtime_min
  if (translated.runtime_max !== undefined)
    query.runtime_max = translated.runtime_max
  if (translated.release_date_from)
    query.release_date_from = translated.release_date_from
  if (translated.release_date_to)
    query.release_date_to = translated.release_date_to
  if (translated.genre) query.genre = translated.genre
  if (translated.override_field)
    query.override_field = translated.override_field
  return query
}
...
function normalizeAdminMovieDraftValue(
  field: AdminMovieField,
  value: AdminMovieDraftValue,
): AdminMovieDraftValue {
  if (field === 'title' && isStringValue(value)) return value.trim()
  if (field === 'genres' && Array.isArray(value))
    return value.map((genre) => genre.trim())
  return cloneValue(value)
}

function valuesEqual(
  left: AdminMovieDraftValue,
  right: AdminMovieDraftValue,
): boolean {
  return JSON.stringify(left) === JSON.stringify(right)
}

function validateOptionalString(
  errors: AdminMovieValidationErrors,
  field: AdminMovieField,
  value: AdminMovieDraftValue,
  maxLength: number,
) {
  if (
    value !== null &&
    (!isStringValue(value) || runeLength(value) > maxLength)
  )
    errors[field] =
      `Ce champ ne peut pas dépasser ${maxLength.toLocaleString('fr-FR')} caractères.`
}
...
function validateTrailerKey(
  errors: AdminMovieValidationErrors,
  field: 'trailer_vf_youtube_key' | 'trailer_vo_youtube_key',
  value: AdminMovieDraftValue,
) {
  if (
    value !== null &&
    (!isStringValue(value) || !/^[A-Za-z0-9_-]{11}$/.test(value))
  )
    errors[field] = 'Utilisez une clé YouTube de 11 caractères.'
}
...
function nonnegativeInteger(value: string | undefined): number | undefined {
  ...
  return Number.isInteger(parsed) && parsed >= 0 && parsed <= maxRuntime
    ? parsed
    : undefined
}
...
function isStringValue(
  value: RouteInputValue | AdminMovieDraftValue,
): value is string {
