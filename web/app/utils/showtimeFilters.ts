...
export const queryLanguageValues = queryLanguageOptions.map(
  (option) => option.value,
)
export const showtimeLanguageValues = [
  'VOSTFR',
  'VF',
  'VO',
  'VF_SME',
  'VFSTF',
] as const satisfies readonly ShowtimeLanguage[]
export type FilmLanguageFilter = 'ALL' | 'ORIGINAL' | 'VOF' | ShowtimeLanguage
export const filmLanguageValues = [
  'ORIGINAL',
  'VOF',
  ...showtimeLanguageValues,
] as const
export const queryFormatOptions = formatOptions
export const queryFormatValues = queryFormatOptions.map(
  (option) => option.value,
)
...
export function languageLabel(
  ...
  return (
    showtimeLanguageOptions.find((option) => option.value === value)?.label ??
...
export function availableLanguageOptions(
  available: readonly ShowtimeLanguage[],
): readonly ShowtimeFilterOption<'ALL' | ShowtimeLanguage>[] {
  const values = new Set(available)
  return showtimeLanguageOptions.filter(
    (option) => option.value === 'ALL' || values.has(option.value),
  )
}

export function availableFilmLanguageOptions(
  available: readonly ShowtimeLanguage[],
): readonly ShowtimeFilterOption<FilmLanguageFilter>[] {
  return [
    queryLanguageOptions[0],
    queryLanguageOptions[1],
    queryLanguageOptions[2],
    ...availableLanguageOptions(available).filter(
      (option) => option.value !== 'ALL',
    ),
  ]
}

export function matchesFilmLanguageFilter(
  language: string,
  requested: FilmLanguageFilter,
  originalLanguage?: string | null,
): boolean {
  if (requested === 'ALL') return true
  if (requested !== 'ORIGINAL' && requested !== 'VOF')
    return language === requested
  return (
    (requested === 'ORIGINAL' &&
      (language === 'VO' || language === 'VOSTFR')) ||
    (originalLanguage === 'fr' &&
      (language === 'VF' || language === 'VF_SME' || language === 'VFSTF'))
  )
}

export function availableFormatOptions(
  available: readonly ShowtimeFormat[],
): readonly (typeof formatOptions)[number][] {
  const values = new Set(available)
...
export function showtimeFilterSummary(
  language: FilmLanguageFilter,
  format: QueryFormat,
  originalLanguage?: string | null,
): string {
  const labels: string[] = []
  ...
  return labels.length
    ? labels.join(' · ')
    : `${languageLabel('ALL')} · ${formatLabel('ALL')}`
}
