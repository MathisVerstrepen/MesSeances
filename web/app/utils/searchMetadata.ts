...
import { calendarDate, enumQueryValue, singularQueryValue } from './routeQuery.ts'
import { languageLabel, queryFormatValues, queryLanguageValues } from './showtimeFilters.ts'

export const DEFAULT_SEARCH_DESCRIPTION = 'Trouvez les séances qui tiennent entièrement dans votre créneau horaire.'

const theaterIdPattern = /^[A-Za-z0-9][A-Za-z0-9_-]{0,127}$/
const validTimes = new Set(createServiceTimeOptions().map((option) => option.value))
...
export function buildSearchMetaDescription(query: LocationQuery): string {
  ...
  const language = enumQueryValue(singularQueryValue(query.language), queryLanguageValues)
  const format = enumQueryValue(singularQueryValue(query.format), queryFormatValues)
  const filters = [
