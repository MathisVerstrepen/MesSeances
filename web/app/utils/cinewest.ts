...
export function safeCinewestBooking(
  raw: string,
  showtimeId?: string | null,
  expectedTheaterId?: string | null,
): { url: string; kind: 'website' | 'booking' } | null {
  ...
  for (const [theaterId, host] of Object.entries(CINEWEST_THEATER_HOSTS)) {
    ...
    if (!theaterId.startsWith('ticketingcine-') || !raw.startsWith(root))
      continue
    ...
    if (
      !match ||
      match[0] !== route ||
      `ticketingcine-EMS${match[2]}` !== theaterId
    )
      continue
    if (
      showtimeId &&
      showtimeId !== cinewestTicketShowingId(theaterId, match[1]!)
    )
      return null
...
export function isSafeCinewestPoster(raw: string): boolean {
  ...
  const match =
    /^(?:https:\/\/cinemedia\.cine\.digital\/medias\/[0-9]+\/[a-f0-9]{2}\/[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}\.(?:jpeg|jpg|png|webp)|https:\/\/images\.monnaie-services\.com\/movie_poster\/(?:120|600)\/FR[A-Z0-9]{5}\/[A-Z0-9]{8}\.webp|https:\/\/all\.web\.img\.acsta\.net\/img\/[a-f0-9]{2}\/[a-f0-9]{2}\/[a-f0-9]{24,64}\.(?:jpeg|jpg|png|webp))$/.exec(
      raw,
    )
  return match?.[0] === raw
}
