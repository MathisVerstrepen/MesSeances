...
const WEEKDAY_ORDER: readonly AdminSyncWeekday[] = [
  'mon',
  'tue',
  'wed',
  'thu',
...
export function adminSyncScheduleDraftFromItem(
  ...
  return {
    ...
    time: '',
    weekdays: [],
    expression: item.schedule.expression,
  }
}

export function adminSyncScheduleDraftFingerprint(
...
export function validateAdminSyncScheduleDraft(
  draft: AdminSyncScheduleDraft,
): AdminSyncScheduleDraftValidation {
  ...
  if (draft.kind === 'daily' || draft.kind === 'weekly') {
    ...
    else if (!TIME_PATTERN.test(draft.time))
      errors.time = 'Utilisez une heure valide au format HH:MM.'
  }
  ...
  if (draft.kind === 'cron') {
    ...
    else if (new TextEncoder().encode(expression).length > 255)
      errors.expression = 'L’expression cron ne peut pas dépasser 255 octets.'
    else if (expression.split(' ').length !== 5)
      errors.expression =
...
export function buildAdminSyncScheduleRequest(
  draft: AdminSyncScheduleDraft,
): AdminSaveSyncScheduleRequest {
  if (draft.kind === 'daily') {
    return {
  ...
  if (draft.kind === 'weekly') {
    ...
    return {
      ...
      schedule: {
        kind: 'weekly',
        time: draft.time,
        weekdays: WEEKDAY_ORDER.filter((weekday) => selected.has(weekday)),
      },
    }
  }
  return {
    ...
    schedule: {
      kind: 'cron',
      expression: normalizeCronExpression(draft.expression),
    },
  }
}
...
export function adminSyncRunDurationMilliseconds(
  run: AdminSyncJob,
  now: number,
): number {
  const startedAt = new Date(run.started_at).getTime()
  const finishedAt =
    run.finished_at === null ? now : new Date(run.finished_at).getTime()
