...
import type {
  ...
  UpcomingReviewFilter,
  UpcomingReviewReason,
} from '../types/api.ts'
...
export const upcomingReviewReasonLabels = {
  ...
  non_theatrical_before_or_same_day:
    'Diffusion hors cinéma antérieure ou le même jour',
...
export function parseUpcomingReviewRoute(
  query: LocationQuery,
): UpcomingReviewRouteState {
  const filter =
    upcomingReviewFilters.find(
      (option) => option.value === singularQueryValue(query.filter),
    )?.value ?? 'needs_review'
  ...
  return {
    ...
    page:
      page <= Math.floor(2_147_483_647 / UPCOMING_REVIEW_PAGE_SIZE) + 1
        ? page
        : 1,
  }
}

export function upcomingReviewRouteQuery(
  state: UpcomingReviewRouteState,
...
export function upcomingReviewApiQuery(
  state: UpcomingReviewRouteState,
): AdminUpcomingMoviesQuery {
  return {
    filter: state.filter,
    search: state.q || undefined,
    limit: UPCOMING_REVIEW_PAGE_SIZE,
    offset: (state.page - 1) * UPCOMING_REVIEW_PAGE_SIZE,
  }
}
