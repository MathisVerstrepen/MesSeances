...
export function statisticsPeriod(route: LocationQuery) {
  ...
  const choice = statisticsPeriods.find(
    (choice) => choice.value === route.period,
  )
  return choice
    ? { period: choice.value, error: '' }
    : {
        period: 'next7' as const,
        error:
          'Période invalide. Choisissez une période ou réinitialisez les filtres.',
      }
}
...
export function historyDateError(date?: string, through?: string) {
  ...
  if (!valid(date) || (through !== undefined && !valid(through)))
...
export function parseStatisticsPageQuery(
  route: LocationQuery,
  today = statisticsParisToday(),
) {
  const { period, error } = statisticsPeriod(route)
  if (error) return { query: {}, error }
  const values = { ...route }
  if (period === 'custom') {
    if (values.date === undefined || values.date_to === undefined)
      return {
        query: {},
        error: 'Choisissez une date de début et une date de fin.',
      }
  } else {
    delete values.date
    ...
    if (range) {
      values.date = range.from
      values.date_to = range.through
    }
  }
...
export function statisticsPageDraft(
  route: LocationQuery,
  today = statisticsParisToday(),
): StatisticsPageDraft {
  ...
  const draft = statisticsDraft(
    values,
    statisticsPeriodRange(period, today),
...
export function statisticsCustomDraft(
  draft: StatisticsPageDraft,
  displayed: StatisticsDateRange | null | undefined,
  today = statisticsParisToday(),
): StatisticsPageDraft {
  ...
  return {
    ...
    period: 'custom',
    date: range.from,
    date_to: range.through,
  }
}
export function statisticsPageDraftQuery(
  draft: StatisticsPageDraft,
  today = statisticsParisToday(),
) {
  return statisticsDraftQuery(
    { ...draft, explicitDates: draft.period === 'custom' },
    today,
    (values) =>
      parseStatisticsPageQuery({ ...values, period: draft.period }, today),
  )
}
export function statisticsPageRoute(
  route: LocationQuery,
  period: StatisticsPeriod = 'next7',
...
export function historySelectionOptions(
  items: readonly StatisticsSelectOption[],
  known: readonly StatisticsSelectOption[],
  selected: readonly string[],
  resolved: ReadonlySet<string>,
) {
  const labels = new Map(
    [...items, ...known].map((option) => [option.value, option]),
  )
  const visible = new Set(items.map((option) => option.value))
  return [
    ...items.map((option) => labels.get(option.value) ?? option),
    ...selected
      ...
      .map(
        (value) =>
          labels.get(value) ?? {
            value,
            label: resolved.has(value) ? `${value} (indisponible)` : value,
          },
      ),
  ]
}
...
export function createHistoryOptionsRequest(callbacks: {
  start: () => void
  success: (result: HistoryOptionsResponse) => void
  ...
  return {
    schedule(
      fetcher: (signal: AbortSignal) => Promise<HistoryOptionsResponse>,
      immediate = false,
    ) {
      clearTimeout(timer)
      request.cancel()
