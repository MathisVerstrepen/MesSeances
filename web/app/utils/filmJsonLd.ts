...
export function buildFilmJsonLd(
  schedule: MovieShowtimesResponse,
  options: FilmJsonLdOptions,
): JsonLdDocument {
  const movieId = `${options.movieUrl}#movie`
  const images = [
    safePosterUrl(schedule.movie.poster_url),
    safeBackdropUrl(schedule.backdrop_url),
  ...
  if (schedule.movie.runtime_minutes > 0)
    movie.duration = `PT${schedule.movie.runtime_minutes}M`
  if (schedule.movie.overview?.trim())
    movie.description = schedule.movie.overview.trim()
      ...
      itemListElement: [
        {
          ...
          position: 1,
          name: 'Accueil',
          item: absoluteSiteUrl(options.siteUrl, '/'),
        },
        {
          '@type': 'ListItem',
          position: 2,
          name: 'Films',
          item: absoluteSiteUrl(options.siteUrl, '/films'),
        },
        {
          '@type': 'ListItem',
          position: 3,
          name: schedule.movie.title,
          item: options.movieUrl,
        },
      ],
    },
  ]
  ...
  for (const theater of schedule.theaters) {
    ...
    const theaterUrl = absoluteSiteUrl(
      options.siteUrl,
