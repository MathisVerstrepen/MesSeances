...
export function isValidShortLinkTarget(target: string): boolean {
  if (
    !target ||
    utf8ByteLength(target) > MAX_TARGET_BYTES ||
    !isValidUnicode(target)
  )
    return false
  if (
    !target.startsWith('/') ||
...
function isValidQuery(
  ...
  for (const field of rawQuery.split('&')) {
    ...
    if (key === SHARED_THEATERS_KEY && !isValidSharedTheaters(value))
      return false
    seen.add(key)
  }
  return true
...
function isValidSharedTheaters(value: string): boolean {
  ...
  return (
    ids.every((id) => THEATER_ID_PATTERN.test(id)) &&
