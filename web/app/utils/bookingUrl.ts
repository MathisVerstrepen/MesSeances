...
function isSafeMegaramaBooking(value: string, hostname: string): boolean {
  ...
  if (aliasCinema) {
    ...
    return (
      session !== null &&
      session[0] === route &&
      `EMS${session[1]}` === aliasCinema
    )
  }
  return /^\/?(?:#showsession\?id=[A-Za-z0-9][A-Za-z0-9_-]{0,110})?$/.test(
    route,
  )
}
...
function isSafeNoeCinemasBooking(
  value: string,
  showtimeId?: string | null,
  theaterId?: string | null,
): boolean {
  if (value.length > 4096) return false
  const entry = Object.entries(NOE_CINEMAS_BOOKING_PREFIXES).find(
  ...
  if (theaterId !== undefined && theaterId !== `noecinemas-${theater}`)
    return false
  if (showtimeId !== undefined) {
    if (showtimeId === null) return false
    const showing = /^noecinemas-showing-([A-Z0-9]{5})-([a-f0-9]{64})$/.exec(
      showtimeId,
    )
    if (!showing || showing[0] !== showtimeId || showing[1] !== theater)
      return false
  }
  return true
}
...
function isSafeCinevilleBooking(value: string): boolean {
  const match =
    /^https:\/\/www\.cineville\.fr\/vad\/([1-9][0-9]{0,18})\/([1-9][0-9]{0,18})\/([1-9][0-9]{0,18})$/.exec(
      value,
    )
  return (
    match !== null &&
    match[0] === value &&
    match.slice(1).every((id) => BigInt(id) <= 9223372036854775807n)
  )
}
...
function isSafeMk2Booking(value: string, showtimeId?: string | null): boolean {
  ...
  return (
    isValidMk2ShowingId(showingId) &&
    (showtimeId === undefined || showtimeId === `mk2-showing-${showingId}`)
  )
}

export function safeBookingUrl(
  ...
  if (
    raw &&
    (expectedProvider === 'noecinemas' || !expectedProvider) &&
    isSafeNoeCinemasBooking(raw, showtimeId, theaterId)
  ) {
  ...
  try {
          ...
          : hostname === 's.pathe.fr'
            ...
            : hostname === 'achat.cgrcinemas.fr'
              ...
              : hostname === 'achat.grandecran.fr'
                ? 'grandecran'
                : hostname === 'www.cineville.fr'
                  ? 'cineville'
                  : hostname === 'www.mk2.com'
                    ? 'mk2'
                    : MEGARAMA_HOSTS.has(hostname) ||
                        MEGARAMA_BOOKING_HOSTS.has(hostname)
                      ? 'megarama'
                      : null
    ...
    if (
      ...
      (provider === 'grandecran' &&
        (raw !== value ||
          parsed.href !== value ||
          !isSafeGrandEcranBooking(value))) ||
      (provider === 'mk2' &&
        (raw !== value ||
          parsed.href !== value ||
          !isSafeMk2Booking(value, showtimeId))) ||
      (provider === 'cineville' &&
        (raw !== value ||
          parsed.href !== value ||
          !isSafeCinevilleBooking(value))) ||
      (provider === 'megarama' &&
        (raw !== value || !isSafeMegaramaBooking(value, hostname)))
    )
      return null
    ...
    return {
      provider,
      url: parsed.href,
      kind: provider === 'megarama' && !parsed.hash ? 'website' : 'booking',
    }
  } catch {
    return null
  }
