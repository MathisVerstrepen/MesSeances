...
export type StatisticsFilterKey = Exclude<
  (typeof statisticsQueryKeys)[number],
  'date' | 'date_to' | 'film'
>
export type StatisticsMultiFilterKey = 'city' | 'theater'
export type StatisticsScalarFilterKey = Exclude<
  StatisticsFilterKey,
  StatisticsMultiFilterKey
>
export type StatisticsDraft = Record<
  Exclude<(typeof statisticsQueryKeys)[number], StatisticsMultiFilterKey>,
  string
> &
  Record<StatisticsMultiFilterKey, string[]> & { explicitDates: boolean }
...
const invalidFilters =
  'Filtres invalides. Vérifiez les champs ou réinitialisez la sélection.'

export function statisticsDateError(
  date: string | undefined,
  through: string | undefined,
  today?: string,
): string {
  if (date === undefined && through === undefined) return ''
  if (!calendarDate(date) || (through !== undefined && !calendarDate(through)))
    return 'Choisissez des dates valides au format jour, mois, année.'
  ...
  if (
    (Date.parse(`${end}T12:00:00Z`) - Date.parse(`${date}T12:00:00Z`)) /
      86400000 >=
    31
  )
    return 'Choisissez une période de 31 jours maximum.'
  return ''
}
...
export function parseStatisticsQuery(
  route: LocationQuery,
  today?: string,
  validateDates = statisticsDateError,
): ParsedStatisticsQuery {
  ...
  for (const key of statisticsQueryKeys) {
    ...
    if (
      (!multiple && Array.isArray(raw)) ||
      (multiple && entries.length > statisticsMaxSelections)
    )
      return { query: {}, error: invalidFilters }
    ...
    for (const entry of entries) {
      ...
      if (
        key === 'film' &&
        (entry.includes('\0') || /[\uD800-\uDFFF]/u.test(entry))
      )
        return { query: {}, error: invalidFilters }
      const isDate = key === 'date' || key === 'date_to'
      if (!isDate && new TextEncoder().encode(entry).length > 200)
        return { query: {}, error: invalidFilters }
  ...
  if (new TextEncoder().encode(encoded.toString()).length > 4096)
    return { query: {}, error: invalidFilters }
  if (
    (values.chain && !chains.includes(values.chain)) ||
    (values.language &&
      !languages.some((value) => value === values.language)) ||
  ...
  if (values.chain)
    query.chain = enumQueryValue(values.chain, [
      'ugc',
      'kinepolis',
      'pathe',
      'cgr',
  ...
  if (values.language)
    query.language = enumQueryValue(values.language, languages)
  if (values.format)
    query.format = enumQueryValue(values.format, [
      '2D',
      '3D',
      'IMAX',
      'DOLBY',
...
export function statisticsDraft(
  route: LocationQuery,
  range?: StatisticsDateRange | null,
  parse = parseStatisticsQuery,
): StatisticsDraft {
  const draft: StatisticsDraft = {
    date: '',
    date_to: '',
    city: [],
    theater: [],
    ...
    pass: '',
    film: '',
    explicitDates: route.date !== undefined || route.date_to !== undefined,
  }
  ...
  for (const key of statisticsQueryKeys) {
    ...
    if (key === 'city' || key === 'theater')
      draft[key] =
        value === undefined
          ? []
          : (Array.isArray(value) ? value : [value]).map((entry) => entry ?? '')
  ...
  } else if (draft.date && route.date_to === undefined)
    draft.date_to = draft.date
  return draft
}

export function statisticsDraftQuery(
...
export function statisticsRouteQuery(
  route: LocationQuery,
  query: StatisticsQuery = {},
): LocationQuery {
  const values = {
    ...query,
    city: query.city?.length ? [...query.city] : undefined,
    theater: query.theater?.length ? [...query.theater] : undefined,
  }
...
export const statisticsHeatmapColors = [
  '#ffffff',
  '#f1f7ed',
  '#e0eed7',
  '#c9e1bc',
...
export function statisticsHeatmapStyle(count: number, maximum: number) {
  ...
  return {
    backgroundColor: statisticsHeatmapColors[level],
    color: level >= 7 ? '#ffffff' : '#20251f',
  }
}

export function statisticsBucketLabel(
  value: string,
  label: string,
...
export interface StatisticsBar extends StatisticsBucket {
  href?: string
  detail?: string
}
export function statisticsBars(rows: readonly StatisticsBar[], total?: number) {
  ...
  return rows.map((row) => ({
    ...
    share: total === undefined ? undefined : statisticsShare(row.count, total),
  }))
}
...
export function statisticsOptionsWithSelection(
  options: readonly StatisticsSelectOption[],
  selected: readonly string[],
) {
  const available = new Set(options.map((option) => option.value))
  const missing = [...new Set(selected)].filter(
  ...
  return missing.length
    ? [
        ...
        ...missing.map((value) => ({
          value,
          label: `${value} (indisponible)`,
        })),
      ]
    : options
}

export function statisticsSearchOptions(
  options: readonly StatisticsSelectOption[],
  selected: readonly string[],
  search: string,
) {
  ...
  return options.filter(
    (option) =>
      ...
      option.label.toLocaleLowerCase('fr-FR').includes(term),
  )
}

export function toggleStatisticsSelection(
  selected: readonly string[],
  value: string,
  maximum: number,
): string[] {
...
export function statisticsSelectionSummary(
  options: readonly StatisticsSelectOption[],
  selected: readonly string[],
  label: string,
  allLabel: string,
  ...
  if (selected.length === 1)
    return (
      ...
      `${selected[0]} (indisponible)`
    )
  return `${selected.length} ${label.toLocaleLowerCase('fr-FR')}s ${label === 'Ville' ? 'sélectionnées' : 'sélectionnés'}`
}

export const statisticsHours = Array.from(
  { length: 24 },
  (_, index) => (index + 8) % 24,
)
export const statisticsWeekdays = [
  'Lundi',
  'Mardi',
  'Mercredi',
  'Jeudi',
...
export function statisticsHeatmapRows(cells: readonly StatisticsHeatmapCell[]) {
  const counts = new Map(
    cells.map((cell) => [`${cell.weekday}:${cell.hour}`, cell.showtime_count]),
  )
  return statisticsWeekdays.map((label, index) => ({
    label,
    cells: statisticsHours.map((hour) => ({
      hour,
...
const cityNameParticles = new Set([
  'à',
  'au',
  'aux',
  'd',
...
export function statisticsCityName(value: string) {
  return value
    ...
    .replace(/\p{L}[\p{L}\p{M}]*/gu, (word: string, offset: number) =>
      offset > 0 && cityNameParticles.has(word)
        ? word
        : word.charAt(0).toLocaleUpperCase('fr-FR') + word.slice(1),
    )
}
export type StatisticsLocalColumn =
  | 'name'
  | 'movie_count'
  | 'showtime_count'
  | 'theater_count'
  | 'city'
export interface StatisticsLocalSort {
  column: StatisticsLocalColumn
...
function localValue(
  row: StatisticsLocalRow,
  column: StatisticsLocalColumn,
): string | number {
  if (column === 'city') return 'city' in row ? row.city : ''
  if (column === 'theater_count')
    return 'theater_count' in row ? row.theater_count : 0
  return row[column]
}
export function nextStatisticsSort(
  current: StatisticsLocalSort | null,
  column: StatisticsLocalColumn,
): StatisticsLocalSort {
    ...
    direction:
      current?.column === column
        ? current.direction === 'ascending'
          ? 'descending'
          : 'ascending'
        : column === 'name' || column === 'city'
          ? 'ascending'
          : 'descending',
  }
}
export function statisticsLocalPage(
  rows: readonly StatisticsLocalRow[],
  sort: StatisticsLocalSort | null,
  page: number,
) {
  const sorted = [...rows].sort((left, right) => {
    const tie =
      compareText(normalized(left.name), normalized(right.name)) ||
      compareText(localID(left), localID(right))
    if (!sort)
      return (
        right.showtime_count - left.showtime_count ||
        right.movie_count - left.movie_count ||
        tie
      )
    const a = localValue(left, sort.column)
    const b = localValue(right, sort.column)
    const comparison =
      sort.column === 'name' || sort.column === 'city'
        ? compareText(normalized(String(a)), normalized(String(b)))
        : Number(a) - Number(b)
  ...
  return {
    rows: sorted.slice((currentPage - 1) * 20, currentPage * 20),
    page: currentPage,
    pages,
    total: rows.length,
  }
...
export function createStatisticsRequest<T>(callbacks: {
  start: () => void
