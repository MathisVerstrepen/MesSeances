...
onMounted(() =>
  nextTick(() => {
    if (backdropImage.value?.complete && backdropImage.value.naturalWidth === 0)
      backdropFailed.value = true
  }),
)
</script>
...
<template>
  <article
    v-if="movie"
    :data-movie-slug="scope === 'single-theater' ? movie.movieSlug : undefined"
    class="border-2 border-ink bg-surface shadow-[6px_6px_0_#27272a]"
  >
    <header
    ...
    >
      ...
      />
      <div
        class="aspect-[2/3] w-24 overflow-hidden border-2 border-ink bg-[#e8e6de] shadow-[5px_5px_0_#27272a] sm:w-[120px]"
      >
        <PosterImage
          :src="posterUrl"
      ...
      <div class="min-w-0 pb-1">
        <h3
          class="break-words text-2xl font-black tracking-[-0.04em] sm:text-3xl"
        >
          <NuxtLink
            :to="`/film/${encodeURIComponent(movie.movieSlug)}`"
        ...
        <p
          class="mt-1 font-mono text-[0.68rem] font-black uppercase tracking-[0.1em]"
          :class="backdropUrl && !backdropFailed ? 'text-white' : undefined"
        >
          {{ movie.movieRuntimeMinutes }} min
        </p>
      </div>
    </header>
    ...
    <header
      v-else
      class="relative overflow-hidden border-b-2 border-ink bg-ink px-4 py-4 text-white sm:px-5 sm:py-5"
    >
      <img
        v-if="backdropUrl && !backdropFailed"
        ref="backdropImage"
      ...
      <div class="relative flex items-center gap-4">
        <div
          class="flex aspect-[2/3] w-16 shrink-0 items-center justify-center overflow-hidden border-2 border-ink bg-[#e8e6de] shadow-[4px_4px_0_#27272a] sm:w-[4.5rem]"
        >
          <PosterImage
            :src="posterUrl"
            :alt="`Affiche de ${movie.movieTitle}`"
        ...
        <div
          class="flex min-w-0 flex-1 flex-col gap-1 sm:flex-row sm:items-end sm:justify-between"
        >
          <div class="min-w-0">
            <h3
              class="text-xl font-black leading-snug tracking-[-0.03em] text-white sm:text-2xl"
            >
              <NuxtLink
                :to="`/film/${encodeURIComponent(movie.movieSlug)}`"
            ...
            <p
              class="mt-1 font-mono text-[10px] font-bold uppercase tracking-[0.1em] text-white"
            >
              {{ movie.movieRuntimeMinutes }} min
            </p>
          </div>
          <p
            class="shrink-0 font-mono text-[10px] font-bold uppercase tracking-[0.1em] text-white"
          >
    ...
    <ul
      v-if="layout === 'lines'"
      class="divide-y-2 divide-ink"
      :aria-label="scope === 'single-theater' ? `Séances de ${movie.movieTitle}` : 'Séances compatibles'"
    >
      <ShowtimeResultLine
        ...
        :scope="scope"
        :show-movie="false"
        :selected="selectedKeySet.has(result.key)"
        @toggle-selection="emit('toggleSelection', $event)"
      />
    </ul>
    <ul
      v-else
      class="grid grid-cols-[repeat(auto-fill,minmax(150px,1fr))] gap-3 p-4 sm:grid-cols-[repeat(auto-fill,minmax(180px,1fr))] sm:gap-4 sm:p-5 lg:grid-cols-[repeat(auto-fill,minmax(210px,1fr))]"
      :aria-label="scope === 'single-theater' ? `Séances de ${movie.movieTitle}` : 'Séances compatibles'"
    >
      <li v-for="result in results" :key="result.key" class="min-w-0">
        <ShowtimeResultBox
          :result="result"
          :scope="scope"
          :show-movie="false"
