    ...
    <div class="grid gap-5 xl:grid-cols-[minmax(0,1fr)_auto] xl:items-start">
      <div class="min-w-0">
        <div class="flex flex-wrap items-center gap-2 text-xs font-semibold">
          ...
          <span class="rounded bg-subtle px-2 py-1 text-stone-700">{{ result.showtime.format }}</span>
        </div>
        ...
        <div class="mt-2 flex flex-wrap gap-x-5 gap-y-2 text-sm text-muted">
          <span class="flex items-center gap-2"><MapPin :size="15" aria-hidden="true" /> {{ result.theater.name }}</span>
