...
<template>
  <nav v-if="totalPages > 1" class="pagination mt-14 grid grid-cols-2 items-stretch justify-between gap-2 border-2 border-ink bg-surface p-3 shadow-[6px_6px_0_#27272a] sm:flex sm:items-center sm:gap-4" aria-label="Pagination des films">
    ...
    <span class="order-first col-span-2 min-w-0 text-center font-mono text-[11px] font-bold uppercase tracking-[0.14em] wrap-anywhere sm:order-none" aria-live="polite">Page {{ page }} / {{ totalPages }}</span>
