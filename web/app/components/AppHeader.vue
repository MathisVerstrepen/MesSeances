...
const hasCompactPageControls = computed(
  () =>
...
const favoriteSummary = computed(() => {
  ...
  for (const theater of favoriteTheaters.value) {
    if (!cityLabels.has(theater.city_slug))
...
<template>
  ...
  >
    <div
      class="mx-auto flex min-h-16 max-w-[1440px] flex-wrap items-center px-4 sm:px-6 lg:min-h-[4.5rem] lg:flex-nowrap lg:px-10"
    >
      <NuxtLink
        to="/"
        class="brand-link flex h-16 shrink-0 items-center gap-2.5 font-black tracking-[-0.04em] text-ink lg:h-[4.5rem]"
        aria-label="MesSeances, accueil"
      >
        <span
          class="grid size-9 rotate-[-2deg] place-items-center rounded-[3px] border-2 border-ink bg-[#ffcf3f] shadow-[3px_3px_0_#27272a]"
        >
          <Clapperboard :size="19" stroke-width="2.5" aria-hidden="true" />
        </span>
        <span class="text-lg"
          >MesSeances<span class="text-primary">.</span></span
        >
      </NuxtLink>
