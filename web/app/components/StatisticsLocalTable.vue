...
const columns = computed<{ key: StatisticsLocalColumn; label: string }[]>(() => [
  ...
  mode.value === 'cities' ? { key: 'theater_count', label: 'Cinémas' } : { key: 'city', label: 'Ville' }
])
const result = computed(() => statisticsLocalPage(props.local[mode.value], sort.value, page.value))
watch(() => props.local, () => { page.value = 1 })
watch(mode, () => { sort.value = null; page.value = 1 })
function sortBy(column: StatisticsLocalColumn) {
  sort.value = nextStatisticsSort(sort.value, column)
  page.value = 1
}
const buttonClass = 'min-h-11 border-2 border-ink px-4 py-2 text-sm font-extrabold hover:bg-highlight focus-visible:outline-3 focus-visible:outline-offset-3 focus-visible:outline-ink disabled:cursor-not-allowed disabled:opacity-40'
</script>

<template>
  <div>
    <fieldset class="mb-6 flex flex-wrap gap-4">
      <legend class="sr-only">Afficher l’offre locale par ville ou cinéma</legend>
      <label v-for="option in [{ value: 'cities', label: 'Villes' }, { value: 'theaters', label: 'Cinémas' }]" :key="option.value" class="flex min-h-11 cursor-pointer items-center gap-2 font-extrabold">
    ...
    <div class="mt-5 flex flex-wrap items-center justify-between gap-3">
      <p class="text-sm" role="status">{{ statisticsCount(result.total) }} {{ mode === 'cities' ? 'villes' : 'cinémas' }} · Page {{ result.page }} sur {{ result.pages }}</p>
      <nav v-if="result.pages > 1" class="flex gap-2" aria-label="Pagination de l’offre locale">
        <button type="button" :class="buttonClass" :disabled="result.page === 1" @click="page = result.page - 1">Précédent</button>
        <button type="button" :class="buttonClass" :disabled="result.page === result.pages" @click="page = result.page + 1">Suivant</button>
      </nav>
    </div>
  </div>
</template>
