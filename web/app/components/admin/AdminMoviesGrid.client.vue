...
const routeState = ref<AdminMovieRouteState>(
  parseAdminMovieRouteQuery(route.query),
)
const searchInput = ref(routeState.value.q)
const overrideStatusInput = ref(routeState.value.override_status)
const overrideFieldInput = ref<AdminMovieField | ''>(
  routeState.value.override_field ?? '',
)
...
const selectedDetailsErrors = computed(() =>
  selectedDetailsItem.value
    ? (validationErrors.value[selectedDetailsItem.value.id] ?? {})
    : {},
)

const context = {
  formatFieldValue,
  isFieldOverridden: (item: AdminMovieItem, field: AdminMovieField) =>
    isAdminMovieFieldOverridden(item, drafts.value[item.id], field),
  restoreField,
  detailsId,
  isDetailsOpen: (item: AdminMovieItem) =>
    selectedDetailsItem.value?.id === item.id,
  isDirty: (item: AdminMovieItem) =>
    isAdminMovieDraftDirty(drafts.value[item.id]),
...
function inlineColumn(
  field: 'title' | 'runtime_minutes' | 'release_date',
  options: Partial<ColDef<AdminMovieItem>> = {},
): ColDef<AdminMovieItem> {
  const editor =
  ...
  return {
    ...
    valueGetter: (params) =>
      params.data
        ? adminMovieFieldValue(params.data, drafts.value[params.data.id], field)
        : null,
...
const columnDefs: ColDef<AdminMovieItem>[] = [
  ...
  {
    colId: 'showtime_count',
    headerName: 'Séances à venir',
    width: 175,
    field: 'showtime_count',
    cellDataType: 'number',
    sortable: true,
    filter: false,
    editable: false,
  ...
  {
    colId: 'updated_at',
    headerName: 'Actualisé',
    width: 155,
    valueGetter: (params) => formatDateTime(params.data?.updated_at),
    sortable: true,
  },
  { colId: 'id', headerName: 'ID', width: 115, field: 'id', sortable: true },
  {
    ...
    filter: false,
    editable: false,
    cellRenderer: AdminMoviesActionsCell,
  },
]
...
const datasource: IDatasource = {
  getRows(params: IGetRowsParams<AdminMovieItem>) {
    ...
    void api
      .adminMovies(query, controller.signal)
      .then((response) => {
        if (
          !mounted ||
        ...
        for (const item of response.items) {
          if (selectedDetailsItem.value?.id === item.id)
...
function updateDraft(
  ...
  const draft = stageAdminMovieOverride(
    item,
...
function setDraft(item: AdminMovieItem, draft: AdminMovieDraft) {
  ...
  validationErrors.value[item.id] = validateAdminMovieDraft(
    item,
    drafts.value[item.id],
  )
  delete rowErrors.value[item.id]
  gridApi.value?.refreshCells({
    rowNodes: gridApi.value.getRowNode(item.id)
      ? [gridApi.value.getRowNode(item.id)!]
      : [],
    force: true,
  })
}
...
async function saveMovie(item: AdminMovieItem) {
  ...
  try {
    ...
    if (selectedDetailsItem.value?.id === item.id)
      selectedDetailsItem.value = updated
...
function toggleDetails(item: AdminMovieItem) {
  selectedDetailsItem.value =
    selectedDetailsItem.value?.id === item.id ? null : item
}
...
function updateDetail(field: AdminMovieField, event: Event) {
  ...
  const value: AdminMovieDraftValue =
    field === 'genres'
      ? raw
          .split('\n')
...
function onSortOrFilterChanged(
  _event: SortChangedEvent<AdminMovieItem> | FilterChangedEvent<AdminMovieItem>,
) {
  if (applyingGridState || !gridApi.value) return
  const next = adminMovieRouteStateFromGrid(
    routeState.value,
    gridApi.value
      .getColumnState()
      .flatMap((column) =>
        column.sort ? [{ colId: column.colId, sort: column.sort }] : [],
      ),
...
function onPaginationChanged(event: PaginationChangedEvent<AdminMovieItem>) {
  ...
  if (nextPage !== routeState.value.page)
    void pushRoute({ ...routeState.value, page: nextPage })
}
...
function updateOverrides() {
  ...
  void replaceRoute({
    ...
    override_field:
      status === 'automatic'
        ? undefined
        : overrideFieldInput.value || undefined,
    page: 1,
  })
}
...
function routeQuery(state: AdminMovieRouteState) {
  const ownedValues = Object.fromEntries(
    Object.entries(adminMovieRouteQuery(state)),
  )
...
async function applyRoute() {
  ...
  const ownedValues = Object.fromEntries(
    Object.entries(adminMovieRouteQuery(nextState)),
  )
  const canonical = mergeOwnedQuery(
    route.query,
    ADMIN_MOVIE_ROUTE_KEYS,
    ownedValues,
  )
  ...
  if (previousDataFingerprint !== routeDataFingerprint)
    gridApi.value.purgeInfiniteCache()
...
<template>
  <section aria-label="Éditeur des métadonnées de films">
    ...
    >
      ...
      <label class="text-sm font-semibold text-ink">
        ...
        >
          ...
          <option
            v-for="(label, field) in fieldOptions"
            :key="field"
            :value="field"
          >
            {{ label }}
    ...
    <div
      v-if="initialLoading"
      class="mb-3 flex min-h-12 items-center justify-center gap-2 text-sm text-muted"
      role="status"
      aria-live="polite"
    >
      <LoaderCircle
    ...
    >
      <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex gap-2">
          ...
          >
            {{
              pendingRows[selectedDetailsItem.id] ? 'Enregistrement…' : 'Enregistrer'
            }}
          </button>
          <button
            type="button"
      ...
      <div class="mt-4 grid gap-5 lg:grid-cols-2">
        <div
          v-for="field in (['genres', 'overview', 'poster_url', 'backdrop_url', 'trailer_vf_youtube_key', 'trailer_vo_youtube_key'] as AdminMovieField[])"
          :key="field"
          :class="field === 'overview' ? 'lg:col-span-2' : ''"
        >
          <div class="mb-1.5 flex min-h-8 flex-wrap items-center gap-2">
            <label
              :for="`${detailsId(selectedDetailsItem)}-${field}`"
              class="text-sm font-semibold text-ink"
              >{{
                adminMovieFieldLabels[field]
              }}</label
            >
            <AdminMoviePosterPicker
              v-if="field === 'poster_url'"
              :key="selectedDetailsItem.id"
              :movie-id="selectedDetailsItem.id"
              :movie-title="selectedDetailsItem.values.title"
              :current-poster-url="detailValue('poster_url') || null"
              :disabled="Boolean(pendingRows[selectedDetailsItem.id])"
              @select="selectPoster"
            />
            <span
            ...
            <button
              v-if="isAdminMovieFieldOverridden(selectedDetailsItem, drafts[selectedDetailsItem.id], field)"
              type="button"
              class="ml-auto text-xs font-semibold text-accent underline underline-offset-2"
              @click="restoreField(selectedDetailsItem, field)"
            >
              Restaurer la valeur automatique
            </button>
          </div>
          <textarea
            ...
            @input="updateDetail(field, $event)"
          />
          <input
            v-else
            :id="`${detailsId(selectedDetailsItem)}-${field}`"
          ...
          >
            Automatique : {{ automaticValue(selectedDetailsItem, field) }}
          </p>
        </div>
      </div>
    </section>
