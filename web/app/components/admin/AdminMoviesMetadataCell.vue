...
const props = defineProps<{
  params: ICellRendererParams<AdminMovieItem> & {
    field: AdminMovieField
    context: MetadataCellContext
  }
}>()
...
const overridden = computed(() =>
  item.value
    ? props.params.context.isFieldOverridden(item.value, props.params.field)
    : false,
)
// SAFETY: Grid value getters for metadata columns return AdminMovieDraftValue.
const displayValue = computed(() =>
  props.params.context.formatFieldValue(
    props.params.field,
    props.params.value as AdminMovieDraftValue,
  ),
)
...
function restore(event: MouseEvent) {
  ...
  if (item.value)
    props.params.context.restoreField(item.value, props.params.field)
}
</script>
