...
const request = createHistoryOptionsRequest({
  ...
  error() {
    error.value =
      'Impossible de charger les options. Vos sélections sont conservées.'
  },
  finish() {
    pending.value = false
  },
...
const options = computed(() =>
  historySelectionOptions(
    items.value,
    [...props.options, ...known.value],
    props.modelValue,
    resolved.value,
  ),
)
function load(immediate = false) {
  const selected = [...new Set(props.modelValue)].slice(
    0,
    statisticsMaxSelections,
  )
  ...
  void request.schedule((signal) => {
    ...
    return api.historyStatisticsOptions(
      { kind: props.kind, q, selected },
      signal,
    )
  }, immediate)
}
...
watch(
  ...
  () => {
    ...
    known.value = options.value.filter((option) =>
...
watch(
  () => props.options,
  (value) => {
    if (!search.value) items.value = value
  },
)
watch(
  () => props.hasMore,
  (value) => {
    if (!search.value) hasMore.value = value ?? false
  },
)
onMounted(() => {
  if (props.modelValue.length) load(true)
})
...
<template>
  <StatisticsMultiSelect
    ...
    :all-label="allLabel"
    :options="options"
    :max-selections="statisticsMaxSelections"
    :single="single"
    external-search
  ...
  >
    <template #feedback>
      <p v-if="pending" role="status" class="mb-2 text-sm">
      ...
      <div v-if="error" role="alert" class="mb-2 text-sm">
        ...
        <button
          type="button"
          class="min-h-11 px-2 font-extrabold underline focus-visible:outline-3 focus-visible:outline-offset-3 focus-visible:outline-ink"
          @click="load(true)"
        >
          Réessayer
        </button>
      </div>
      <p v-if="hasMore" class="mb-2 text-sm font-bold">
        Affinez la recherche pour voir les autres options.
      </p>
    </template>
  </StatisticsMultiSelect>
</template>
