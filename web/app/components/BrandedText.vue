...
type Brand = 'UGC' | 'CGR' | 'MEGARAMA' | 'CINEVILLE' | 'MK2' | 'CINEWEST' | 'IMAX' | 'KINEPOLIS' | 'PATHE'
...
const segments = computed<Segment[]>(() => props.text
  .split(/(?<![\p{L}\p{N}_])(UGC|CGR|Megarama|IMAX|Kinepolis|Pathé|Pathe|Cinéville|Cineville|MK2|Cinewest)(?![\p{L}\p{N}_])/giu)
  .filter(Boolean)
  .map((value) => {
