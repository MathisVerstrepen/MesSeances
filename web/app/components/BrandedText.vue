...
type Brand = 'UGC' | 'CGR' | 'MEGARAMA' | 'CINEVILLE' | 'MK2' | 'IMAX' | 'KINEPOLIS' | 'PATHE'
...
const segments = computed<Segment[]>(() => props.text
  .split(/(?<![\p{L}\p{N}_])(UGC|CGR|Megarama|IMAX|Kinepolis|Pathé|Pathe|Cinéville|Cineville|MK2)(?![\p{L}\p{N}_])/giu)
  .filter(Boolean)
  .map((value) => {
    const brand = value.toUpperCase().replace('É', 'E')
    return brand === 'UGC' || brand === 'CGR' || brand === 'MEGARAMA' || brand === 'CINEVILLE' || brand === 'MK2' || brand === 'IMAX' || brand === 'KINEPOLIS' || brand === 'PATHE' ? { value, brand } : { value }
  }))
</script>
