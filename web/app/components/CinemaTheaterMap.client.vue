...
const theaterData = computed(() =>
  buildTheaterFeatureCollection(props.theaters, favoriteIds.value),
)
const mappedTheaters = computed(() => {
  const mappedIds = new Set(
    theaterData.value.features.map((feature) => feature.properties.id),
  )
...
const selectedTheater = computed(
  () =>
    mappedTheaters.value.find(
      (theater) => theater.id === selectedTheaterId.value,
    ) ?? null,
)
...
function fitTheaterFeatures() {
  ...
  try {
    if (!bounds) {
      ...
      const padding =
        mapContainer.value && mapContainer.value.clientWidth < 640 ? 32 : 56
      map.fitBounds(
        [
...
async function onClusterClick(event: MapLayerMouseEvent) {
  ...
  if (!Number.isSafeInteger(clusterId) || feature?.geometry.type !== 'Point')
    return
  const source = map.getSource(THEATER_SOURCE_ID)
  if (!(source instanceof maplibregl.GeoJSONSource)) return
  try {
    ...
    map.addLayer({
      ...
      paint: {
        ...
        'circle-color': [
          ...
          ['get', 'provider'],
          'ugc',
          THEATER_PROVIDER_COLORS.ugc,
          'kinepolis',
          THEATER_PROVIDER_COLORS.kinepolis,
          'pathe',
          THEATER_PROVIDER_COLORS.pathe,
          'cgr',
          THEATER_PROVIDER_COLORS.cgr,
          'megarama',
          THEATER_PROVIDER_COLORS.megarama,
          'cineville',
          THEATER_PROVIDER_COLORS.cineville,
          'mk2',
          THEATER_PROVIDER_COLORS.mk2,
          'cinewest',
          THEATER_PROVIDER_COLORS.cinewest,
          'grandecran',
          THEATER_PROVIDER_COLORS.grandecran,
          'noecinemas',
          THEATER_PROVIDER_COLORS.noecinemas,
          '#52525b',
        ],
        'circle-stroke-color': '#ffffff',
        'circle-stroke-width': 2,
...
watch(
  () => props.favoriteTheaterIds,
  () => updateTheaterSource(false),
)
...
<template>
  <div class="border-2 border-ink bg-surface shadow-[6px_6px_0_#27272a]">
    <div
      v-if="runtimeFailed"
      class="flex flex-wrap items-center justify-between gap-4 border-2 border-primary bg-primary-soft p-4 text-[#7f1d1d]"
      role="alert"
    >
    ...
    <template v-else>
      <div class="border-b-2 border-ink bg-[#f1efe8] p-[0.9rem]">
        <p class="text-[0.82rem] font-[750]" role="status" aria-live="polite">
          {{ mapTheaterSummary }}
        </p>
      </div>
      ...
      <div
        ref="mapLayout"
        class="map-layout grid min-w-0"
        :class="selectedTheater ? 'min-[900px]:grid-cols-[minmax(0,1fr)_minmax(17rem,22rem)]' : ''"
      >
        <div class="map-panel relative min-w-0">
          <div
            ref="mapContainer"
            class="map-canvas h-[clamp(25rem,62vh,43rem)] w-full max-[520px]:h-[26rem]"
            role="region"
            aria-label="Carte interactive des cinémas"
          ></div>
          <div
            v-if="mappedTheaters.length === 0"
            class="absolute top-4 left-4 z-[2] max-w-[min(22rem,calc(100%_-_5rem))] border-2 border-ink bg-surface p-3 font-extrabold shadow-[3px_3px_0_#27272a]"
            role="status"
          >
            <p>
              Aucun cinéma de cette sélection ne peut être placé sur la carte.
            </p>
            <button
              type="button"
              class="mt-3 inline-flex min-h-11 items-center justify-center border-2 border-ink bg-ink px-[0.8rem] py-[0.6rem] font-mono text-[0.65rem] font-black tracking-[0.06em] text-surface uppercase enabled:hover:bg-primary focus-visible:outline-[3px] focus-visible:outline-offset-[3px] focus-visible:outline-accent disabled:cursor-not-allowed disabled:opacity-50"
              @click="emit('show-list')"
            >
              Afficher la liste
            </button>
          ...
          <div
            class="flex flex-wrap gap-x-[0.9rem] gap-y-2 border-t-2 border-ink bg-[#f8f7f2] px-[0.8rem] py-[0.65rem] text-xs font-extrabold"
            aria-label="Légende des exploitants"
          >
            <span
              v-for="(label, provider) in THEATER_PROVIDER_LABELS"
              :key="provider"
              class="inline-flex items-center gap-[0.35rem]"
              ><i
                class="inline-block size-[0.8rem] rounded-full border-2 border-ink"
                :style="{ backgroundColor: THEATER_PROVIDER_COLORS[provider] }"
                aria-hidden="true"
              ></i>{{ label }}</span
            >
            <span class="inline-flex items-center gap-[0.35rem]"
              ><i
                class="inline-block size-4 rounded-full border-3 border-double border-ink bg-transparent"
                aria-hidden="true"
              ></i
              >Mes cinémas</span
            >
          ...
          <p
            class="border-t border-ink bg-surface px-[0.8rem] py-[0.4rem] text-[0.7rem] leading-[1.4]"
          >
            Carte
            <a
              class="font-[850] underline decoration-2 underline-offset-[3px] focus-visible:outline-[3px] focus-visible:outline-offset-[3px] focus-visible:outline-accent"
        ...
        <article
          v-if="selectedTheater"
          class="theater-details border-t-2 border-ink bg-[#f1efe8] p-4 min-[900px]:border-t-0 min-[900px]:border-l-2"
          aria-labelledby="selected-theater-heading"
        >
          <div class="flex items-start justify-between gap-4">
            <div>
              <p
                class="font-mono text-[0.68rem] font-black uppercase tracking-[0.09em]"
              >
                {{ THEATER_PROVIDER_LABELS[selectedTheater.provider] }}
              </p>
              <h3
                id="selected-theater-heading"
                ref="detailHeading"
                tabindex="-1"
                class="mt-[0.35rem] text-[clamp(1.35rem,3vw,2rem)] leading-none font-black tracking-[-0.04em] focus-visible:outline-[3px] focus-visible:outline-offset-4 focus-visible:outline-accent"
              >
                <TheaterName
                  :name="selectedTheater.name"
                  :provider="selectedTheater.provider"
                />
              </h3>
            </div>
            <button
              type="button"
              class="min-h-11 p-[0.4rem] font-mono text-[0.65rem] font-black uppercase underline underline-offset-[3px] focus-visible:outline-[3px] focus-visible:outline-offset-[3px] focus-visible:outline-accent"
              aria-label="Fermer les détails du cinéma"
              @click="closeDetails"
            >
              Fermer
            </button>
          </div>
          <p class="mt-4 leading-[1.55] font-[650]">
            <template v-if="selectedTheater.address"
              >{{ selectedTheater.address }},
            </template>{{ selectedTheater.postal_code }}
            {{ selectedTheater.city }}
          </p>
          <p
            class="mt-4 font-mono text-[0.68rem] font-black uppercase tracking-[0.09em]"
          >
            {{
              favoriteIds.has(selectedTheater.id) ? 'Dans mes cinémas' : 'Hors de mes cinémas'
            }}
          </p>
          <div class="mt-4 flex flex-wrap items-center gap-x-4 gap-y-3">
            ...
            <NuxtLink
              :to="`/cinema/${encodeURIComponent(selectedTheater.slug)}`"
              class="font-[850] underline decoration-2 underline-offset-[3px] focus-visible:outline-[3px] focus-visible:outline-offset-[3px] focus-visible:outline-accent"
