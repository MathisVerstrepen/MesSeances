...
const visibleOptions = computed(() => statisticsSearchOptions(props.options, props.modelValue, search.value))
...
function toggle(value: string) {
  emit('update:modelValue', toggleStatisticsSelection(props.modelValue, value, props.maxSelections))
}
...
<template>
  <fieldset class="min-w-0">
    ...
    <details ref="disclosure" class="relative mt-2 min-w-0 border-2 border-ink bg-surface open:z-30" @keydown.esc="close">
      ...
      <div class="absolute -inset-x-0.5 top-full min-w-0 border-2 border-ink bg-surface p-3 shadow-[5px_5px_0_#27272a]">
        ...
        <div class="max-h-64 overflow-y-auto overscroll-contain p-1">
          <label v-for="(option, index) in visibleOptions" :key="option.value" :for="`${id}-option-${index}`" class="flex min-h-11 cursor-pointer items-center gap-3 py-2 text-sm has-disabled:cursor-not-allowed has-disabled:opacity-40">
            <input :id="`${id}-option-${index}`" type="checkbox" :checked="selected.has(option.value)" :disabled="atLimit && !selected.has(option.value)" :aria-describedby="atLimit ? `${id}-limit` : undefined" :class="['size-5 shrink-0 accent-ink', focusClass]" @change="toggle(option.value)" />
