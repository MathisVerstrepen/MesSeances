...
async function component(name: string): Promise<Component> {
  ...
  const localRequire = (id: string) => id.startsWith('~/assets/') ? { __esModule: true, default: id } : require(id)
  new Function('require', 'exports', 'computed', compiled)(localRequire, exports, computed)
  assert.ok(exports.default)
  return exports.default
}

const [TheaterName, BrandLogo, BrandedText] = await Promise.all(['TheaterName', 'BrandLogo', 'BrandedText'].map(component))

async function render(name: string, provider: Provider, options: { decorative?: boolean; logoClass?: string } = {}) {
...
test('Cinewest theater names get one provider logo and preserve actual source names', async () => {
  for (const name of ['Cinéma GALAXY', 'Étoile Cinémas', 'Les Toiles du Moun', 'Cinéma Liberté', 'Capitole Studios']) {
...
test('Cinewest already in the name keeps its text without duplicate logos or accessible brand names', async () => {
  for (const name of ['Cinewest Le Lido', 'Cinéma Cinewest', 'CINEWEST GALAXY']) {
    const html = await render(name, 'cinewest')
    assert.equal((html.match(/<img /g) || []).length, 1)
  ...
  const decorative = await render('Cinéma GALAXY', 'cinewest', { decorative: true })
...
for (const provider of providerKeys) {
  ...
  test(`${provider}: provider logo is unique and source name stays exact with or without its brand`, async () => {
    for (const name of ['Cinéma GALAXY', `Le ${providers[provider]} Centre`, `${providers[provider]} ${providers[provider]}`]) {
      ...
      assert.ok(html.includes(` ${name}</span>`), html)
      assert.doesNotMatch(html, /class="sr-only"/)
      if (name === 'Cinéma GALAXY') assert.ok(html.includes(`alt="${providers[provider]}"`), html)
  ...
  test(`${provider}: other brand and format tokens cannot select extra logos; decorative and classes work`, async () => {
    ...
    assert.ok(html.includes(` ${name}</span>`), html)
    const decorative = await render(name, provider, { decorative: true, logoClass: 'brightness-0 invert' })
    assert.match(decorative, /^<span aria-hidden="true">/)
    assert.match(decorative, /alt(?:="")? aria-hidden="true"/)
...
test('brand detection folds accents and case but never matches a partial name token', async () => {
  for (const provider of providerKeys) {
    for (const name of [`Le ${providers[provider].toUpperCase()} Centre`, `(${provider}) Centre`, `${providers[provider].normalize('NFD')} Centre`]) {
      const html = await render(name, provider)
      assert.match(html, /alt(?:="")? aria-hidden="true"/)
      assert.ok(html.includes(` ${name}</span>`), html)
    }
    for (const name of [`${provider}ville`, `é${provider}`, `${provider}_centre`, `${provider}2`]) {
      const html = await render(name, provider)
      assert.ok(html.includes(`alt="${providers[provider]}"`), html)
    }
  }
...
test('all public theater-name surfaces supply provider metadata, while timeline movies keep generic text', async () => {
  for (const path of ['pages/cinemas.vue', 'pages/cinema/[slug].vue', 'pages/ville/[slug]/cinemas.vue', 'pages/film/[slug].vue', 'components/ShowtimeResultLine.vue', 'components/ShowtimeResultBox.vue', 'components/TimelineMatrix.vue', 'components/CinemaTheaterMap.client.vue']) {
    const value = await source(path)
    assert.match(value, /<TheaterName[^>]*:provider=/, path)
    assert.doesNotMatch(value, /<BrandedText :text="(?:theater\.name|result\.theaterName|selected\.theater\.name)"/, path)
  ...
  assert.match(timeline, /<BrandedText\s+v-if="mode === 'theater'"\s+:text="item\.showtime\.movie\.title"/)
