...
for (const [path, count] of sources) {
  test(`${path} keeps mobile pagination actions paired below a full-width count`, async () => {
    ...
    const pagers = [...source.matchAll(/<nav\b[\s\S]*?<\/nav>/g)].filter(([nav]) => /[Pp]agination/.test(nav))
    assert.equal(pagers.length, count)
    for (const [pager] of pagers) {
      const navClass = pager.match(/\bclass="([^"]+)"/)?.[1]?.split(' ') ?? []
      for (const token of ['grid', 'grid-cols-2', 'sm:flex']) assert.ok(navClass.includes(token), token)
      assert.ok(!navClass.includes('flex-col'))
      assert.ok(!navClass.includes('flex-wrap'))
      const countClass = pager.match(/<span class="([^"]+)"[^>]*>Page /)?.[1]?.split(' ') ?? []
      for (const token of ['order-first', 'col-span-2', 'min-w-0', 'wrap-anywhere', 'sm:order-none']) assert.ok(countClass.includes(token), token)
    }
  })
}

test('public catalogs share pagination without a Films-only mobile override', async () => {
  for (const path of ['films/index.vue', 'films/prochainement.vue', 'ville/[slug]/cinemas.vue']) {
...
test('shared pager retains native links, unavailable spans and single navigation events', async () => {
  const source = await readFile(new URL('../app/components/MovieCatalogPagination.vue', import.meta.url), 'utf8')
  assert.equal([...source.matchAll(/<NuxtLink\b/g)].length, 2)
  assert.match(source, /<span v-if="!previousTo"[^>]+aria-disabled="true"/)
  assert.match(source, /<span v-if="!nextTo"[^>]+aria-disabled="true"/)
  assert.equal([...source.matchAll(/:aria-disabled="pending \|\| undefined"/g)].length, 2)
