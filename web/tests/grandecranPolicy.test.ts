...
import { parseShowtimeSelection, serializeShowtimeSelection, toSlotShowtimeResults, toTheaterShowtimeResults } from '../app/utils/showtimeResults.ts'
...
import { THEATER_PROVIDER_LABELS, buildTheaterFeatureCollection } from '../app/utils/theaterMap.ts'
...
const corpus: { valid: string[], invalid: string[] } = JSON.parse(await readFile(new URL('./fixtures/grandecran-booking-urls.json', import.meta.url), 'utf8'))

test('Grand Ecran booking accepts only canonical exact-host routes and 4096-byte bounds', () => {
  for (const value of corpus.valid) {
    assert.deepEqual(safeBookingUrl(value, 'grandecran'), { provider: 'grandecran', url: value, kind: 'booking' })
    assert.equal(safeBookingUrl(value)?.url, value)
  }
  for (const value of corpus.invalid) assert.equal(safeBookingUrl(value), null, JSON.stringify(value))
  for (const provider of ['ugc', 'kinepolis', 'pathe', 'cgr', 'megarama', 'cineville', 'mk2'] as const) assert.equal(safeBookingUrl(booking, provider), null)
...
test('Grand Ecran crawlability booking policy matches frontend malicious corpus', async () => {
  ...
  const policy = runInNewContext(`${source.slice(start, end)}; ({ reservationUrl })`, { URL })
  for (const value of [...corpus.valid, ...corpus.invalid]) {
    assert.equal(policy.reservationUrl({ booking_url: value, provider: 'grandecran', id: showingId }), safeBookingUrl(value, 'grandecran')?.url ?? null, JSON.stringify(value))
  }
})

test('Grand Ecran g tokens preserve both theater shapes and canonical SHA-256 encoding', () => {
  const keys = ['P9488', 'G028P', '00000'].map((theater) => `grandecran:grandecran-showing-${theater}-${hash}`)
  for (const [index, theater] of ['P9488', 'G028P', '00000'].entries()) {
    assert.equal(serializeShowtimeSelection([keys[index]!]), `g${theater}-${compactHash}`)
    assert.deepEqual(parseShowtimeSelection(`g${theater}-${compactHash}`), [keys[index]])
  }
  const all = [...keys, `cgr:cgr-showing-P9488-${hash}`, 'mk2:mk2-showing-0004-140350', 'ugc:ugc-showing-12']
  assert.deepEqual(parseShowtimeSelection(serializeShowtimeSelection([...all, ...keys])), all.toSorted())
  for (const theater of ['g028p', 'P948', 'P94888', 'G02_P', 'G02-P', 'G02éP']) {
    assert.equal(serializeShowtimeSelection([`grandecran:grandecran-showing-${theater}-${hash}`]), undefined)
    assert.deepEqual(parseShowtimeSelection(`g${theater}-${compactHash}`), [])
  }
  for (const token of [`gG028P-${compactHash}=`, `gG028P-${compactHash.slice(0, -1)}x`, `gG028P-${compactHash}\n`, `gG028P-${compactHash} `, `gG028P-${compactHash.slice(1)}`]) assert.deepEqual(parseShowtimeSelection(token), [], token)
  for (const value of [hash.toUpperCase(), `${hash}\n`, `${hash} `, hash.slice(1), `${hash}0`]) assert.equal(serializeShowtimeSelection([`grandecran:grandecran-showing-G028P-${value}`]), undefined)
})
...
test('Grand Ecran selected-showing URLs reload with cinema scope, future dates and filters', () => {
  ...
  const selectedShowtimeKeys = theaterIds.map((theater) => `grandecran:${theater.replace('grandecran-', 'grandecran-showing-')}-${hash}`)
  const target = buildCompleteSearchShareTarget({
    theaterIds, date: '2027-07-01', startAfter: '00:00', finishBefore: '23:30', language: 'VOSTFR', format: 'DOLBY', includeAds: false, bufferAds: 15,
  ...
  assert.equal(query.get('selected'), `gG028P-${compactHash},gP9488-${compactHash}`)
  ...
  for (const runtime of [0, 93, 118]) {
    ...
    if (runtime > 0) {
      for (const ads of [0, 15, 30]) {
        ...
        assert.deepEqual(toTheaterShowtimeResults(response)[0]!.end, { time, estimated: true, adsMinutes: ads })
        assert.deepEqual(toSlotShowtimeResults([slot])[0]!.end, { time, estimated: true, adsMinutes: ads })
        assert.equal(response.showtimes[0]!.end_time, start)
      }
    }
    assert.equal(THEATER_PROVIDER_LABELS.grandecran, 'Grand Ecran')
    assert.deepEqual(buildTheaterFeatureCollection([response.theater], new Set([response.theater.id])).features[0]?.properties, { id: 'grandecran-G028P', provider: 'grandecran', favorite: true })
  }
  assert.deepEqual(availableLanguageOptions(['VOSTFR', 'VF']).map((option) => option.value), ['ALL', 'VOSTFR', 'VF'])
  assert.equal(safePosterUrl(null), null)
  assert.equal(safePosterUrl('https://fr.web.img6.acsta.net/pictures/event.jpg'), 'https://fr.web.img6.acsta.net/pictures/event.jpg')
})

test('Grand Ecran brand-only names keep city context without duplicating location text', () => {
  for (const name of ['Grand Ecran', 'Grand Écran', 'grand écran', 'GRAND ECRAN']) assert.equal(theaterDisplayName({ name, city: 'Vichy' }), `${name} Vichy`)
  assert.equal(theaterDisplayName({ name: 'Grand Écran Vichy', city: 'Vichy' }), 'Grand Écran Vichy')
  assert.match(cinemaDescription({ provider: 'grandecran', name: 'Grand Ecran Vichy', city: 'Vichy', availableDateCount: 2 }), /est un cinéma Grand Ecran/)
})
