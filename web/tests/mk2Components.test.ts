...
test('MK2 imports supplied SVG as an image for both sizes with accessible and decorative semantics', async () => {
  ...
  assert.equal(
    ...
    'd9ffc0b3862b6d4a2015aef14cab76b8d4829f50c939e34ef75b15e59b5f0b62',
  )
...
test('MK2 appears in individual, all-run and schedule provider surfaces without adding controls', async () => {
  ...
  assert.match(
    ...
    /mk2: selectLatestProviderRun\(\s*'mk2'/,
  )
  assert.match(
    await source('components/CinemaTheaterMap.client.vue'),
    /'mk2',\s+THEATER_PROVIDER_COLORS\.mk2/,
  )
...
test('every shared showtime surface suppresses silent language and binds booking to showing identity', async () => {
  for (const path of [
    ...
    assert.equal(
      (value.match(/<BookingLink\b/g) ?? []).length,
      (value.match(/:showtime-id="result\.showtimeId"/g) ?? []).length,
    )
  }
