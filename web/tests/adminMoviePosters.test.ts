...
function poster(
  ...
  return {
    url: `https://image.tmdb.org/t/p/w500/${name}.jpg`,
    width: 1000,
    height: 1500,
    language,
  }
...
test('loads every language and every result without a display limit, rejecting unsafe image URLs and duplicates', async () => {
  const all = Array.from({ length: 120 }, (_, index) =>
    poster(String(index), index % 3 === 0 ? null : index % 2 ? 'ja' : 'fr'),
  )
  ...
  response.resolve({
    posters: [
      ...all,
      all[0]!,
      { ...poster('bad'), url: 'https://evil.example/poster.jpg' },
      { ...poster('bad'), url: 'https://image.tmdb.org/t/p/w500/../bad.jpg' },
      { ...poster('bad'), url: 'https://ugc.fr/poster.jpg' },
    ],
  })
  await loading
...
test('close/unmount reset aborts requests, clears images, and ignores late errors across reopening', async () => {
  ...
  const picker = useAdminMoviePosters((_id, requestSignal) => {
    ...
    return ++calls === 1
      ? first.promise
      : Promise.resolve({ posters: [poster('reopened')] })
  })
  const loading = picker.load('1')
  picker.reset()
...
test('poster API GET encodes movie IDs, includes credentials and signal, and leaves retry to the UI', async () => {
  ...
  assert.deepEqual(
    await useMesSeancesApi().adminMoviePosters(
      '9007199254740993/a',
      controller.signal,
    ),
    response,
...
test('dialog wires native focus, dismissal, cancellation and draft-only selection through safe lazy previews', async () => {
  const [picker, grid, api, image] = await Promise.all([
    readFile(
      new URL(
        '../app/components/admin/AdminMoviePosterPicker.vue',
        import.meta.url,
      ),
      'utf8',
    ),
  ...
  assert.match(picker, /watch\(\s*\(\) => props.movieId,\s*\(\) => closeModal/)
  ...
  assert.match(picker, /<PosterImage\s+:src="poster.url"/)
  assert.match(image, /loading="lazy"/)
  assert.match(
    grid,
    /<AdminMoviePosterPicker[^>]+:key="selectedDetailsItem.id"[^>]+@select="selectPoster"/,
  )
  assert.match(
    grid,
    /function selectPoster\(url: string\) \{[^}]+updateDraft\(item, 'poster_url', url\)/,
  )
  assert.match(
    api,
    /adminMoviePosters\(id: string, signal\?: AbortSignal\) \{\s+return withAdminRedirect/,
  )
})
