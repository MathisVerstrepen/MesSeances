...
async function component(name: string): Promise<Component> {
  ...
  const localRequire = (id: string) => id.startsWith('~/assets/') ? { __esModule: true, default: id } : id === '~/utils/formats' ? { formatBrand, formatLabel } : require(id)
...
async function render(root: Component, props: Record<string, string | boolean>) {
...
test('Infinity Vision uses unchanged supplied PNGs for inline and display horizontal logos', async () => {
  const variants = [
    { variant: 'inline', size: 'small', hash: 'a99ff06bf24296d208edb7f3d680e805a1aecdadd3c1955eb5d48ea39638b0d3', width: 300, height: 61 },
    { variant: 'display', size: 'large', hash: '02a74df0d5182f71a280ff5d3bd05e665842c707e1cd234d471bc0cd5ae06343', width: 1129, height: 228 }
  ]
  for (const { variant, size, hash, width, height } of variants) {
    const basename = `infinity_vision_logo_${size}.png`
    const bytes = await readFile(new URL(`../app/assets/imgs/${basename}`, import.meta.url))
    ...
    if (variant === 'inline') assert.ok(html.includes('h-[0.68em] w-auto align-[-0.06em]'))
...
test('Infinity Vision decorative logos retain selected-state inversion without duplicate accessible names', async () => {
  const html = await render(BrandLogo, { brand: 'INFINITY_VISION', decorative: true, class: 'brightness-0 invert' })
...
test('Infinity Vision showtime badge has exactly one accessible label and a decorative image', async () => {
  ...
  assert.match(html, /alt(?:="")? aria-hidden="true"/)
  const decorative = await render(ShowtimeFormat, { format: 'INFINITY_VISION', decorative: true, logoClass: 'brightness-0 invert' })
