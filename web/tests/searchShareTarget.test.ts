...
for (const language of ['ORIGINAL', 'VOF'] as const) {
  test(`${language} survives complete and short-link sharing without expanding into concrete languages`, () => {
    ...
    const target = buildCompleteSearchShareTarget({
      theaterIds, date: '2027-06-27', startAfter: '18:00', finishBefore: '23:30',
      language, format: 'ALL', includeAds: false, bufferAds: 15,
      grouping: 'movie', layout: 'lines', selectedShowtimeKeys: [], selectedOnly: false
    })
    const shared = withSharedTheaterSelection(target, theaterIds)!
    assert.deepEqual(new URL(target, 'https://messeances.fr').searchParams.getAll('language'), [language])
    assert.equal(isValidShortLinkTarget(shared), true)
    assert.deepEqual(new URL(shared, 'https://messeances.fr').searchParams.getAll('language'), [language])
  })
}
