...
const cities: CitiesResponse = {
  ...
  items: [
    ...
    },
    {
      ...
      slug: 'lyon',
      theaters: [
        { provider: 'cgr', id: 'cgr-1', slug: 'cgr-lyon', name: 'CGR Lyon' },
      ],
    },
  ],
}
...
test('groups canonical film URLs and applies source-backed lastmod formulas', () => {
  ...
  const enrichedCurrent = movie('film-10', {
    showtime_count: 1,
    updated_at: '2026-08-30T11:00:00Z',
  })
  const richEnded = movie('film-30', {
    showtime_count: 0,
    updated_at: '2026-08-29T07:00:00Z',
    poster_url: null,
    imdb_id: 'tt0030',
  })
  const thinEnded = movie('film-40', {
    showtime_count: undefined,
    overview: ' ',
  })
  const homepageMovie = movie('film-10', {
    updated_at: '2026-08-30T12:00:00Z',
    showtime_count: 1,
  })
  const filmsMovie = movie('film-20', {
  ...
  assert(
    !entries.some(
      (entry) =>
        entry.path.includes('film-40') ||
        ['/planning', '/recherche'].includes(entry.path),
    ),
  )
})

test('advances films lastmod when an off-page current movie changes global catalog metadata', () => {
  const currentMovies = Array.from({ length: 25 }, (_, index) =>
    movie(`film-${index + 1}`, {
      showtime_count: 1,
      updated_at:
        index === 24 ? '2026-08-30T13:00:00Z' : '2026-08-30T09:00:00Z',
  ...
  assert.equal(
    filmsCatalog.items.some((item) => item.slug === 'film-25'),
    false,
  )
  assert.equal(
    filmsCatalog.available_genres.includes('Genre hors première page'),
    true,
  )
  assert.equal(
    entries.find((entry) => entry.path === '/films')?.lastmod,
...
test('renders deterministic escaped sitemap index and URL set with trailing newlines', () => {
  ...
  const urlset = renderSitemap('https://messeances.fr', [
    { path: '/film/film-42', lastmod: GENERATED_AT },
  ])
  assert(
    urlset.includes(
...
test('rejects malformed timestamps, inconsistent snapshots, and duplicate URLs', () => {
  assert.throws(
    () => latestTimestamp(GENERATED_AT, 'not-a-timestamp'),
    /Invalid sitemap timestamp/,
  )
  assert.throws(
    () =>
      validateCatalogPage(
        { ...catalog([], 6), generated_at: 'invalid' },
        { page: 1, pageSize: 6 },
      ),
    /Inconsistent movie catalog snapshot/,
  )
  assert.throws(
    () =>
      buildFilmSitemapEntries(
...
test('validates later paginated catalog item counts against remaining total', () => {
  ...
  assert.doesNotThrow(() =>
    validateCatalogPage(page, { page: 2, pageSize: 100, total: 101 }),
  )
})
