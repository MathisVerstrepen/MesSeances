...
test('Grand Ecran reuses exact supplied WebP for accessible inline and display logos with contrast', async () => {
  const bytes = await readFile(
    new URL('../app/assets/imgs/grand_ecran_logo_small.webp', import.meta.url),
  )
  assert.equal(
    createHash('sha256').update(bytes).digest('hex'),
    '5b483a298244f8acae3be4ffe54b448529a32d96e475aeb1ddb7fa3871a653cf',
  )
  ...
  assert.match(
    credits,
    /brand: 'Grand Ecran',\s*name: 'Grand Ecran',\s*url: 'https:\/\/www\.grandecran\.fr\/'/,
  )
  assert.equal(
    (credits.match(/credit\.brand\.replaceAll\(' ', '-'\)/g) ?? []).length,
    2,
  )
})
...
test('Grand Ecran branding matches accents and case without consuming cinema location text', async () => {
  ...
  const pattern = /\.split\(\s*\/(.+)\/(giu),?\s*\)/.exec(component)!
  ...
  for (const brand of [
    ...
    assert.deepEqual(`Cinéma ${brand} Vichy`.split(brands).filter(Boolean), [
      'Cinéma ',
      brand,
      ' Vichy',
    ])
...
test('Grand Ecran follows existing MK2 and Cinewest order in manual, all and scheduled sync and admin labels', async () => {
  ...
  assert.match(
    ...
    /grandecran: selectLatestProviderRun\(\s*'grandecran'/,
  )
  assert.match(
    await source('components/CinemaTheaterMap.client.vue'),
    /'grandecran',\s+THEATER_PROVIDER_COLORS\.grandecran/,
  )
  assert.match(
