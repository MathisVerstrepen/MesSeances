...
} from '../app/utils/shortLinkTarget.ts'
import { mergeOwnedQuery } from '../app/utils/routeQuery.ts'
import {
  canonicalizeTheaterSelection,
  parseSharedTheaterSelection,
  theaterSelectionsEqual,
  withSharedTheaterSelection,
} from '../app/utils/sharedTheaterSelection.ts'
...
test('accepts every supported route and owned query key', () => {
  ...
  for (const target of targets)
    assert.equal(isValidShortLinkTarget(target), true, target)
})
...
test('accepts selected-only search links without allowing the option on other routes or duplicate keys', () => {
  assert.equal(
    isValidShortLinkTarget('/recherche?selected=u12&selected_only=1'),
    true,
  )
  assert.equal(isValidShortLinkTarget('/films?selected_only=1'), false)
  assert.equal(
    isValidShortLinkTarget(
      '/recherche?selected=u12&selected_only=1&selected_only=0',
    ),
    false,
  )
})
...
test('accepts cinema films view and its display query', () => {
  ...
  assert.equal(
    isValidShortLinkTarget(
      '/cinema/ugc-lille?date=2026-08-22&grouping=movie&layout=boxes&view=films&shared_theaters=ugc-25%2Cugc-26',
    ),
    true,
  )
})
...
test('rejects unsafe, malformed, and unsupported targets', () => {
  const targets = [
    ...
    '/film/slug!',
    '/admin',
    '/ville/lille/cinemas/extra',
    '/?q=x',
    '/films?unknown=x',
    '/credits?q=x',
    '/cinema/ugc-lille?unknown=x',
    '/cinemas',
    '/cinemas?q=Lille',
    '/cinemas?%71=Lille+centre',
    '/cinemas?shared_theaters=ugc-25',
    '/cinemas?q=Lille&shared_theaters=ugc-25',
    '/films?q=x&q=y',
    '/films?q=x&%71=y',
    '/films?',
    '/films?q=x&',
    ...
    '/films?%0A=x',
    '/films?q=x\r\nInjected:x',
    '/films?date=today&date=tomorrow',
    '/films?duration=short&duration=long',
    '/films?q=x#fragment',
    `/films?q=${'x'.repeat(2040)}`,
    `/${'x'.repeat(2048)}`,
    `/films?q=${String.fromCharCode(0xd800)}`,
  ]
  for (const target of targets)
    assert.equal(isValidShortLinkTarget(target), false, target)
})

test('validates shortlink codes exactly', () => {
  for (const code of ['AAAAAAAAAAAAAAAAAAAAAA', 'Abcdefghijklmnopqr_1-2'])
    assert.equal(isValidShortLinkCode(code), true, code)
  for (const code of [
    '',
    'short',
    'AAAAAAAAAAAAAAAAAAAAA!',
    'éAAAAAAAAAAAAAAAAAAAAA',
    'AAAAAAAAAAAAAAAAAAAAAAA',
  ])
    assert.equal(isValidShortLinkCode(code), false, code)
})
...
test('accepts shared theater selection on seven remaining routes', () => {
  for (const path of [
    ...
    assert.equal(
      isValidShortLinkTarget(
        `${path}${separator}shared_theaters=ugc-25%2Ckinepolis_42`,
      ),
      true,
      path,
    )
  ...
  assert.equal(
    isValidShortLinkTarget(`/films?shared_theaters=${'a'.repeat(128)}`),
    true,
  )
...
test('rejects malformed shared theater selections', () => {
  for (const value of [
    ...
    assert.equal(
      isValidShortLinkTarget(
        `/films?shared_theaters=${encodeURIComponent(value)}`,
      ),
      false,
      value,
    )
  ...
  assert.equal(
    isValidShortLinkTarget(
      '/films?shared_theaters=ugc-25&shared_theaters=ugc-26',
    ),
    false,
  )
...
test('parses, canonicalizes, and compares theater selections', () => {
  assert.deepEqual(parseSharedTheaterSelection('known-2,stale,known-1'), [
    'known-2',
    'stale',
    'known-1',
  ])
  ...
  assert.deepEqual(
    canonicalizeTheaterSelection(['known-2', 'stale', 'known-1'], catalog),
    ['known-1', 'known-2'],
  )
  assert.deepEqual(canonicalizeTheaterSelection(['stale'], catalog), [])
  assert.equal(
    theaterSelectionsEqual(['known-1', 'known-2'], ['known-2', 'known-1']),
    true,
  )
...
test('upserts one canonical shared field without changing other raw fields', () => {
  assert.equal(
    withSharedTheaterSelection('/films?sort=title&q=Am%C3%A9lie+Poulain', [
      'known-1',
      'known-2',
    ]),
  ...
  assert.equal(
    withSharedTheaterSelection('/films?shared%5Ftheaters=old&q=a%26b%3Dc', [
      'known-2',
    ]),
  ...
  assert.equal(
    withSharedTheaterSelection(
      '/films?shared_theaters=old&q=x&shared_theaters=older',
      ['known-1'],
    ),
