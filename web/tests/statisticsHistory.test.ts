...
import { createStatisticsRequest, statisticsBars, statisticsOptionsWithSelection } from '../app/utils/statistics.ts'
...
test('VOF survives every period, shared reload, API transport and router back/forward', async () => {
  ...
  await router.push({ path: '/statistiques', query: { period: 'all', language: 'VOF', city: ['paris'], film: 'missing-film', campaign: ['footer', 'test'] } })
  ...
  try {
    ...
    for (const { value: period } of statisticsPeriods) {
      draft = period === 'custom' ? statisticsCustomDraft(draft, { from: '2020-01-01', through: '2030-01-01' }, today) : { ...draft, period }
      const parsed = statisticsPageDraftQuery(draft, today)
      assert.equal(parsed.error, '')
      assert.equal(parsed.query.language, 'VOF')
      await router.push({ query: statisticsPageRoute(router.currentRoute.value.query, period, parsed.query) })
      ...
      assert.deepEqual(statisticsPageDraftQuery(statisticsPageDraft(reloaded.query, today), today), parsed)
      ...
      assert.equal(request.searchParams.get('date_to'), parsed.query.date_to ?? null)
      for (const key of ['campaign', 'period', 'mode']) assert.equal(request.searchParams.has(key), false)
      // An empty intersection or absent inventory option must not clear the applied VOF draft.
      assert.deepEqual(statisticsBars([], 0), [])
      assert.deepEqual(statisticsOptionsWithSelection([], [draft.language]), [{ value: 'VOF', label: 'VOF (indisponible)' }])
    ...
    for (const [delta, indices] of [[-1, [2, 1, 0]], [1, [1, 2, 3]]] as const) {
      for (const index of indices) {
        ...
        assert.deepEqual(statisticsPageDraftQuery(draft, today), parseStatisticsPageQuery(router.currentRoute.value.query, today))
      }
    }
    draft.language = 'VF'
    ...
    assert.deepEqual(router.currentRoute.value.query, { campaign: ['footer', 'test'] })
  } finally {
    stop()
...
test('every statistics period accepts padded VOF but rejects malformed and combined version selectors', () => {
  for (const { value: period } of statisticsPeriods) {
    ...
    assert.equal(parseStatisticsPageQuery({ ...route, language: ' VOF ' }).query.language, 'VOF')
    for (const language of ['vof', 'Vof', 'ORIGINAL', 'ALL', 'VOF,VOSTFR', 'VOF+VOSTFR', '', null, ['VOF'], ['VOF', 'VOF'], ['VOF', 'VF']]) {
      assert.ok(parseStatisticsPageQuery({ ...route, language }).error, `${period}: ${JSON.stringify(language)}`)
    }
  }
})
