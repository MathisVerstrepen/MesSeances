...
} from '../app/utils/adminUpcomingMovies.ts'

const state = { filter: 'needs_review', q: '', page: 1 } as const
function movie(
  overrides: Partial<AdminUpcomingMovie> = {},
): AdminUpcomingMovie {
  return {
    tmdb_id: 42,
    public_movie_id: '7',
    ...
    title: 'Film test',
    poster_url: null,
    french_release_date: '2026-10-07',
    active: true,
    in_window: true,
    publicly_visible: true,
    assessment_status: 'assessed',
    assessed_at: '2026-09-13T12:00:00Z',
    french_releases: [
      {
        type: 2,
        date: '2026-10-07',
        note: '<img src=x onerror=alert(1)> Séance unique',
      },
    ],
    reason_codes: ['limited_only', 'single_screening_note'],
    decision: 'unreviewed',
    revision: 1,
    ...overrides,
  }
...
test('route defaults, five filters, canonical search and bounded pagination match the frozen contract', () => {
  ...
  assert.deepEqual(
    upcomingReviewFilters.map((filter) => filter.value),
    ['needs_review', 'pending_assessment', 'approved', 'excluded', 'all'],
  )
  for (const { value } of upcomingReviewFilters) {
    const parsed = parseUpcomingReviewRoute({
      filter: value,
      q: '  42  ',
      page: '2',
    })
    assert.deepEqual(upcomingReviewApiQuery(parsed), {
      filter: value,
      search: '42',
      limit: 50,
      offset: 50,
    ...
    assert.deepEqual(
      parseUpcomingReviewRoute(upcomingReviewRouteQuery(parsed)),
      parsed,
    )
  }
  assert.deepEqual(
  ...
  for (const page of ['1e2', '0', '9007199254740992', '42949674'])
    assert.equal(parseUpcomingReviewRoute({ page }).page, 1)
  assert.equal(
    upcomingReviewApiQuery(parseUpcomingReviewRoute({ page: '42949673' }))
      .offset,
    2147483600,
  )
  assert.equal(
    upcomingReviewApiQuery(parseUpcomingReviewRoute({ q: '0042' })).search,
    '0042',
    'never reinterpret noncanonical ID as canonical',
  )
  assert.equal(
    upcomingReviewApiQuery(parseUpcomingReviewRoute({ q: '%_\\' })).search,
    '%_\\',
    'literal search is server-owned',
  )
  assert.equal(
    Array.from(normalizeUpcomingReviewSearch(' 😀'.repeat(1200))).length,
    1024,
  )
  assert.deepEqual(
    upcomingReviewRouteQuery(state, {
      filter: 'all',
      q: 'x',
      page: '2',
      other: 'kept',
...
test('labels preserve evidence meaning and visibility never infers exclusion from reasons', () => {
  ...
  assert.equal(
    upcomingReviewVisibility(
      movie({ assessment_status: 'pending', reason_codes: [] }),
    ),
    'Visible',
  )
  assert.equal(
...
test('GET and PATCH use exact routes, credentials, abortable reads and no retry or extra fields', async () => {
  ...
  await api.adminUpcomingMovies(
    { filter: 'all', search: undefined, limit: 50, offset: 0 },
    signal,
  )
  await api.adminSetUpcomingDecision(42, {
    decision: 'excluded',
    expected_revision: 7,
  })
  assert.deepEqual(calls, [
    {
      url: 'http://localhost:8080/api/v1/admin/tmdb-upcoming-movies',
      options: {
        credentials: 'include',
        query: { filter: 'all', limit: 50, offset: 0 },
        signal,
        retry: false,
      },
    },
    {
      url: 'http://localhost:8080/api/v1/admin/tmdb-upcoming-movies/42/decision',
      options: {
        method: 'PATCH',
        credentials: 'include',
        body: { decision: 'excluded', expected_revision: 7 },
        retry: false,
      },
    },
  ])
...
test('stale GET success or error cannot replace newer rows and disposal aborts reads', async () => {
  for (const reject of [false, true]) {
    ...
    const review = useAdminUpcomingMovies(
      {
        adminUpcomingMovies: (_query, signal) => {
          if (++calls === 1) {
            oldSignal = signal
            return old.promise
          }
          return Promise.resolve(response([movie({ title: 'Latest' })]))
        },
        adminSetUpcomingDecision: async () => {
          assert.fail('no edit expected')
        },
      },
      async () => {
        assert.fail('no clamp expected')
      },
    )
  ...
  const review = useAdminUpcomingMovies(
    {
      adminUpcomingMovies: (_q, s) => {
        signal = s
        return pending.promise
      },
      adminSetUpcomingDecision: async () => movie(),
    },
    async () => {},
  )
...
test('canonical poster values survive list and decision refresh without extra movie reads', async () => {
  for (const poster_url of [
    null,
    'https://image.tmdb.org/t/p/w500/poster.jpg',
    'https://messeances.fr/uploads/manual-poster.webp',
  ]) {
    let reads = 0
    const review = useAdminUpcomingMovies(
      {
        adminUpcomingMovies: async () => {
          reads += 1
          return response([
            movie({
              poster_url,
              decision: reads > 1 ? 'approved' : 'unreviewed',
            }),
          ])
        },
        adminSetUpcomingDecision: async () =>
          movie({ poster_url, decision: 'approved', revision: 2 }),
      },
      async () => {},
    )
...
test('all three decisions send displayed revision, suppress duplicates/no-ops and reload totals', async () => {
  for (const decision of ['approved', 'excluded', 'unreviewed'] as const) {
    ...
    const initial = movie({
      revision: 8,
      decision: decision === 'unreviewed' ? 'excluded' : 'unreviewed',
    })
    const review = useAdminUpcomingMovies(
      {
        adminUpcomingMovies: async () => {
          reads += 1
          return response(reads === 1 ? [initial] : [])
        },
        adminSetUpcomingDecision: (id, input) => {
          edits += 1
          assert.equal(id, 42)
          assert.deepEqual(input, { decision, expected_revision: 8 })
          return edit.promise
        },
      },
      async () => {},
    ...
    const review = useAdminUpcomingMovies(
      {
        adminUpcomingMovies: async () => {
          reads += 1
          return response([movie({ revision: reads })])
        },
        adminSetUpcomingDecision: async () => {
          edits += 1
...
test('list and uncertain mutation errors preserve rows/search and require explicit read recovery', async () => {
  for (const cause of [
    new Error('private'),
    failure(500, 'upcoming_review_update_failed'),
  ]) {
    let reads = 0
    let edits = 0
    const review = useAdminUpcomingMovies(
      {
        adminUpcomingMovies: async (query) => {
          reads += 1
          assert.equal(query.search, 'Film')
          if (reads === 2) throw failure(500, 'upcoming_review_list_failed')
          return response()
        },
        adminSetUpcomingDecision: async () => {
          edits += 1
  ...
  const review = useAdminUpcomingMovies(
    {
      adminUpcomingMovies: async () => {
        reads += 1
        return response(reads === 1 ? [movie()] : [], reads === 1 ? 51 : 50)
      },
  ...
  const review = useAdminUpcomingMovies(
    {
      ...
      adminSetUpcomingDecision: () => edit.promise,
    },
    async () => {},
  )
  await review.load()
...
test('French error mappings never display diagnostic details', () => {
  for (const code of [
    'invalid_upcoming_review_query',
    'invalid_upcoming_review_id',
    'invalid_upcoming_review_update',
    'upcoming_review_not_found',
    'upcoming_review_conflict',
    'upcoming_review_list_failed',
    'upcoming_review_update_failed',
  ]) {
...
test('page is client-only authenticated, accessible, escaped and preserves review history alongside relocated sync', async () => {
  ...
  assert.match(
    page,
    /onBeforeUnmount\(\(\) => \{[^}]*cancelSearch\(\);?\s+dispose\(\)/,
  )
  assert.match(page, /router\.push\(/)
  assert.match(page, /router\.replace\(/)
  assert.match(page, /watch\(\s*\(\) => route\.query/)
  assert.match(page, /UPCOMING_REVIEW_SEARCH_DELAY/)
  assert.match(page, /<label\s+for="review-search"[^>]*>\s*Titre ou ID TMDB/)
  assert.match(page, /<details\s+v-if="movie.assessment_status === 'assessed'"/)
  assert.match(page, /<summary[^>]+min-h-11/)
  assert.match(page, /wrap-anywhere text-muted"\s*>\s*\{\{ release.note \}\}/)
  assert.match(page, /movie.assessment_status === 'pending'/)
  assert.match(page, /Évaluation après la prochaine synchronisation\s+réussie/)
  assert.match(
    page,
  ...
  assert.doesNotMatch(
    page,
    /v-html|localStorage|sessionStorage|adminStartUpcomingSync|useAsyncData/,
  )
  for (const decision of [
    'approved',
    'excluded',
    'unreviewed',
  ] satisfies UpcomingReviewDecision[])
    assert.ok(page.includes(`decide(movie, '${decision}')`))
  ...
  assert.doesNotMatch(
    dashboard,
    /useAdminUpcomingSync|@click="startUpcomingSync"/,
  )
})
