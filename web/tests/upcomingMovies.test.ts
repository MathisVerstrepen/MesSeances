...
test('groups chronologically without inventing empty weeks or changing within-week backend order or actual dates', () => {
  const films = [
    movie('2027-01-13', 'film-2'),
    movie('2027-01-05', 'film-3'),
    movie('2026-12-30', 'film-1'),
    movie('2026-12-30', 'film-4'),
  ]
...
test('weeks cross month, year, leap day and DST boundaries without timezone drift', () => {
  const cases = [
    ...
    ['2026-10-06', '2026-09-30'],
    ['2027-01-01', '2026-12-30'],
    ['2027-01-05', '2026-12-30'],
    ['2027-01-06', '2027-01-06'],
    ['2028-02-29', '2028-02-23'],
    ['2028-03-01', '2028-03-01'],
    ['2026-10-25', '2026-10-21'],
    ['2026-10-27', '2026-10-21'],
    ['2026-10-28', '2026-10-28'],
    ['2027-03-28', '2027-03-24'],
    ['2027-03-30', '2027-03-24'],
  ...
  try {
    for (const timezone of [
      ...
      for (const [date, expected] of cases)
        assert.equal(releaseWeekStart(date), expected, `${timezone}: ${date}`)
...
test('keeps all films in a complete week spanning two months', () => {
  ...
  assert.deepEqual(groupUpcomingMovies([september, october]), [
    { weekStart: '2026-09-30', movies: [september, october] },
  ])
})

test('renders every supplied film in four nonempty weeks without a 24 or 100 item cap', () => {
  const bigWeek = Array.from({ length: 126 }, (_, index) =>
    movie('2026-09-30', `film-${index + 1}`),
  )
  const films = [
    ...bigWeek,
    movie('2026-10-15', 'film-127'),
    movie('2026-12-31', 'film-128'),
    movie('2027-02-02', 'film-129'),
  ]
  const groups = groupUpcomingMovies(films)
  assert.deepEqual(
    groups.map((group) => group.weekStart),
    ['2026-09-30', '2026-10-14', '2026-12-30', '2027-01-27'],
  )
...
test('rejects missing or invalid verified dates rather than falling back to general release dates', () => {
  for (const date of [
    ...
    assert.throws(
      () => releaseWeekStart(invalid),
      /Invalid French release date/,
    )
    assert.throws(
      () => groupUpcomingMovies([movie(invalid)]),
      /Invalid French release date/,
    )
    assert.throws(
      () => formatReleaseWeek(invalid),
      /Invalid French release date/,
    )
  }
})
...
test('API composable sends only frozen upcoming query and disables retry', async () => {
  ...
  Object.assign(globalThis, {
    ...
    $fetch: (url: string, options: UpcomingFetchOptions) => {
  ...
  assert.deepEqual(calls, [
    {
      url: 'http://localhost:8080/api/v1/movies/upcoming',
...
test('catalog-only SSR preserves upcoming and withdrawn states with no invented screenings', async () => {
  for (const status of ['upcoming', 'unavailable'] as const) {
    ...
    const result = await loadInitialFilmSchedule({
      ...
      today: '2026-09-13',
      fetchScoped: async () => response,
      fetchNationwide: async () => response,
    })
    assert.equal(result.scoped.release_status, status)
    assert.equal(result.selectedDate, '2026-09-13')
    const json = JSON.stringify(
      buildFilmJsonLd(response, {
        movieUrl: 'https://messeances.fr/film/film-1',
...
test('SSR page and detail use exact states, shared cards and no catalog-only preference blocker', async () => {
  ...
  const tabs = await readFile(
    new URL('../app/components/FilmCatalogTabs.vue', import.meta.url),
    'utf8',
  )
  ...
  assert.match(
    page,
    /if \(state\.page > lastPage\)\s+response = await api\.upcomingMovies/,
  )
  ...
  assert.match(
    page,
    /Sortie le\s+<time\s+:datetime="movie\.french_release_date"\s*>\s*\{\{\s*formatFrenchReleaseDate\(movie\.french_release_date\)\s*\}\}/,
  )
