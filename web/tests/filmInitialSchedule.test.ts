...
} from '../app/utils/filmInitialSchedule.ts'
...
test('configured SSR uses one bundle request for a date already available', async () => {
  ...
  const result = await loadInitialFilmSchedule({
    ...
    fetchScoped: async () => {
      throw new Error('public scoped request used')
    },
    fetchNationwide: async () => {
      throw new Error('public nationwide request used')
    },
...
test('configured SSR makes exactly one bundle request per distinct fallback date', async () => {
  ...
  const finalNationwide = schedule(
    RESOLVED_DATE,
    [RESOLVED_DATE],
    'Final France',
  )
  ...
  assert.deepEqual(result, {
    scoped: finalScoped,
    nationwide: finalNationwide,
    selectedDate: RESOLVED_DATE,
  })
})
...
test('blank-secret fallback preserves public Paris and nationwide request behavior', async () => {
  ...
  const result = await loadInitialFilmSchedule({
    ...
    fetchNationwide: async (date) => {
      ...
      return date === REQUESTED_DATE
        ? schedule(date, [RESOLVED_DATE], 'Discarded France')
        : finalNationwide
    },
  })
...
test('blank-secret nationwide failure preserves resolved-date upstream error behavior', async () => {
  const upstreamCause = Object.assign(new Error('upstream unavailable'), {
    status: 502,
  })
    ...
    loadInitialFilmSchedule({
      ...
      fetchNationwide: async (date) => {
        if (date === REQUESTED_DATE)
          return schedule(date, [RESOLVED_DATE], 'Discarded France')
        throw upstreamCause
      },
    }),
    (error) =>
      error instanceof NationwideInitialScheduleError &&
