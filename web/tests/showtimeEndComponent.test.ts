...
test('estimated time has a unique accessible description, actual-value formula and local hover/focus/Escape state', async () => {
  ...
  assert.match(value, /Fin estimée : début annoncé à \$\{formatParisTime\(props\.advertisedStart\)\} \+ \$\{props\.end\.adsMinutes\} min de publicités \+ \$\{props\.runtimeMinutes\} min de film\./)
  assert.match(value, /@mouseenter="openTooltip\('hover'\)"/)
  assert.match(value, /@mouseleave="leaveTooltip"/)
  assert.match(value, /setTimeout\(\(\) => \{ hovered\.value = false \}, 100\)/)
...
test('estimated booking-card triggers are siblings, never nested inside booking links', async () => {
  for (const path of ['pages/film/[slug].vue', 'components/ShowtimeResultBox.vue']) {
    const value = await source(path)
    const links = value.match(/<BookingLink\b[\s\S]*?<\/BookingLink>/g) ?? []
    for (const link of links) {
      if (link.includes('<ShowtimeEndTime')) assert.match(link, /v-if="(?:showtime|result)\.end && !(?:showtime|result)\.end\.estimated"/)
      assert.doesNotMatch(link, /tabindex="0"/)
    }
    assert.match(value, /<\/BookingLink>\s*<span v-if="(?:showtime|result)\.end\?\.estimated"[\s\S]*?<ShowtimeEndTime/)
  }
})

test('frontend search default agrees with schedule default', async () => {
  const frontend = await source('pages/recherche.vue')
  const backend = await readFile(new URL('../../api/internal/schedule/model.go', import.meta.url), 'utf8')
