...
test('Cinéville uses both exact supplied assets, no-inline imports, accessible names and credits', async () => {
  for (const [size, hash] of [
    [
      'small',
      '81ffddedbe8c9e75a759b57842b6bbcf5fe9a64354a6007f93d758e376dcb1f8',
    ],
    [
      'large',
      '55900e7de3f80168d12058eecebcca8e5f6c753ed86e20ac054ded291d1fcd90',
    ],
  ]) {
    const bytes = await readFile(
      new URL(
        `../app/assets/imgs/cineville_logo_${size}.webp`,
        import.meta.url,
      ),
    )
    assert.equal(createHash('sha256').update(bytes).digest('hex'), hash)
    assert.ok(
      (await source('components/BrandLogo.vue')).includes(
        `cineville_logo_${size}.webp?no-inline`,
      ),
    )
  }
  ...
  assert.match(
    ...
    /brand: 'CINEVILLE',\s*name: 'Cinéville',\s*url: 'https:\/\/www\.cineville\.fr\/'/,
  )
})
...
test('Cinéville appears in every existing admin provider surface, map and booking label', async () => {
  ...
  assert.match(
    ...
    /cineville: selectLatestProviderRun\(\s*'cineville'/,
  )
  assert.match(
    await source('components/CinemaTheaterMap.client.vue'),
    /'cineville',\s+THEATER_PROVIDER_COLORS\.cineville/,
  )
