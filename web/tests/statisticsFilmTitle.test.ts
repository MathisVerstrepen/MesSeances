...
import { computed, effectScope, isRef, nextTick, reactive, ref, shallowRef, watch, type Ref } from 'vue'
...
const source = await readFile(new URL('../app/pages/statistiques.vue', import.meta.url), 'utf8')
const script = source.match(/<script setup lang="ts">([\s\S]*?)<\/script>/)![1]!
const parsed = ts.createSourceFile('statistics.ts', script, ts.ScriptTarget.Latest, true)
...
interface MovieResult { movie: { slug: string; title: string }; backdrop_url?: string | null; currently_screened: boolean; theaters: never[] }
interface TitleResult { film: string; title: string; backdrop?: string | null }
interface Page {
...
async function harness(query: LocationQuery, options: { historyError?: boolean; lookup?: (film: string) => Promise<MovieResult> } = {}) {
  ...
  const bindings = {
    ...statistics, ...history, queriesEqual, safeBackdropUrl, computed, ref, shallowRef,
    watch: (...args: Parameters<typeof watch>) => scope.run(() => watch(...args)),
    useRoute: () => route,
    useRouter: () => ({ push: async ({ query: next }: { query: LocationQuery }) => { route.query = next } }),
    ...
    useMesSeancesApi: () => ({
      historyStatistics: async (query: StatisticsQuery) => {
        ...
        return { options: null, limits: { options: undefined }, totals: { showtimes: 0, movies: 0, theaters: 0, cities: 0 } }
      },
    ...
    useAsyncData: async <T>(key: string | Ref<string>, load: () => Promise<T>, options: { lazy?: boolean; server?: boolean }) => {
      ...
      if (isRef(key)) {
        ...
        scope.run(() => watch(key, async current => {
    ...
    useSeoMeta: () => {}, useHead: () => {},
    getApiErrorStatus: () => 503, getApiErrorCode: () => 'history_unavailable', getFrenchApiError: () => 'Indisponible',
    document: { removeEventListener: () => {} }
  }
  // SAFETY: The wrapper explicitly returns these actual page setup bindings.
   const page = await new Function(...Object.keys(bindings), `return (async () => { ${compiled}\nreturn { selectedFilmLabel, backdropUrl, backdropAvailable, backdropFailed, draft, error, apply, reset } })()`)(...Object.values(bindings)) as Page
  return { page, route, movieCalls, historyCalls, titleData: titleData!, stop: () => { unmount(); scope.stop() } }
}
...
test('selected title resolves with zero rows or failed history, including redirected and ended films', async context => {
  for (const historyError of [false, true]) {
    const h = await harness({ period: 'all', film: 'old-film-alias', city: 'no-intersection' }, {
      historyError,
    ...
    assert.deepEqual(h.movieCalls, [{ film: 'old-film-alias', query: { date: history.statisticsParisToday() } }])
...
test('title requests use only valid film identity, independently of other invalid filters', async context => {
  for (const film of [undefined, null, '', '  ', [], ['film-230'], ['film-230', 'film-231'], 'é'.repeat(101), '\0film', '\ud800']) {
  ...
  const h = await harness({ film: ' film-230 ', period: 'invalid', language: 'invalid' })
  context.after(h.stop)
...
test('unknown, unavailable and blank titles retain readable fallback and removable draft', async context => {
  for (const lookup of [async () => { throw new Error('not found') }, async () => { throw new Error('offline') }, async () => movie('  ')]) {
    const h = await harness({ period: 'all', film: 'unknown-film', campaign: 'test' }, { lookup })
    context.after(h.stop)
    assert.equal(h.page.selectedFilmLabel.value, 'Titre indisponible')
...
test('rapid film changes never show previous or late title and retain requested alias identity', async context => {
  ...
  const h = await harness({ film: 'film-230' }, { lookup: film => {
    if (film === 'film-231') return new Promise(resolve => { resolveSecond = resolve })
    if (film === 'redirect-alias') return new Promise(resolve => { resolveThird = resolve })
...
test('movie title stays plain text and never becomes route or API identity', async context => {
  const title = '<img src=x onerror=alert(1)> & Une très longue histoire'.repeat(10)
  const h = await harness({ film: 'film-230', period: 'all' }, { lookup: async () => movie(title) })
  context.after(h.stop)
...
test('backdrop reuses selected metadata and existing validation, with no extra API lookup', async context => {
  for (const url of [backdrop, undefined, null, 'https://untrusted.example/image.jpg', 'https://image.tmdb.org/t/p/w500/poster.jpg']) {
    const h = await harness({ film: 'old-film-alias', period: 'all' }, {
      historyError: true,
...
test('draft removal immediately hides backdrop before Apply; metadata cannot restore it', async context => {
  const h = await harness({ film: 'film-230' }, { lookup: async () => ({ ...movie(), backdrop_url: backdrop }) })
  context.after(h.stop)
...
test('failed image falls back and resets for changed image or film, including shared image URLs', async context => {
  ...
  await h.page.apply()
  await settle()
  assert.equal(h.page.backdropAvailable.value, false, 'same film/image does not retry on unrelated filters')
  h.titleData.value = { film: 'film-230', title: 'New image', backdrop: 'https://image.tmdb.org/t/p/w780/new-image.jpg' }
  assert.equal(h.page.backdropAvailable.value, true)
  h.page.backdropFailed.value = true
  h.route.query = { film: 'film-231' }
  assert.equal(h.page.backdropAvailable.value, false, 'old metadata hidden synchronously')
  await settle()
  assert.equal(h.page.backdropAvailable.value, true)
  ...
  await settle()
  assert.equal(h.page.backdropAvailable.value, true, 'different film retries even with identical image')
})
...
test('route changes reject stale backdrops and late metadata, including route removal', async context => {
  ...
  const h = await harness({ film: 'film-230' }, { lookup: film => film === 'film-231'
...
test('backdrop is decorative behind entire unclipped form, with contrast for labels and controls', () => {
  ...
  assert.match(form, /text-white hover:bg-white\/20 focus-visible:outline-white/)
