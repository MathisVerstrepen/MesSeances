...
const filmsSource = await readFile(
...
const showtimeSource = await readFile(
  new URL('../app/components/ShowtimeDatePicker.vue', import.meta.url),
  'utf8',
)
const dateBarSource = await readFile(
  new URL('../app/components/ShowtimeDateBar.vue', import.meta.url),
  'utf8',
)
const filmSource = await readFile(
...
const deferredSource = await readFile(
  new URL('../app/components/DeferredVueDatePicker.vue', import.meta.url),
  'utf8',
)
const loaderSource = await readFile(
  new URL('../app/utils/vueDatePickerLoader.ts', import.meta.url),
  'utf8',
)

const routeAndSharedSources = [
  filmsSource,
  searchSource,
  showtimeSource,
  dateBarSource,
...
test('route and shared date controls have no static Vuepic JavaScript or CSS imports', () => {
  for (const source of routeAndSharedSources) {
    assert.doesNotMatch(
      source,
      /import\s+(?:[^('].*?\s+from\s+)?['"]@vuepic\/vue-datepicker/,
    )
    assert.doesNotMatch(
      source,
      /import\s+['"]@vuepic\/vue-datepicker\/dist\/main\.css['"]/,
    )
  }
})
...
test('films only mounts deferred pickers inside custom and range modes', () => {
  assert.match(
    filmsSource,
    /v-if="draftFilters\.dateMode === 'custom'"[\s\S]*?<DeferredVueDatePicker[\s\S]*?\bactive\b/,
  )
  assert.match(
    filmsSource,
    /v-else-if="draftFilters\.dateMode === 'range'"[\s\S]*?<DeferredVueDatePicker[\s\S]*?\bactive\b/,
  )
...
test('custom-trigger pickers render their trigger before activation then load and open', () => {
  ...
  assert.match(
    deferredSource,
    /await ensureLoaded\(\)[\s\S]*?await nextTick\(\)[\s\S]*?datePicker\.value\?\.openMenu\(\)/,
  )
  assert.match(
    deferredSource,
    /inputRoot\s*\.querySelector<HTMLElement>\('button'\)\s*\?\.focus\(\{ preventScroll: true \}\)/,
  )
  assert.match(searchSource, /<ShowtimeDateBar/)
  assert.match(dateBarSource, /<ShowtimeDatePicker[\s\S]*?<template #trigger/)
  assert.match(
    showtimeSource,
    /<DeferredVueDatePicker[\s\S]*?<template #trigger>/,
  )
})
...
test('single-date showtime surfaces share the canonical date bar', () => {
  ...
  for (const source of [filmSource, searchSource])
    assert.doesNotMatch(source, /<ShowtimeDatePicker/)
  assert.match(dateBarSource, /role="tablist"/)
  assert.match(dateBarSource, /event\.key === 'ArrowRight'/)
  assert.match(
    dateBarSource,
    /selectAdjacentDate\(\s*event: KeyboardEvent,\s*index: number,\s*dates: string\[\],?\s*\)/,
  )
  assert.match(
    dateBarSource,
...
test('date bar limits navigation and roving tabindex to each explicit responsive tab mode', () => {
  ...
  assert.match(
    dateBarSource,
    /group\.dates\.includes\(props\.selectedDate\)\s*\? props\.selectedDate\s*: group\.dates\[0\]/,
  )
  assert.match(
    dateBarSource,
...
test('today and tomorrow tabs stay selectable independently from screening availability', () => {
  ...
  assert.doesNotMatch(
    dateBarSource,
    /props\.availableDates\.filter\(date => date === props\.today \|\| date === tomorrowDate\.value\)/,
  )
...
test('date bar preserves two-tab keyboard wrapping and post-selection focus', () => {
  assert.match(
    dateBarSource,
    /event\.key === 'ArrowRight'\) nextIndex = \(index \+ 1\) % dates\.length/,
  )
  assert.match(
    dateBarSource,
    /event\.key === 'ArrowLeft'\)\s+nextIndex = \(index - 1 \+ dates\.length\) % dates\.length/,
  )
  assert.match(dateBarSource, /event\.key === 'Home'\) nextIndex = 0/)
  assert.match(
    dateBarSource,
    /event\.key === 'End'\) nextIndex = dates\.length - 1/,
  )
  assert.match(
    dateBarSource,
    /emit\('select', nextDate\)[\s\S]*?nextTick\(\(\) => tabs\?\.\[nextIndex\]\?\.focus\(\)\)/,
  )
})
...
test('search accepts unavailable quick dates without broadening calendar-date availability', () => {
  assert.match(
    searchSource,
    /addCalendarDays,\s*createServiceTimeOptions,\s*formatLongDate,\s*todayInParis/,
  )
  assert.match(
    searchSource,
    /const quickDateOptions = computed\(\(\) => \[\s*todayDate\.value,\s*addCalendarDays\(todayDate\.value, 1\),?\s*\]\)/,
  )
  assert.match(
    searchSource,
  ...
  assert.match(
    searchSource,
    /const query = submittedQuery\(search, preserveSelection\)[\s\S]*?await router\.replace\(\{ query \}\)/,
  )
  assert.match(
    searchSource,
  ...
  assert.match(
    searchSource,
    /:disabled="!hasAvailableDates"[\s\S]*?desktop-tabs="today-tomorrow"/,
  )
})
...
test('cinema keeps route-selected quick tabs visible with empty availability', () => {
  ...
  assert.match(
    cinemaSource,
    /<ShowtimeDateBar\s+:selected-date="selectedDate"\s+:available-dates="availableDates"\s+:today="todayInParis\(\)"\s+:disabled="availableDates\.length === 0"\s+@select="selectDate"\s*\/>/,
  )
  assert.doesNotMatch(
    cinemaSource,
    /<ShowtimeDateBar\s+v-if="availableDates\.length"/,
  )
  assert.match(
    cinemaSource,
  ...
  assert.match(
    cinemaSource,
    /v-else-if="normalizedResults\.length === 0"[\s\S]*?Aucune séance à cette date/,
  )
})
...
test('planning empty states and film all-date controls remain isolated', () => {
  ...
  assert.match(
    planningSource,
    /date: date\.value,[\s\S]*?api\.timeline|api\.timeline\(\{[\s\S]*?date: date\.value/,
  )
  assert.match(
    planningSource,
    /Aucune séance pour cette date et cette langue\./,
  )
  assert.match(filmSource, /<ShowtimeDateBar[^>]*mobile-tabs="all"/)
  assert.match(
    filmSource,
    /<ShowtimeDateBar\s+v-if="hasAvailableDates"[^>]*:available-dates="availableDates"/,
  )
...
test('search compacts desktop date tabs while film expands its mobile date tabs', () => {
  assert.match(
    searchSource,
    /<ShowtimeDateBar[\s\S]*?desktop-tabs="today-tomorrow"[\s\S]*?calendar-position="end"[\s\S]*?stretch-tabs[\s\S]*?@select="form\.date = \$event"/,
  )
  assert.match(
    filmSource,
    /<ShowtimeDateBar[^>]*mobile-tabs="all"[^>]*@select="selectMobileDate"/,
  )
  assert.equal((filmSource.match(/mobile-tabs="all"/g) ?? []).length, 1)
  assert.match(
    filmSource,
    /<ShowtimeDateBar[^>]*@select="updateFilmQuery\(\{ date: \$event === fallbackDate\(\) \? undefined : \$event \}\)"/,
  )
})
...
test('search date tabs share full row width before a fixed calendar trigger', () => {
  ...
  assert.match(
    dateBarSource,
    /layoutOrder = computed<Array<'calendar' \| 'tabs'>>\(\s*\(\) =>\s*props\.calendarPosition === 'end'\s*\? \['tabs', 'calendar'\]\s*: \['calendar', 'tabs'\],?\s*\)/,
  )
  assert.match(
    dateBarSource,
  ...
  assert.match(
    dateBarSource,
    /group\.mode === 'today-tomorrow' &&\s*\(group\.key === 'mobile' \|\| stretchTabs\)\s*\? 'w-full'\s*: 'w-auto'/,
  )
  assert.match(
    dateBarSource,
    /class="calendar-trigger grid size-9 shrink-0[^"]*sm:size-10"/,
  )
})

test('film mobile date selection updates the route without closing its panel', () => {
  const selectMobileDateBody = filmSource.match(
    /function selectMobileDate\(date: string\) \{([\s\S]*?)\n\}/,
  )?.[1]
  assert.ok(selectMobileDateBody)
  assert.match(
    selectMobileDateBody,
    /updateFilmQuery\(\{ date: date === fallbackDate\(\) \? undefined : date \}\)/,
  )
  assert.doesNotMatch(
    selectMobileDateBody,
    /openMobilePanel|closeMobilePanel|mobileDateTrigger|\.focus\(/,
  )
...
test('custom picker CSS targets Vuepic 14 classes and outranks late package defaults', () => {
  for (const source of routeAndSharedSources)
    assert.doesNotMatch(source, /dp__/)
  ...
  assert.match(
    showtimeSource,
    /\.editorial-calendar-menu \.dp--calendar-header-item/,
  )
  assert.match(
    showtimeSource,
    /\.editorial-calendar-menu \.dp--month-year-select/,
  )
