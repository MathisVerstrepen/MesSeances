...
test('keeps rejected matches out of the unresolved section', () => {
  assert.deepEqual(
    adminPendingMatchesForFilter(matches, 'unresolved').map(
      (match) => match.id,
    ),
    ['review', 'unmatched'],
  )
...
test('keeps only rejected matches in the Non-TMDB section', () => {
  assert.deepEqual(
    adminPendingMatchesForFilter(matches, 'rejected').map((match) => match.id),
    ['rejected'],
  )
})
...
test('keeps only matched movies in the associated TMDB section', () => {
  assert.deepEqual(
    adminPendingMatchesForFilter(matches, 'matched').map((match) => match.id),
    ['matched'],
  )
})
...
test('encodes normalized TMDB queries on a fixed HTTPS movie-search URL', () => {
  assert.equal(
    adminTMDBSearchURL('America America 2 (Version Kannada)'),
    'https://www.themoviedb.org/search/movie?query=America+America+2',
  )
...
test('shows safe new-tab TMDB search alongside generic provider links in shared movie cards', async () => {
  ...
  const sourceSection = page.match(
    /<section\s+class="min-w-0"\s+:aria-labelledby="`source-title-\$\{domKey\(match\)\}`"\s*>[\s\S]*?<\/section>/,
  )?.[0]
  assert.ok(sourceSection)
  const searchLink = sourceSection.match(
    /<a\s+v-if="adminTMDBSearchURL\(match.source_title\)"[^>]*>[\s\S]*?<\/a\s*>/,
  )?.[0]
  assert.ok(searchLink)
  assert.match(searchLink, /:href="adminTMDBSearchURL\(match.source_title\)"/)
  assert.match(searchLink, /target="_blank"\s+rel="noopener noreferrer"/)
  assert.match(searchLink, /aria-label="TMDB, ouverture dans un nouvel onglet"/)
  assert.match(
    sourceSection,
    /<a\s+v-if="match.source_detail_url"\s+:href="match.source_detail_url"\s+target="_blank"\s+rel="noopener noreferrer"/,
  )
  assert.match(
    sourceSection,
    /Voir sur \$\{providerLabel\(match.source_provider\)\}/,
  )
  assert.doesNotMatch(
    sourceSection,
    /activeTab|match\.status|source_provider ===/,
  )
  ...
  assert.deepEqual(calls, [
    {
      ...
      options: {
        credentials: 'include',
        query: { status: 'unresolved', limit: 20, offset: 0 },
      },
    },
    {
      url: 'http://localhost:8080/api/v1/admin/tmdb-matches',
      options: {
        credentials: 'include',
        query: { status: 'matched', limit: 20, offset: 20 },
      },
    },
    {
      url: 'http://localhost:8080/api/v1/admin/tmdb-matches',
      options: {
        credentials: 'include',
        query: { status: 'matched', limit: 20, offset: 40, search: 'Alien_%' },
      },
    },
  ])
})
...
test('posts a matched correction with credentials, encoded identity, and optimistic token', async () => {
  ...
  const input = {
    tmdb_id: 456,
    expected_updated_at: '2026-08-30T12:34:56.123456Z',
  }
  Object.assign(globalThis, {
    useRuntimeConfig: () => ({ public: { apiBase: 'http://localhost:8080/' } }),
    $fetch: (url: string, options: AdminFetchOptions) => {
  ...
  const response = await useMesSeancesApi().adminCorrectMatch(
    'pathe',
    'film/123',
    input,
  )
  ...
  Object.assign(globalThis, {
    ...
    $fetch: (url: string, options: AdminFetchOptions) => {
      ...
      return Promise.resolve({
        ...
        local_movie_id: 'local-film/7',
      })
    },
  })

  const response = await useMesSeancesApi().adminAddLocalMovieMembers(
    'local-film/7',
    input,
  )
...
test('maps matched-metadata refresh failures to safe French messages', () => {
  ...
  assert.equal(
    getFrenchAdminApiError(failure('tmdb_metadata_refresh_in_progress')),
    'Une autre opération TMDB est déjà en cours.',
  )
  assert.equal(
    getFrenchAdminApiError(failure('tmdb_metadata_refresh_unavailable')),
    'Le service d’actualisation des métadonnées TMDB est temporairement indisponible.',
  )
  assert.equal(
    getFrenchAdminApiError(failure('tmdb_metadata_refresh_failed')),
...
test('maps stale matched corrections to the list-refresh conflict message', () => {
  assert.equal(
    getFrenchAdminApiError({
      status: 409,
      data: { error: { code: 'review_conflict' } },
    }),
...
test('maps matched-metadata job states to loading, counters, and safe failure UI', () => {
  ...
  assert.deepEqual(adminTMDBMetadataRefreshPresentation(null), {
    running: false,
    summary: null,
    error: '',
  })
  assert.deepEqual(adminTMDBMetadataRefreshPresentation(jobs.running), {
    running: true,
    summary: null,
    error: '',
  })
  assert.deepEqual(adminTMDBMetadataRefreshPresentation(jobs.succeeded), {
    running: false,
    summary,
    error: '',
  })
...
test('refreshes match lists once only after an observed running job succeeds', () => {
  const startedAt = '2026-08-28T12:00:00Z'
  const running: AdminTMDBMetadataRefreshJob = {
    state: 'running',
    started_at: startedAt,
    finished_at: null,
    summary: null,
  ...
  assert.equal(
    shouldRefreshAdminTMDBMatchLists(null, succeeded, new Set()),
    false,
  )
  assert.equal(
    shouldRefreshAdminTMDBMatchLists(startedAt, running, new Set()),
    false,
  )
  assert.equal(
    shouldRefreshAdminTMDBMatchLists(startedAt, succeeded, new Set()),
    true,
  )
  assert.equal(
    shouldRefreshAdminTMDBMatchLists(
...
test('keeps matched search route-owned, debounced, server-side, and cleaned up', async () => {
  ...
  assert.match(
    page,
    /const OWNED_QUERY_KEYS = \[\s*'tab',\s*'q',\s*'page',\s*'rejected_page',\s*'matched_page',\s*'groups_page',?\s*\] as const/,
  )
  assert.match(page, /q: nextSearch \|\| undefined/)
  assert.match(
    page,
    /const requestedSearch = adminTMDBMatchedSearch\(\s*singularQueryValue\(route\.query\.q\),?\s*\)/,
  )
  assert.match(page, /matchedSearchInput\.value = requestedSearch/)
  assert.match(
    page,
    /matchedPage\.value !== lastLoadMatchedPage \|\|\s*matchedSearch\.value !== lastLoadMatchedSearch/,
  )
  assert.match(
    page,
    /api\.adminPendingMatches\(\s*'matched',\s*PAGE_SIZE,\s*matchedOffset\.value,\s*matchedSearch\.value \|\| undefined,?\s*\)/,
  )

  assert.match(
    page,
    /function updateMatchedSearch\(\) \{\s*clearMatchedSearchTimer\(\)/,
  )
  assert.match(page, /}, ADMIN_TMDB_MATCH_SEARCH_DEBOUNCE_MS\)/)
  assert.match(
    page,
    /adminQuery\(\s*page\.value,\s*rejectedPage\.value,\s*1,\s*groupsPage\.value,\s*activeTab\.value,\s*search,?\s*\)/,
  )
  assert.match(
    page,
    /onBeforeUnmount\(\(\) => \{[\s\S]*clearMatchedSearchTimer\(\)[\s\S]*clearMetadataRefreshPolling\(\)/,
  )

  assert.match(page, /v-if="activeTab === 'matched'"\s+for="matched-search"/)
  assert.match(
    page,
    /id="matched-search"[\s\S]*maxlength="1024"[\s\S]*@input="updateMatchedSearch"/,
  )
