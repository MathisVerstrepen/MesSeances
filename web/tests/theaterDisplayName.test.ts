...
test('brand-only cinema names include the city', () => {
  for (const city of ['Annecy', 'Boulogne sur Mer']) {
    assert.equal(
      theaterDisplayName({ name: 'Megarama', city }),
      `Megarama ${city}`,
    )
  }
  for (const name of [
...
test('brand matching ignores case and surrounding whitespace', () => {
  assert.equal(
    theaterDisplayName({ name: ' megarama ', city: ' Annecy ' }),
    'megarama Annecy',
  )
  assert.equal(
    theaterDisplayName({ name: ' cinéville ', city: ' Laval ' }),
    'cinéville Laval',
  )
  assert.equal(
    theaterDisplayName({ name: ' cinewest ', city: ' Royan ' }),
    'cinewest Royan',
  )
})
