...
test('directory query restores all independent mode combinations and omits defaults', () => {
  for (const view of ['list', 'map'] as const) {
    for (const location of ['city', 'nearby'] as const) {
      const query = cinemaDirectoryQuery(
        {},
        { view, location, search: ' Lille ' },
      )
  ...
  assert.deepEqual(parseCinemaDirectoryQuery({}), {
    view: 'list',
    location: 'city',
    search: '',
  })
...
test('directory query removes invalid, empty, repeated and explicit default values', () => {
  ...
  assert.deepEqual(
    cinemaDirectoryQuery({ view: 'map', location: ['nearby', 'nearby'] }),
    { view: 'map' },
  )
  assert.deepEqual(
    cinemaDirectoryQuery({ view: ['map', 'map'], location: 'nearby' }),
    { location: 'nearby' },
  )
})
...
test('directory changes preserve unrelated query values and do not mutate input', () => {
  ...
  assert.deepEqual(cinemaDirectoryQuery(query, { view: 'list' }), {
    q: 'Lille',
    location: 'nearby',
    tag: query.tag,
    flag: null,
  })
  assert.deepEqual(cinemaDirectoryQuery(query, { location: 'city' }), {
    q: 'Lille',
    view: 'map',
    tag: query.tag,
    flag: null,
  })
  assert.deepEqual(cinemaDirectoryQuery(query, { search: '' }), {
    view: 'map',
    location: 'nearby',
    tag: query.tag,
    flag: null,
...
test('memory history restores independent modes, replaces search and does not retry on unrelated changes', async () => {
  ...
  const state = computed(() =>
    parseCinemaDirectoryQuery(router.currentRoute.value.query),
  )
  scope.run(() =>
    watch(
      () => state.value.location,
      (mode) => location.setNearbyMode(mode === 'nearby'),
      { flush: 'sync' },
    ),
  )
...
test('page wires route-derived display, push controls, replace search, mounted activation and disposal', async () => {
  ...
  assert.match(
    page,
    /function setDisplayMode[\s\S]*?router\.push\(\{ query \}\)/,
  )
  assert.match(
    page,
    /function updateSearch[\s\S]*?router\.replace\(\{ query \}\)/,
  )
  assert.match(
    page,
    /watch\(\s*locationMode,[\s\S]*?if \(isMounted\) location\.setNearbyMode[\s\S]*?flush: 'sync'/,
  )
  assert.match(
    page,
    /onMounted\(async \(\) => \{\s*isMounted = true\s*location\.setNearbyMode/,
  )
  ...
  assert.match(
    page,
    /function showList\(\) \{\s*setDisplayMode\(\{ view: 'list' \}\)/,
  )
