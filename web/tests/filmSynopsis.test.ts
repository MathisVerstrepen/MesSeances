...
test('film synopsis clamps to three lines and exposes expansion only when text overflows', () => {
  ...
  assert.match(
    page,
    /:aria-expanded="synopsisOverflows \? synopsisExpanded : undefined"/u,
  )
...
test('film synopsis expansion resets when its movie changes', () => {
  assert.match(
    page,
    /watch\(\s*\(\) => schedule\.value\?\.movie\.overview,\s*async \(\) => \{\s+synopsisExpanded\.value = false/u,
  )
  assert.match(
    page,
    /watch\(slug, \(\) => \{[\s\S]*?synopsisExpanded\.value = false/u,
  )
})
