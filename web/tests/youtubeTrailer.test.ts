...
} from '../app/utils/youtubeTrailer.ts'
...
test('accepts only canonical 11-character YouTube video keys', () => {
  ...
  for (const key of invalidKeys)
    assert.equal(validatedYouTubeKey(key), null, String(key))
})
...
test('constructs only privacy-enhanced autoplay embed URLs from valid keys', () => {
  ...
  assert.equal(
    youtubeNoCookieTrailerEmbedUrl('https://youtube.com/watch?v=AbC-12_xYz9'),
    null,
  )
...
test('trailer header exposes one preferred action and modal owns the optional VO switch', async () => {
  const component = await readFile(
    new URL('../app/components/MovieTrailer.vue', import.meta.url),
    'utf8',
  )
  ...
  assert.match(
    component,
    /v-if="secondaryTrailer && activeTrailer\?\.variant !== 'VO'"/u,
  )
  assert.match(component, />\s*Voir en VO\s*<\/button>/u)
  assert.match(component, /@click="selectTrailer\(secondaryTrailer\)"/u)
  assert.match(
    component,
    /async function selectTrailer\(trailer: TrailerSelection \| null\)/u,
  )
  assert.match(
    component,
    /const embedUrl = computed\(\s*\(\) =>\s*isOpen\.value\s*\? youtubeNoCookieTrailerEmbedUrl\(activeTrailer\.value\?\.youtubeKey\)/u,
  )
  ...
  assert.match(
    component,
    /:key="`\$\{activeTrailer\?\.variant\}-\$\{activeTrailer\?\.youtubeKey\}`"/u,
  )
  assert.equal(component.match(/>\s*Bande-annonce\s*<\/button>/gu)?.length, 1)
  assert.doesNotMatch(
    component,
    /Bande-annonce \{\{ primaryTrailer\.variant \}\}/u,
  )
  assert.doesNotMatch(
    component,
    /Bande-annonce \{\{ secondaryTrailer\.variant \}\}/u,
  )
  assert.match(
    component,
    /:aria-label="`Voir la bande-annonce \$\{primaryTrailer\.variant\} de \$\{movieTitle\}`"/u,
  )
  assert.match(
    component,
    /:title="`Bande-annonce \$\{activeTrailer\?\.variant\} de \$\{movieTitle\} sur YouTube`"/u,
  )
  assert.match(
    component,
    /:aria-label="`Voir la bande-annonce VO de \$\{movieTitle\}`"/u,
  )
  assert.match(
    component,
    /:aria-label="`Fermer la bande-annonce \$\{activeTrailer\?\.variant\} de \$\{movieTitle\}`"/u,
  )
})
