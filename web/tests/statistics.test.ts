...
test('two draft multi-controls use native disclosure, labeled search and checkboxes while advanced filters stay scalar', async () => {
  ...
  assert.match(page, /<StatisticsMultiSelect v-for="filter in primaryFilters"/)
  assert.match(page, /:max-selections="statisticsMaxSelections"/)
  ...
  assert.match(page, /else if \(error.value\) await reload\(\)/)
...
test('page integrates eight sections, accessible components, recovery and footer-only discovery', async () => {
  ...
  assert.match(page, /Aucune séance pour ces filtres/)
  assert.match(page, /coverage\.stale/)
