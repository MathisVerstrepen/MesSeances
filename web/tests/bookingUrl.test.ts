...
test('accepts the captured CGR booking host and route', () => {
  assert.deepEqual(
    safeBookingUrl(
      'https://achat.cgrcinemas.fr/cgr-abbeville-la-sucrerie/r/123456',
      'cgr',
    ),
    {
      provider: 'cgr',
...
test('rejects unsafe or mismatched CGR booking URLs', () => {
  ...
  for (const url of invalidUrls)
    assert.equal(safeBookingUrl(url, 'cgr'), null, url)
  assert.equal(
    safeBookingUrl('https://achat.cgrcinemas.fr/cgr-lille/r/123', 'ugc'),
    null,
  )
})
...
test('accepts only the 34 verified Megarama cinema roots and classifies reservation fragments', () => {
  ...
  for (const host of hosts) {
    for (const slash of ['', '/']) {
      ...
      assert.deepEqual(safeBookingUrl(root, 'megarama'), {
        provider: 'megarama',
        kind: 'website',
        url: `https://${host}/`,
      })
      ...
      for (const provider of ['ugc', 'kinepolis', 'pathe', 'cgr'] as const)
        assert.equal(safeBookingUrl(root, provider), null)
    }
  }
})
...
test('rejects unsafe Megarama authorities, paths, fragments and identity lengths before URL normalization', () => {
  ...
  const invalid = [
    'https://megarama.fr',
    'https://www.megarama.fr',
    'https://unknown.megarama.fr',
    'https://other.ticketingcine.com',
    'https://www.cine-armentieres.fr',
    `${root}.evil.test`,
    `${root}:443`,
    `${root}:8443`,
    `${root}.`,
    'https://user@bordeaux.megarama.fr',
    'https://user:password@bordeaux.megarama.fr',
    'http://bordeaux.megarama.fr',
    '//bordeaux.megarama.fr',
    'javascript:alert(1)',
    'https://BORDEAUX.MEGARAMA.FR',
    `${root}/foo/..`,
    `${root}/%2e%2e`,
    `${root}\\`,
    `${root}//`,
    ...
    `${root}#`,
    `${root}#other`,
    `${root}${fragment}&other=1`,
    `${root}${fragment}?other=1`,
    `${root}${fragment}/`,
    `${root}#showsession?id=`,
    `${root}#showsession?id=_bad`,
    `${root}#showsession?id=-bad`,
    `${root}#showsession?id=bad.id`,
    `${root}#showsession?id=bad%2fid`,
    `${root}#showsession?id=${'a'.repeat(112)}`,
    ` ${root}`,
    `${root}\n`,
    'https://bord\neaux.megarama.fr',
  ]
  for (const url of invalid)
    assert.equal(safeBookingUrl(url, 'megarama'), null, url)
  const maximum = `${root}#showsession?id=${'a'.repeat(111)}`
  assert.equal(safeBookingUrl(maximum, 'megarama')?.kind, 'booking')
  assert.equal(
    safeBookingUrl(`${root}#showsession?id=Ab_1-2`, 'megarama')?.kind,
    'booking',
  )
})
...
test('accepts exactly five booking-only aliases for their matching session cinema', () => {
  ...
  for (const [host, cinema] of aliases) {
    for (const slash of ['', '/']) {
      ...
      for (const provider of ['ugc', 'kinepolis', 'pathe', 'cgr'] as const)
        assert.equal(safeBookingUrl(`${root}${fragment}`, provider), null)
      ...
      for (const [, otherCinema] of aliases) {
        if (otherCinema !== cinema)
          assert.equal(
            safeBookingUrl(`${root}#showsession?id=emsx${otherCinema}00123456`),
            null,
          )
      }
      for (const suffix of [
        ...
        '#showsession',
        '#showsession?id=',
        '#showsession?id=emsx056500123456',
        `#showsession?id=emsx${cinema}1234567`,
        `#showsession?id=emsx${cinema}123456789`,
        `#showsession?id=EMSX${cinema}00123456`,
        `#showsession?id=emsx${cinema}0012345a`,
        `${fragment}&extra=1`,
        `${fragment}/`,
        `${fragment}%0A`,
        `${fragment}\n`,
  ...
  assert.equal(
    safeBookingUrl(
      'https://royalpalacenogent.fr/#showsession?id=emsx080900123456',
    ),
    null,
  )
  assert.equal(
    safeBookingUrl(
      'https://www.jean-jaures.megarama.fr/#showsession?id=emsx120400123456',
    ),
    null,
  )
})
test('accepts only exact Cinéville session booking routes, never a website fallback', () => {
  ...
  assert.deepEqual(safeBookingUrl(url), {
    provider: 'cineville',
    url,
    kind: 'booking',
  })
  assert.deepEqual(safeBookingUrl(url, 'cineville'), {
    provider: 'cineville',
    url,
    kind: 'booking',
  })
  ...
  assert.equal(
    safeBookingUrl('https://www.ugc.fr/reservation', 'cineville'),
    null,
  )
  const invalid = [
    ...
    `${url}/`,
    `${url}/../1234`,
    url.replace('/vad/', '/other/../vad/'),
    url.replace('/707/', '/%37%30%37/'),
    url.replace('/707/', '/707%2F/'),
    url.replace('/707/', '/707\\/'),
    url.replace('/707/', '/707\n/'),
    ` ${url}`,
    `${url}\n`,
    'https://www.cineville.fr/',
    'https://www.cineville.fr/programmes/laval',
  ]
  for (const component of [
    ...
    for (const value of ['707', '149056', '1234'])
      invalid.push(url.replace(`/${value}`, `/${component}`))
  }
