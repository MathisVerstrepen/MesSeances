...
test('keeps share controls below the app header and share popups above it', async () => {
  ...
  const controlZIndex = Number(
    shareButton.match(
      /class="[^"]*\bshare-control\b[^"]*\bz-(\d+)\b[^"]*"/,
    )?.[1],
  )
  const popupZIndex = Number(
    shareButton.match(/class="[^"]*\bshare-popup\b[^"]*\bz-(\d+)\b[^"]*"/)?.[1],
  )
  const headerZIndex = Number(
    appHeader.match(/class="[^"]*\bz-(\d+)\b[^"]*"/)?.[1],
  )
