...
test('adapts slot results without mutation and preserves effective time, raw end, theater, and top-level media', () => {
  const source: SlotResult[] = [
    {
      showtime: {
        ...
        end_time: '2026-08-24T20:01:00+02:00',
        estimated_end_time: null,
        estimated_end_ads_minutes: null,
        language: 'VOSTFR',
        format: 'IMAX',
        room: '4',
        booking_url: 'https://www.ugc.fr/reservation',
      },
      theater: {
        provider: 'ugc',
        id: 'ugc-1',
  ...
  assert.deepEqual(result, {
    ...
    theaterId: 'ugc-1',
    advertisedStartTime: '2026-08-24T18:00:00+02:00',
    effectiveStartTime: '2026-08-24T18:15:00+02:00',
    end: canonical('2026-08-24T20:01:00+02:00'),
    language: 'VOSTFR',
    format: 'IMAX',
    room: '4',
    bookingUrl: 'https://www.ugc.fr/reservation',
...
test('adapts theater showtimes without mutation and injects theater while mapping advertised start as effective', () => {
  const source = {
    ...
    theater: {
      provider: 'kinepolis',
      id: 'k-1',
      slug: 'kinepolis-lille',
      name: 'Kinepolis Lille',
      address: 'Rue du film',
      city: 'Lille',
      city_slug: 'lille',
      postal_code: '59000',
    ...
    showtimes: [
      {
        ...
        id: 'show-1',
        movie,
        start_time: '2026-08-24T19:00:00+02:00',
        end_time: '2026-08-24T21:01:00+02:00',
        estimated_end_time: null,
        estimated_end_ads_minutes: null,
        language: 'VF',
        format: '2D',
...
test('normalizers preserve explicit estimated provenance including custom zero ads and never infer from runtime', () => {
  for (const ads of [0, 15, 30, 120]) {
    ...
    const showtime = {
      provider: 'cineville' as const,
      id: 'cineville-showing-4670-1',
      movie: { ...movie, runtime_minutes: 93 },
      start_time: start,
      end_time: start,
      estimated_end_time: time,
      estimated_end_ads_minutes: ads,
    ...
    const slot: SlotResult = {
      ...
      poster_url: null,
      backdrop_url: null,
      effective_start_time: new Date(
        Date.parse(start) + ads * 60_000,
      ).toISOString(),
      effective_end_time: time,
      buffer_ads_minutes: ads,
      slack_before_minutes: 0,
      slack_after_minutes: 0,
    }
    ...
    assert.equal(
      toSlotShowtimeResults([slot])[0]!.effectiveStartTime,
      slot.effective_start_time,
    )
    ...
    assert.deepEqual(
      toSlotShowtimeResults([slot])[0]!.end,
      canonical(slot.showtime.end_time),
    )
  }
})
...
test('estimated compatibility uses returned ends and effective starts, with touching boundaries and invalid intervals', () => {
  const selected = view({
    provider: 'cineville',
    key: 'cineville:cineville-showing-4670-1',
    movieRuntimeMinutes: 93,
    effectiveStartTime: '2026-08-24T18:30:00+02:00',
    end: { time: '2026-08-24T20:03:00+02:00', estimated: true, adsMinutes: 30 },
  })
  const touching = view({
    effectiveStartTime: '2026-08-24T20:03:00+02:00',
    end: canonical('2026-08-24T22:00:00+02:00'),
  })
  assert.equal(areShowtimeResultsCompatible(selected, touching), true)
  assert.equal(
    areShowtimeResultsCompatible(selected, {
      ...touching,
      effectiveStartTime: '2026-08-24T20:02:00+02:00',
    }),
    false,
  )
  ...
  assert.equal(
    areShowtimeResultsCompatible(
      selected,
      view({
        effectiveStartTime: '2026-08-24T16:00:00+02:00',
        end: canonical('2026-08-24T18:30:00+02:00'),
      }),
    ),
    true,
  )
  for (const end of [
    null,
    canonical('invalid'),
    canonical(selected.effectiveStartTime),
    canonical(selected.advertisedStartTime),
  ]) {
    assert.equal(
      areShowtimeResultsCompatible({ ...selected, end }, touching),
      false,
    )
  }
  assert.deepEqual(
    parseShowtimeSelection(serializeShowtimeSelection([selected.key])),
    [selected.key],
  )
})

function view(
  overrides: Partial<ShowtimeResultViewModel>,
  ...
  return {
    ...
    theaterId: 'ugc-25',
    advertisedStartTime: '2026-08-24T18:00:00+02:00',
    effectiveStartTime: '2026-08-24T18:00:00+02:00',
    end: canonical('2026-08-24T20:00:00+02:00'),
    language: 'VF',
    format: '2D',
    room: '',
    bookingUrl: null,
...
test('Cinéville selection tokens preserve cinema-scoped IDs and reject noncanonical numbers', () => {
  ...
  assert.equal(
    new Set(parseShowtimeSelection(serializeShowtimeSelection(keys))).size,
    ids.length,
  )
  for (const id of [
    '0-1',
    ...
    assert.equal(
      serializeShowtimeSelection([`cineville:cineville-showing-${id}`]),
      undefined,
      id,
    )
  }
...
test('runtime alone never turns unknown ends into compatible sessions', () => {
  const known = view({
    key: 'ugc:ugc-showing-1',
    advertisedStartTime: '2026-08-24T22:00:00+02:00',
    effectiveStartTime: '2026-08-24T22:00:00+02:00',
    end: canonical('2026-08-24T23:00:00+02:00'),
  })
  for (const runtime of [0, 93, 118]) {
    const unknown = view({
      provider: 'cineville',
      key: 'cineville:cineville-showing-707-149056',
    ...
    assert.deepEqual(filterCompatibleShowtimeResults([unknown, known], []), [
      unknown,
      known,
    ])
    assert.deepEqual(
      filterCompatibleShowtimeResults([unknown, known], [unknown.key]),
      [unknown],
    )
    assert.deepEqual(
      filterSelectedShowtimeResults([unknown, known], [unknown.key]),
      [unknown],
    )
    assert.deepEqual(
      parseShowtimeSelection(serializeShowtimeSelection([unknown.key])),
      [unknown.key],
    )
    assert.equal(
      areShowtimeResultsCompatible(
        {
          ...unknown,
            ...
            time: '2026-08-24T19:48:00+02:00',
            estimated: true,
            adsMinutes: 15,
          },
        },
  ...
  for (const runtime of [0, 93, 118]) {
    const response: TheaterShowtimesResponse = {
      ...
      showtimes: [
        {
          provider: 'cineville',
          id: 'cineville-showing-639-149056',
          movie: {
            ...movie,
            slug: 'cineville-film--693091020261',
          ...
          format: 'DOLBY',
          room: '4',
          booking_url: 'https://www.cineville.fr/vad/639/149056/1234',
...
test('sorts non-mutatively by advertised start then showtime ID', () => {
  const source = [
    view({
      key: 'late',
      showtimeId: 'z',
      advertisedStartTime: '2026-08-24T19:00:00+02:00',
    }),
  ...
  assert.deepEqual(
    sorted.map((result) => result.key),
    ['tie-a', 'tie-b', 'late'],
  )
  assert.deepEqual(
    source.map((result) => result.key),
    sourceOrder,
  )
...
test('groups by provider and movie key while preserving first-seen group and item order', () => {
  ...
  assert.deepEqual(
    ...
    ['ugc:film-1', 'kinepolis:film-1'],
  )
  assert.deepEqual(
    groups.map((group) => group.results.map((result) => result.key)),
    [['ugc-a', 'ugc-c'], ['kinepolis-a']],
  )
})
...
test('round-trips compact selection tokens for every provider in canonical key order', () => {
  ...
  const compact =
    'cP0798-64xwG_nrkC9zjLejLtFMtVueK0Lg_DRqx52c8R0XG7w,v707-149056,kVista_Session-42,memsx056500123456,pV3001S170227,u330660140434'

  assert.equal(serializeShowtimeSelection(keys), compact)
  assert.deepEqual(parseShowtimeSelection(compact), [
    cgr,
    cineville,
    kinepolis,
    megarama,
  ...
  assert.deepEqual(parseShowtimeSelection(`${compact},${compact}`), [
    cgr,
    cineville,
    kinepolis,
    megarama,
...
test('ignores malformed provider tokens and old verbose selection values', () => {
  ...
  assert.deepEqual(
    parseShowtimeSelection([...malformed, 'u12', 'u12'].join(',')),
    [valid],
  )
...
test('round-trips exact Megarama case and ASCII identity limits while rejecting unsafe tokens', () => {
  ...
  for (const id of [
    ...
    assert.equal(
      serializeShowtimeSelection([`megarama:megarama-showing-${id}`]),
      undefined,
    )
...
test('unknown Megarama ends remain selectable but never prove compatibility', () => {
  const unknown = view({
    provider: 'megarama',
    key: 'megarama:megarama-showing-emsx056500123456',
    end: null,
    effectiveStartTime: '2026-08-24T18:15:00+02:00',
  })
  const known = view({
  ...
  assert.deepEqual(
    filterCompatibleShowtimeResults([unknown, known], [known.key]),
    [known],
  )
  assert.deepEqual(
    filterCompatibleShowtimeResults([unknown, known], [unknown.key, known.key]),
    [unknown, known],
  )
  assert.deepEqual(
    parseShowtimeSelection(serializeShowtimeSelection([unknown.key])),
...
test('keeps only available selection keys in deterministic order', () => {
  ...
  assert.deepEqual(
    validShowtimeSelectionKeys(results, [
      'stale:key',
      'ugc:b',
      'ugc:a',
      'ugc:b',
...
test('treats touching effective-start-to-end intervals as compatible', () => {
  ...
  const before = view({
    key: 'before',
    effectiveStartTime: '2026-08-24T16:00:00+02:00',
    end: canonical('2026-08-24T18:15:00+02:00'),
  })
  const after = view({
    key: 'after',
    effectiveStartTime: '2026-08-24T20:00:00+02:00',
    end: canonical('2026-08-24T22:00:00+02:00'),
  })
  const overlapping = view({
    key: 'overlap',
    effectiveStartTime: '2026-08-24T19:59:00+02:00',
    end: canonical('2026-08-24T21:00:00+02:00'),
  })
...
test('shows selections and only candidates compatible with every selection', () => {
  const source = [
    view({
      key: 'early',
      effectiveStartTime: '2026-08-24T16:00:00+02:00',
      end: canonical('2026-08-24T18:00:00+02:00'),
    }),
    view({
      key: 'middle',
      effectiveStartTime: '2026-08-24T18:00:00+02:00',
    ...
    view({
      key: 'overlap-early',
      effectiveStartTime: '2026-08-24T17:00:00+02:00',
      end: canonical('2026-08-24T18:30:00+02:00'),
    }),
    view({
      key: 'overlap-late',
      effectiveStartTime: '2026-08-24T19:30:00+02:00',
      end: canonical('2026-08-24T21:00:00+02:00'),
    }),
  ]
  ...
  assert.deepEqual(
    filtered.map((result) => result.key),
    ['early', 'middle', 'late'],
  )
  assert.deepEqual(
    source.map((result) => result.key),
...
test('ignores stale selections and fails closed for invalid intervals while retaining selected entries', () => {
  ...
  assert.deepEqual(
    filterCompatibleShowtimeResults([valid, invalid], ['stale:key']).map(
      (result) => result.key,
    ),
    ['valid', 'invalid'],
  )
  assert.deepEqual(
    filterCompatibleShowtimeResults([valid, invalid], ['invalid']).map(
      (result) => result.key,
    ),
    ['invalid'],
  )
...
test('selected-only results keep exact sessions, not other screenings of selected movies', () => {
  ...
  const sameMovie = view({
    key: 'ugc:ugc-showing-13',
    effectiveStartTime: '2026-08-24T20:00:00+02:00',
    end: canonical('2026-08-24T22:00:00+02:00'),
  })
  const otherMovie = view({
    key: 'kinepolis:kinepolis-showing-42',
    provider: 'kinepolis',
    movieKey: 'kinepolis:film-2',
    effectiveStartTime: '2026-08-24T22:00:00+02:00',
    end: canonical('2026-08-24T23:00:00+02:00'),
  })
  ...
  assert.deepEqual(
    filterSelectedShowtimeResults(source, [
      selected.key,
      'stale:key',
      selected.key,
    ]),
  ...
  assert.deepEqual(
    filterSelectedShowtimeResults(source, [otherMovie.key, selected.key]),
    [selected, otherMovie],
  )
  assert.deepEqual(
    filterCompatibleShowtimeResults(source, [selected.key]),
    source,
  )
  assert.deepEqual(source, before)
})
...
test('selection query serialization requires a nonempty serializable selection for selected-only mode', () => {
  ...
  assert.deepEqual(showtimeSelectionQueryValues(keys, true), {
    selected: 'u12',
    selected_only: '1',
  })
  assert.deepEqual(showtimeSelectionQueryValues(keys, false), {
    selected: 'u12',
    selected_only: undefined,
  })
  for (const empty of [[], ['invalid']]) {
    assert.deepEqual(showtimeSelectionQueryValues(empty, true), {
      selected: undefined,
      selected_only: undefined,
    })
  }
