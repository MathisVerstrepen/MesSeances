...
const middlewareSource = await readFile(
  new URL('../server/middleware/request-id.ts', import.meta.url),
  'utf8',
)
...
test('builds server API headers with one bounded request ID and optional private token', () => {
  ...
  assert.deepEqual(buildInternalApiHeaders('', REQUEST_ID), {
    [REQUEST_ID_HEADER]: REQUEST_ID,
  })
  assert.throws(
    () => buildInternalApiHeaders(SECRET, 'visitor-value'),
    /Invalid internal request ID/,
  )
})
...
test('keeps service identity private and server-only while blank config preserves public calls', () => {
  ...
  const publicConfig =
    runtimeConfig.match(/public:\s*\{([\s\S]*)\n    \}/u)?.[1] ?? ''
  assert.match(runtimeConfig, /internalApiSharedSecret:\s*''/u)
  assert.doesNotMatch(
    publicConfig,
    /internalApiSharedSecret|INTERNAL_API_SHARED_SECRET/u,
  )
  assert.match(
    composableSource,
    /hasInternalApiIdentity =\s*import\.meta\.server && config\.internalApiSharedSecret !== ''/u,
  )
  assert.match(
    composableSource,
    /import\.meta\.server[\s\S]*?import\(\s*'~~\/server\/utils\/internalApi'\s*\)/u,
  )
  assert.match(
    composableSource,
    /if \(!hasInternalApiIdentity\)\s+throw new Error\('Internal API identity unavailable'\)/u,
  )
...
test('Nitro middleware replaces visitor request IDs and propagates its generated value', () => {
  ...
  assert.match(
    middlewareSource,
    /setResponseHeader\(event, REQUEST_ID_HEADER, requestId\)/u,
  )
  assert.doesNotMatch(
    middlewareSource,
    /get(?:Request)?Header|headers\[['"]x-request-id/u,
  )
})
