...
const corpus: { valid: string[]; invalid: string[] } = JSON.parse(
  await readFile(
    new URL('./fixtures/noecinemas-booking-urls.json', import.meta.url),
    'utf8',
  ),
)
...
const crawler = runInNewContext(
  `${crawlerSource.slice(crawlerStart, crawlerEnd)}; ({ reservationUrl })`,
  { URL },
)

test('Noé default desktop booking corpus freezes exactly 24 theater paths on 14 hosts', () => {
  assert.equal(
    new Set(corpus.valid.map((value) => new URL(value).hostname)).size,
    14,
  )
  ...
  for (const value of [...corpus.valid, maximum]) {
    assert.deepEqual(safeBookingUrl(value, 'noecinemas'), {
      provider: 'noecinemas',
      url: value,
      kind: 'booking',
    })
    ...
    assert.equal(
      crawler.reservationUrl({ booking_url: value, provider: 'noecinemas' }),
      value,
    )
  }
  for (const value of [...corpus.invalid, `${maximum}1`]) {
    assert.equal(
      safeBookingUrl(value, 'noecinemas'),
      null,
  ...
  for (const provider of providers) {
    ...
    assert.equal(
      crawler.reservationUrl({ booking_url: booking, provider }),
      null,
    )
  }
})
...
test('Noé booking binds each path to optional theater and showing contexts, never reconstructs session from hash', () => {
  for (const [index, theater] of theaters.entries()) {
    ...
    assert.equal(
      safeBookingUrl(value, 'noecinemas', id, `noecinemas-${theater}`)?.url,
      value,
    )
    for (const other of [
      ...theaters.filter((item) => item !== theater),
      'W8391',
    ]) {
      assert.equal(
        safeBookingUrl(value, 'noecinemas', undefined, `noecinemas-${other}`),
        null,
      )
      assert.equal(
        safeBookingUrl(
          value,
          'noecinemas',
          `noecinemas-showing-${other}-${hash}`,
        ),
        null,
      )
    }
    assert.equal(
      crawler.reservationUrl({
        booking_url: value,
        provider: 'noecinemas',
        id,
        theater_id: `noecinemas-${theater}`,
      }),
      value,
    )
  }
  for (const id of [
    ...
    showingId + '\n',
    showingId + ' ',
    showingId.replace(hash, hash.toUpperCase()),
    showingId.replace('P8088', 'p8088'),
    showingId.replace('noecinemas', 'grandecran'),
    'noecinemas-showing-P8088-19',
  ]) {
  ...
  for (const theater of [
    ...
    assert.equal(
      safeBookingUrl(booking, 'noecinemas', showingId, theater),
      null,
    )
    assert.equal(
      crawler.reservationUrl({
...
test('Noé n tokens are canonical, full-string, collision-free cinema-qualified SHA-256 encodings', () => {
  const keys = ['P8088', 'B0158', '00000'].map(
    (theater) => `noecinemas:noecinemas-showing-${theater}-${hash}`,
  )
  for (const [index, theater] of ['P8088', 'B0158', '00000'].entries()) {
    assert.equal(
      serializeShowtimeSelection([keys[index]!]),
      `n${theater}-${compactHash}`,
    )
  ...
  const all = [
    ...keys,
    `grandecran:grandecran-showing-P8088-${hash}`,
    `cgr:cgr-showing-P8088-${hash}`,
  ...
  for (const theater of [
    ...
    assert.equal(
      serializeShowtimeSelection([
        `noecinemas:noecinemas-showing-${theater}-${hash}`,
      ]),
      undefined,
    )
    assert.deepEqual(parseShowtimeSelection(`n${theater}-${compactHash}`), [])
  ...
  for (const token of [
    `nP8088-${compactHash}=`,
    `nP8088-${compactHash.slice(0, -1)}x`,
    `nP8088-${compactHash}\n`,
    `nP8088-${compactHash} `,
    `nP8088-${compactHash.slice(1)}`,
  ])
  ...
  for (const value of [
    ...
    assert.equal(
      serializeShowtimeSelection([
        `noecinemas:noecinemas-showing-P8088-${value}`,
      ]),
      undefined,
    )
})
...
test('Noé selected URL reload retains cinema scope, future dates and selected-only filters', () => {
  ...
  const selectedShowtimeKeys = theaterIds.map(
    (theater) =>
      `noecinemas:${theater.replace('noecinemas-', 'noecinemas-showing-')}-${hash}`,
  )
  ...
  assert.equal(
    query.get('selected'),
    `nB0158-${compactHash},nP8088-${compactHash}`,
  )
...
test('Noé results preserve event IDs, VFSTF, source rooms and unknown metadata without fabricating ends', () => {
  ...
  for (const runtime of [0, 93, 118]) {
    ...
    for (const result of [
      ...
      const reloaded = parseShowtimeSelection(
        serializeShowtimeSelection([result.key]),
      )
      assert.deepEqual(validShowtimeSelectionKeys([result], reloaded), [
        result.key,
      ])
      assert.deepEqual(filterSelectedShowtimeResults([result], reloaded), [
        result,
      ])
    }
    ...
    assert.deepEqual(
      ...
      { id: 'noecinemas-P8088', provider: 'noecinemas', favorite: true },
    )
  }
  assert.deepEqual(
    availableLanguageOptions(['VOSTFR', 'VF', 'VFSTF']).map(
      (option) => option.value,
    ),
    ['ALL', 'VOSTFR', 'VF', 'VFSTF'],
  )
  ...
  assert.equal(
    safePosterUrl('https://all.web.img.acsta.net/pictures/event.jpg'),
    'https://all.web.img.acsta.net/pictures/event.jpg',
  )
  assert.equal(
    safePosterUrl('https://all.web.img.acsta.net.evil.test/pictures/event.jpg'),
    null,
  )
})
...
test('Noé brand-only names keep city context without corrupting other words', () => {
  for (const name of [
    ...
    'noé cinémas',
    'NOE CINEMAS',
    'Noé Cinémas'.normalize('NFD'),
  ])
    assert.equal(
      theaterDisplayName({ name, city: "L'Aigle" }),
      `${name} L'Aigle`,
    )
  for (const name of [
    "Noé Cinémas L'Aigle",
    ...
    assert.equal(theaterDisplayName({ name, city: "L'Aigle" }), name)
  assert.match(
    cinemaDescription({
      provider: 'noecinemas',
      name: "Cinéma L'Aigle",
