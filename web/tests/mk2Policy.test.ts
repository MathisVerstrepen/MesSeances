...
const booking =
  'https://www.mk2.com/panier/seance/tickets?cinemaId=0004&sessionId=140350'
const showingId = 'mk2-showing-0004-140350'
const poster =
  'https://srv-web-vista.mk2.com/CDN/media/entity/get/FilmPosterGraphic/HO00006568'
...
const invalidBookings = [
  ...
  `${booking}#tickets`,
  `${booking}&extra=1`,
  `${booking}&sessionId=140350`,
  booking.replace('https:', 'http:'),
  booking.replace('www.mk2.com', 'www.mk2.com.evil.test'),
  booking.replace('www.mk2.com', 'WWW.MK2.COM'),
  booking.replace('www.mk2.com', 'user@www.mk2.com'),
  booking.replace('www.mk2.com', 'www.mk2.com:443'),
  booking.replace('www.mk2.com', 'www.mk2.com:8443'),
  booking.replace('/panier/', '/other/../panier/'),
  booking.replace('/panier/', '/%70anier/'),
  booking.replace(
    'cinemaId=0004&sessionId=140350',
    'sessionId=140350&cinemaId=0004',
  ),
  booking.replace('0004', '0000'),
  booking.replace('0004', '%30%30%30%34'),
  booking.replace('0004', '+4'),
  booking.replace('140350', '0'),
  booking.replace('140350', '0140350'),
  booking.replace('140350', '1e3'),
  booking.replace('140350', '1'.repeat(112)),
  booking.replace('tickets?', 'tickets/?'),
  'https://www.mk2.com/',
]
const invalidPosters = [
  ...
  poster.replace('.mk2.com', '.mk2.com.evil.test'),
  poster.replace('srv-web-vista.', 'www.'),
  poster.replace('srv-web-vista.', 'user@srv-web-vista.'),
  poster.replace('.com/', '.com:443/'),
  poster.replace('.com/', '.com:8443/'),
  poster.replace('/CDN/', '/foo/../CDN/'),
  poster.replace('/CDN/', '/%43DN/'),
  poster.replace('FilmPosterGraphic', 'FilmBackdrop'),
  poster.replace('HO00006568', 'ho00006568'),
  poster.replace('HO00006568', 'HO'),
  poster.replace('HO00006568', `HO${'1'.repeat(118)}`),
  poster.replace('HO00006568', '%48O00006568'),
  `${poster}/`,
  `${poster}${'1'.repeat(2048)}`,
]

test('MK2 booking permits only canonical bytes, bounded identities and exact showing binding', () => {
  assert.deepEqual(safeBookingUrl(booking, 'mk2', showingId), {
    provider: 'mk2',
    url: booking,
    kind: 'booking',
  })
  ...
  for (const id of [
    'mk2-showing-4-140350',
    'mk2-showing-0005-140350',
    'mk2-showing-0004-140351',
    'ugc-showing-0004-140350',
    '',
    null,
  ]) {
  ...
  for (const value of invalidBookings)
    assert.equal(safeBookingUrl(value), null, value)
...
test('MK2 source posters use only verified FilmPosterGraphic grammar without TMDB transforms', () => {
  ...
  for (const value of invalidPosters)
    assert.equal(safePosterUrl(value), null, value)
...
test('MK2 crawlability policy agrees with frontend policy on accepted and rejected bytes and binding', async () => {
  ...
  const policy = runInNewContext(
    `${source.slice(start, end)}; ({ safePosterUrl, reservationUrl })`,
    { URL },
  )
  for (const value of [poster, ...invalidPosters])
    assert.equal(policy.safePosterUrl(value), safePosterUrl(value), value)
  for (const value of [booking, ...invalidBookings]) {
    for (const id of [showingId, 'mk2-showing-0005-140350', '', null]) {
      assert.equal(
        policy.reservationUrl({ booking_url: value, provider: 'mk2', id }),
        safeBookingUrl(value, 'mk2', id)?.url ?? null,
        `${value}: ${id}`,
      )
    }
  }
...
test('MK2 x tokens preserve leading zeros, cinema scope, bounds and cross-provider identities', () => {
  const keys = [
    ...
    'mk2:mk2-showing-4-140350',
    'cineville:cineville-showing-4-140350',
    'ugc:ugc-showing-140350',
  ]
  ...
  for (const id of [
    ...
    assert.equal(
      serializeShowtimeSelection([`mk2:mk2-showing-${id}`]),
      undefined,
      id,
    )
  }
  const max = `mk2:mk2-showing-0004-${'1'.repeat(111)}`
  assert.deepEqual(parseShowtimeSelection(serializeShowtimeSelection([max])), [
    max,
  ])
})
...
test('MK2 silent sessions retain source fields, future local date, empty room and explicit end provenance', () => {
  ...
  assert.deepEqual(availableLanguageOptions(['']), [
