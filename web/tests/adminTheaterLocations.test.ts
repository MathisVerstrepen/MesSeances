...
test('parses trimmed decimal dots and commas into JSON-ready numbers', () => {
  ...
  assert.deepEqual(
    parseAdminTheaterLocationCoordinates({ latitude: '-.5', longitude: '+,75' })
      .coordinates,
    {
      latitude: -0.5,
      longitude: 0.75,
...
test('requires both coordinate values', () => {
  assert.deepEqual(
    parseAdminTheaterLocationCoordinates({ latitude: '', longitude: ' ' }),
    {
      coordinates: null,
      errors: {
  ...
  assert.deepEqual(
    parseAdminTheaterLocationCoordinates({ latitude: '48.8', longitude: '' }),
    {
      coordinates: null,
  ...
  assert.deepEqual(
    parseAdminTheaterLocationCoordinates({ latitude: '', longitude: '2.3' }),
    {
      coordinates: null,
...
test('rejects non-numbers and Infinity-like input', () => {
  for (const latitude of [
    ...
    const result = parseAdminTheaterLocationCoordinates({
      latitude,
      longitude: '2',
    })
...
test('accepts inclusive coordinate boundaries and rejects values outside them', () => {
  assert.deepEqual(
    parseAdminTheaterLocationCoordinates({ latitude: '-90', longitude: '180' })
      .coordinates,
    {
      latitude: -90,
      longitude: 180,
...
test('keeps theater location page authenticated and renders API text literally', async () => {
  const page = await readFile(
    new URL('../app/pages/admin/theater-locations.vue', import.meta.url),
    'utf8',
  )

  assert.match(page, /definePageMeta\(\{ middleware: 'admin-auth' \}\)/)
  assert.match(page, /\{\{ item\.suggestion\.label \}\}/)
  assert.match(page, />\s*Géocodage IGN\s*</)
...
test('uses authenticated geocoding endpoints and cleans up two-second polling', async () => {
  ...
  assert.match(
    page,
    /onBeforeUnmount\(\(\) => \{[\s\S]*clearGeocodingPolling\(\)/,
  )
  assert.match(
    page,
    /nextJob\?\.state === 'succeeded'[\s\S]*refreshLocationsAfterGeocoding\(\)/,
  )
  assert.match(
    page,
    /getApiErrorStatus\(error\) === 409[\s\S]*loadGeocodingStatus\(\)/,
  )
})
