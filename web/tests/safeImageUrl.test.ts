...
test('Cinéville posters use only the exact production bucket and one safe filename', () => {
  ...
  const invalid = [
    ...
    url.replace('https:', ''),
    url.replace('storage.', 'other.storage.'),
    url.replace('.com/', '.com.evil.test/'),
    url.replace('cineville-files-prod', 'other-bucket'),
    url.replace('/images/', '/posters/'),
    url.replace('storage.', 'user@storage.'),
...
test('never synthesizes candidates for absent, unsafe, or non-TMDB poster sources', () => {
  ...
  assert.deepEqual(
    posterImageSources('https://image.tmdb.org.evil.test/t/p/w500/poster.jpg'),
    { src: null, srcset: null },
  )
  assert.deepEqual(
    posterImageSources('https://image.tmdb.org/t/p/w500/%2e%2e/secret.jpg'),
    { src: null, srcset: null },
  )
  assert.deepEqual(posterImageSources('https://www.ugc.fr/posters/film.jpg'), {
    src: 'https://www.ugc.fr/posters/film.jpg',
    srcset: null,
  })
})
...
test('accepts canonical Pathé poster URLs on the apex host and subdomains', () => {
  assert.equal(
    safePosterUrl('https://www.pathe.fr/media/posters/film.webp'),
    'https://www.pathe.fr/media/posters/film.webp',
  )
  assert.equal(
    safePosterUrl('https://pathe.fr/posters/film.jpg'),
    'https://pathe.fr/posters/film.jpg',
  )
  assert.equal(
    safePosterUrl('https://cdn.pathe.fr/a/b/film.jpg'),
    'https://cdn.pathe.fr/a/b/film.jpg',
  )
  assert.equal(
    safePosterUrl('https://WWW.PATHE.FR/media/posters/film.webp'),
    'https://www.pathe.fr/media/posters/film.webp',
...
test('accepts canonical CGR poster URLs on acsta.net and rejects unsafe variants', () => {
  assert.equal(
    safePosterUrl('https://images.acsta.net/pictures/film.jpg'),
    'https://images.acsta.net/pictures/film.jpg',
  )
  assert.equal(
    safePosterUrl('https://ACSTA.NET/posters/film.webp'),
    'https://acsta.net/posters/film.webp',
  )
...
test('rejects unsafe Megarama poster hosts, paths, sizes, and queries', () => {
  ...
  const urls = [
    `http://images.monnaie-services.com${global}`,
    `//images.monnaie-services.com${global}`,
    `https://user@images.monnaie-services.com${global}`,
    `${origin}:443${global}`,
    `${origin}:8443${global}`,
    `${origin}.evil.test${global}`,
    `https://other.monnaie-services.com${global}`,
    `https://IMAGES.MONNAIE-SERVICES.COM${global}`,
    ` ${origin}${global}`,
    `${origin}${global}\n`,
    `${origin}${global}?ts=123`,
    `${origin}${global}?`,
    `${origin}${global}#`,
    `${origin}${global.replace('/600/', '/500/')}`,
    `${origin}${global.replace('FRAB123', 'AB123')}`,
    `${origin}${global.replace('AB12CD34', 'ab12cd34')}`,
    `${origin}${global.replace('.webp', '.jpg')}`,
    `${origin}${local.replace('/120/', '/600/')}`,
    `${origin}${local.replace('0565', '565')}`,
    `${origin}${local}?ts=123&ts=456`,
    `${origin}${local}?other=123`,
    `${origin}${local}?ts=1.5`,
    `${origin}${local}?ts=-1`,
    `${origin}${local}?ts=`,
    `${origin}${local}?ts=%31`,
    `${origin}${local}?ts=1#poster`,
    `${origin}${local}/../HC1.jpg`,
    `${origin}/movie_poster/600/FRAB123/../FRAB123/AB12CD34.webp`,
    `${origin}${global.replace('AB12CD34', '%41B12CD34')}`,
    `${origin}\\${global}`,
  ]
