...
function showing(
  platform: 'cineoffice' | 'ticketingcine' | 'webediamovies',
  overrides: Partial<Showtime> = {},
): Showtime {
  return {
    provider: 'cinewest',
    id: `cinewest-showing-${platform}-${'a'.repeat(64)}`,
    movie,
    start_time: start,
    end_time: start,
...
function theaterResponse(showtimes: Showtime[]): TheaterShowtimesResponse {
  return {
    ...
    showtimes: showtimes.map((showtime) => ({
      ...
      duration_minutes:
        (Date.parse(showtime.estimated_end_time ?? showtime.end_time) -
...
test('Cine Office published end survives display normalization regardless of source or enriched runtime', () => {
  for (const runtime of [0, 93, 118]) {
    const showtime = showing('cineoffice', {
    ...
    const slot: SlotResult = {
      ...
      poster_url: null,
      backdrop_url: null,
      effective_start_time: '2027-06-27T18:15:00+02:00',
      effective_end_time: publishedEnd,
    ...
    assert.deepEqual(
      ...
      { time: publishedEnd, estimated: false, adsMinutes: null },
    )
  }
})
...
test('ticketingcine computed ends, Capitole response estimates and unknown ends retain distinct provenance', () => {
  ...
  assert.equal(
    (Date.parse(ticketing.end_time) - Date.parse(start)) / 60_000,
    movie.runtime_minutes + 15,
  )
...
test('Cinewest compatibility uses exact published or returned estimated boundaries, never catalog runtime', () => {
  const [published, estimated, unknown] = toTheaterShowtimeResults(
    theaterResponse([
  ...
  const following = {
    ...published,
    key: 'following',
    effectiveStartTime: publishedEnd,
    end: {
      time: '2027-06-27T23:00:00+02:00',
      estimated: false,
      adsMinutes: null,
    },
  }
  ...
  assert.equal(
    areShowtimeResultsCompatible(published, {
      ...following,
      effectiveStartTime: '2027-06-27T21:07:42.122+02:00',
    }),
    false,
  )
  assert.equal(
    areShowtimeResultsCompatible(estimated, {
      ...following,
      effectiveStartTime: runtimeEnd,
    }),
    true,
  )
  assert.equal(
    areShowtimeResultsCompatible(estimated, {
      ...following,
      effectiveStartTime: '2027-06-27T20:12:59.999+02:00',
    }),
    false,
  )
  ...
  assert.deepEqual(
    filterCompatibleShowtimeResults(
      [published, unknown, following],
      [published.key],
    ),
    [published, following],
  )
  assert.deepEqual(
    filterCompatibleShowtimeResults([unknown, following], [unknown.key]),
    [unknown],
  )
  assert.deepEqual(
    filterSelectedShowtimeResults([unknown, following], [unknown.key]),
    [unknown],
  )
})
...
test('Cinewest language and format options reuse existing filters and do not invent a silent-language label', () => {
  const showtimes = [
    ...
    showing('webediamovies', {
      id: `cinewest-showing-webediamovies-${'c'.repeat(64)}`,
      language: 'VF',
  ...
  assert.deepEqual(
    availableLanguageOptions(results.map((result) => result.language)).map(
      (option) => option.value,
    ),
    ['ALL', 'VOSTFR', 'VF', 'VO', 'VFSTF'],
  )
  assert.deepEqual(
    availableFormatOptions(results.map((result) => result.format)).map(
      (option) => option.value,
    ),
    ['ALL', '2D', 'DOLBY', 'SCREENX', '4DX', 'ICE', 'INFINITY_VISION'],
  )
...
test('Cinewest film JSON-LD preserves published ends and omits estimated or unknown endDate', () => {
  ...
  const schedule: MovieShowtimesResponse = {
    ...
    date: response.date,
    available_dates: response.theater.available_dates,
  ...
  const graph = buildFilmJsonLd(schedule, {
    ...
    movieUrl: `https://messeances.fr/film/${movie.slug}`,
  })['@graph']
  ...
  assert.equal(
    graph.find((node) => node['@type'] === 'MovieTheater')?.name,
    'Capitole Studios',
  )
...
test('Infinity Vision and ICE sessions of the same movie retain distinct formats in timeline and slot results', () => {
  ...
  for (const results of [
    toTheaterShowtimeResults(response),
    toSlotShowtimeResults(slots),
  ]) {
    assert.deepEqual(
