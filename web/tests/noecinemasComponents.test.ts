...
const [BrandLogo, BrandedText, TheaterName, BookingLink] = await Promise.all(
  ['BrandLogo', 'BrandedText', 'TheaterName', 'BookingLink'].map(component),
)
...
test('Noé exact supplied bytes serve both logo sizes on light backing with accessible brand', async () => {
  const image = await readFile(
    new URL('../app/assets/imgs/noe_cinema_logo_small.webp', import.meta.url),
  )
  assert.equal(
    createHash('sha256').update(image).digest('hex'),
    '78231e6aa9d9026f1bc68d266419d45477aaf263fa8ef1f959f2356278179e5a',
  )
  ...
  assert.match(
    await render(BrandLogo!, { brand: 'Noé Cinémas', decorative: true }),
    /alt(?:="")? aria-hidden="true"/,
  )
  assert.match(
    await source('pages/credits.vue'),
    /brand: 'Noé Cinémas',\s*name: 'Noé Cinémas',\s*url: 'https:\/\/www\.noecinemas\.com\/'/,
  )
})
...
test('Noé text recognition uses full accented or unaccented names and preserves location/source text', async () => {
  for (const name of [
    ...
    const theater = await render(TheaterName!, {
      name: text,
      provider: 'noecinemas',
    })
  ...
  for (const name of [
    ...
    assert.match(
      await render(TheaterName!, { name, provider: 'noecinemas' }),
      /alt="Noé Cinémas"/,
    )
  }
})
...
test('Noé BookingLink renders safe new-tab link or unavailable text, never unsafe href', async () => {
  ...
  const props = {
    url,
    provider: 'noecinemas',
    showtimeId: `noecinemas-showing-P8088-${'a'.repeat(64)}`,
    theaterId: 'noecinemas-P8088',
  }
  const html = await render(BookingLink!, props)
  assert.match(
    html,
    /href="https:\/\/achat\.cinema-laigle\.com\/reserver\/r\/19"/,
  )
  assert.match(html, /target="_blank" rel="noopener noreferrer"/)
  assert.match(
    html,
    /aria-label="Réserver sur Noé Cinémas, ouverture dans un nouvel onglet"/,
  )
  for (const overrides of [
    { url: `${url}?relay=1` },
    { theaterId: 'noecinemas-B0181' },
    { showtimeId: null },
    { provider: 'cgr' },
    { url: null },
  ]) {
...
test('Noé follows Grand Ecran throughout admin, latest-run, map and labels', async () => {
  for (const page of [
    ...
    if (page.startsWith('sync'))
      assert.match(
        value,
        /const providers = \[\s*'ugc',\s*'kinepolis',\s*'pathe',\s*'cgr',\s*'megarama',\s*'cineville',\s*'mk2',\s*'cinewest',\s*'grandecran',\s*'noecinemas',?\s*\] as const/,
      )
  }
  assert.match(
    await source('pages/admin/sync-schedules.vue'),
    /noecinemas: selectLatestProviderRun\(\s*'noecinemas'/,
  )
  assert.match(
    await source('components/CinemaTheaterMap.client.vue'),
    /'noecinemas',\s+THEATER_PROVIDER_COLORS\.noecinemas/,
  )
})
