...
test('updates only favorite feature properties without mutating input', () => {
  ...
  assert.deepEqual(Object.keys(collection.features[0]!.properties).sort(), [
    'favorite',
    'id',
    'provider',
  ])
...
test('spreads coincident theaters deterministically on a 35 meter ring', () => {
  ...
  const reversed = buildTheaterFeatureCollection(
    [...theaters].reverse(),
    new Set(),
  )
...
test('keeps singleton coordinates exact and calculates empty, singleton, and multiple bounds', () => {
  const empty = buildTheaterFeatureCollection(
    [theater({ id: 'missing', latitude: null })],
    new Set(),
  )
  assert.equal(theaterFeatureBounds(empty), null)

  const singleton = buildTheaterFeatureCollection(
    [theater({ id: 'one', latitude: 43.3, longitude: 5.4 })],
    new Set(),
  )
