...
test('cinemas page uses deferred browser geolocation with bounded options', async () => {
  ...
  const location = await readFile(new URL('../app/utils/cinemaDirectoryLocation.ts', import.meta.url), 'utf8')

  assert.match(page, /'Utiliser ma position'/)
  assert.match(page, />\s*Afficher par ville\s*</)
  assert.match(page, /@click="useCurrentPosition"/)
  assert.match(page, /createCinemaDirectoryLocation\(\(\) => import.meta.client \? navigator.geolocation : undefined\)/)
  assert.match(location, /enableHighAccuracy: false,\s*timeout: 8000,\s*maximumAge: 600000/)
...
test('cinemas page exposes accessible pending, failure, and nearest states', async () => {
  ...
  assert.match(location, /Localisation refusée\. Autorisez l’accès à votre position dans les réglages du navigateur, puis réessayez\./)
  assert.match(location, /Position indisponible\. Vérifiez que la localisation est activée, puis réessayez\./)
  assert.match(location, /La localisation a pris trop de temps\. Réessayez\./)
  assert.match(location, /La localisation n’est pas disponible dans ce navigateur\. Continuez avec la liste par ville\./)
  ...
  assert.match(location, /Number\.isFinite\(position\.coords\.accuracy\) && position\.coords\.accuracy >= 0/)
...
test('cinemas page lazy-loads client-only map mode while preserving list fallback and shared state', async () => {
  ...
  assert.match(page, /const viewMode = computed\(\(\) => routeState.value.view\)/)
