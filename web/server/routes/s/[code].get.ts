...
} from '../../../app/utils/shortLinkTarget'
...
function parseShortLinkResponse(
  response: UpstreamShortLinkPayload,
  requestedCode: string,
): ShortLinkResponse | null {
  if (
    response.code !== requestedCode ||
    Object.prototype.toString.call(response.target) !== '[object String]'
  )
    return null
...
export default defineEventHandler(async (event) => {
  ...
  if (!isValidShortLinkCode(code)) {
    throw createError({
      statusCode: 404,
      statusMessage: 'Lien introuvable',
      message: 'Ce lien de partage est introuvable.',
    })
  }
  ...
  try {
    response = await $fetch<unknown>(
      `${apiBase}/api/v1/shortlinks/${encodeURIComponent(code)}`,
    )
  } catch (error) {
    ...
    throw createError({
      ...
      message: 'Le service de partage est temporairement indisponible.',
    })
  }
  ...
  if (!shortLink) {
    throw createError({
      statusCode: 502,
      statusMessage: 'Réponse invalide',
      message: 'Le service de partage a renvoyé une réponse invalide.',
    })
  }
