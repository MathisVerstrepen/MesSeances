...
Sync timing defaults are `SYNC_REQUEST_TIMEOUT=20s`, `SYNC_KINEPOLIS_REQUEST_INTERVAL=2s`, `SYNC_CINEVILLE_REQUEST_INTERVAL=2s`, and `SYNC_OPERATION_TIMEOUT=2m`. Request timeout applies to UGC, Kinepolis, Pathé, CGR, Megarama, Cinéville, MK2, Cinewest, Grand Ecran, and Noé Cinémas and must be between 5s and 60s. Kinepolis and Cinéville intervals must be at least 1s, and operation timeout must be positive. MK2 uses a fixed shared two-second request interval.

Cinéville supports manual, all-provider, and scheduled sync through the existing admin controls. Proxy-only acquisition discovers the current Next.js build and actual cinema routes, excludes the test headquarters, and imports real sessions from both program arrays across the entire advertised future range. A stale-build 404, unexpected page content type, or malformed cinema-page payload allows one re-bootstrap and full acquisition restart, discarding every earlier page. A 404 requires a changed build; transient content-type or payload failures can recover on the same build. Malformed bootstrap, persistent page failures, and normalization failures remain terminal. Cinema-scoped showing IDs and signed int64 film visas are preserved. Missing source metadata stays missing; all Cinéville source and canonical ends remain unknown (`end_time == start_time`), even after TMDB enrichment. Runtime from the resolved movie, including cached enrichment, can provide the separate display/planning estimate described above without changing stored data. Malformed or incomplete acquisition never replaces the previous snapshot.

The opt-in Cinéville contract test validates the complete advertised snapshot through operator-supplied proxies, without database publication or TMDB/IGN calls. Ordinary tests skip it:

```sh
# From api/
CINEVILLE_LIVE_PROXY_FILE=/path/to/proxies.txt go test ./internal/cineville -run '^TestProxyFullSyncContractIntegration$' -count=1 -v -timeout 6m
```

MK2 supports manual, all-provider, and scheduled sync through those same admin controls, after Cinéville in the provider order. Proxy-only acquisition joins the live cinema and film catalogs with each distinct Paris cinema-complex program, using two workers, shared pacing, and bounded responses. All advertised future sessions, including events and early-morning screenings, retain their Europe/Paris calendar date. Opaque cinema codes preserve leading zeros; film IDs and cinema-scoped session IDs remain stable. Silent sessions have no language badge, rooms remain empty, and every stored end stays unknown (`end_time == start_time`), even with positive source or enriched runtime; display/planning estimates remain separate. Source posters and exact MK2 checkout links use strict host/path/identity policies. Incomplete or conflicting acquisition leaves the prior snapshot untouched. No MK2 schedule is automatically created or enabled.
...
The opt-in Cinewest source contract test validates the complete 13-cinema snapshot in memory through operator-supplied proxies, without database publication or TMDB/IGN calls. Ordinary tests skip it:

```sh
# From api/
CINEWEST_LIVE_PROXY_FILE=/path/to/proxies.txt go test ./internal/cinewest -run '^TestProxyFullSyncContractIntegration$' -count=1
```

Grand Ecran (`grandecran`) follows Cinewest in the manual, all-provider, and scheduled order. Proxy-only acquisition discovers the current cinema list and asset build, follows each cinema's advertised future dates without a fixed horizon, and retains numeric films and opaque local events. Original-version sessions use VOSTFR; service days begin at 03:00 Europe/Paris. Missing runtime, poster, or room stays unknown; safe posters outside the existing acsta.net image policy are omitted, while malformed URLs fail acquisition. Stored ends always equal starts, with separate runtime/ads estimates for display and planning. Stable showing IDs exclude room metadata. Missing rooms receive best-effort booking-page enrichment with four workers, at most 4096 distinct URLs, and a separate 20-minute stage budget; unresolved rooms remain empty. Canonical booking links never change to resolved checkout URLs. Challenge/block responses and parent cancellation fail the sync; ordinary room failures do not discard sessions. Incomplete acquisition preserves the previous snapshot, and all-provider publication remains atomic. No schedule is seeded. `SYNC_OPERATION_TIMEOUT` bounds publication and TMDB work, not the room stage.

Opt-in Grand Ecran contract verification runs without database publication: from `api/`, use `GRANDECRAN_LIVE_PROXY_FILE=/home/mathis/Documents/Dev/movieflow/tmp/proxies.txt go test ./internal/grandecran -run '^TestProxyScheduleContractIntegration$' -count=1 -timeout 5m`. It checks the full advertised schedule and at most eight room pages, logging sanitized counters only. The optional `TestProxyFullSyncContractIntegration` uses the same environment and `-timeout 25m` to exercise the complete bounded room stage. Neither test stores raw responses or proxy credentials.

Noé Cinémas (`noecinemas`) is the tenth provider, after Grand Ecran in manual, all-provider, and scheduled order. Trigger through authenticated admin/API (`POST /api/v1/admin/syncs/noecinemas`) or existing sync schedules; there is no Noé CLI or `make sync` extension. Acquisition requires `PROXY_FILE`, discovers all cinemas including empty programs, and follows every advertised future service date without a fixed horizon. Whole cinema days start at 03:00 Europe/Paris; numeric films and opaque local events retain their identities. VF, VOSTFR, VFSTF, and projection formats remain distinct. Missing runtime/poster/room stays unknown; stored ends equal starts regardless of runtime, with existing estimates separate. Session identity hashes cinema ID and exact source session ID, never booking URL or recovered room. Only verified default desktop booking entries on the finite per-cinema allowlist are accepted; relay and resolved checkout URLs are never published. Missing L'Aigle rooms alone receive optional same-host, no-cookie HTML recovery, bounded to four workers, 4096 URLs, and 20 minutes. Ordinary room failures leave empty rooms; challenge or parent cancellation fails acquisition. Incomplete acquisition preserves the previous snapshot. No schedule is seeded or enabled, and no deployment configuration changes are required.

Opt-in Noé contract verification runs without database publication: from `api/`, use `NOECINEMAS_LIVE_PROXY_FILE=/home/mathis/Documents/Dev/movieflow/tmp/proxies.txt go test ./internal/noecinemas -run '^TestProxyScheduleContractIntegration$' -count=1 -v -timeout 10m`. It validates the complete advertised snapshot and samples at most eight missing-room pages within an eight-minute parent deadline, logging normalized counters only. Ordinary unit tests use synthetic fixtures and never contact providers.
