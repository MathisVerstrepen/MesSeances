...
All discovery pages and previously active omissions are verified before a single atomic publication. Invalid pagination, malformed release evidence, provider failures, and commit failures preserve the previous publication. Explicit empty evidence or movie HTTP 404 withdraws membership without deleting identities or URLs. Eligible movies reuse metadata for 30 days; the existing metadata-refresh target also includes retained TMDB-only movies. Provider matching later attaches showtimes to the durable public identity. Manual metadata overrides and general `release_date` remain distinct from verified `french_release_date`.
