...
`GET /api/v1/movies/upcoming` accepts only optional `month=YYYY-MM`, comma-separated `genres`, positive `page` (default 1), and `page_size` (default 24, maximum 100). Results sort by French date, normalized title, and numeric public ID. Genre matching is OR; available months/genres describe the full eligible year before filters. Response includes `window`, `timezone`, last successful `generated_at`, `catalog_revision`, `items`, facets, and filtered `total`. Unsupported, duplicate, or malformed parameters return `400 invalid_query`. An existing catalog without a successful import returns `503 upcoming_unavailable` with `Cache-Control: no-store`; a successful empty import returns `200` with empty arrays. Membership and catalog revision age at Paris midnight without another import.
