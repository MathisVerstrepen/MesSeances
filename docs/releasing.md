...
Merging a valid same-repository `dev` to `main` pull request runs one serialized release workflow. Read-only validation first checks the supported event/action, open/unmerged state, same-repository non-fork dev/main identity, both SHAs, and exact base checkout. It prefers ordinary tooling at the exact pre-merge base. Only an absent base tooling path permits fallback to exact non-fork head; existing nonregular or symlink base paths fail, never fall back. Selected checkout SHA and ordinary tooling file are verified before execution. Unsafe-but-present validation tooling must be rejected by operator preflight, not opportunistically replaced by dev tooling.

Privileged finalization and promotion each verify the exact base checkout, closed/merged event identity, and all three SHAs. Before API reads, they require `WORKFLOW_SHA` from `github.workflow_sha` to be a full lowercase 40-character SHA equal to `MERGE_SHA`. Fresh GitHub PR number/base/head/merge equality and current protected `main == MERGE_SHA` must then pass. Only exact merge tooling is selected, regardless of whether base tooling is old, absent, or nonregular. Selected source must be `merge`, selected ref and checked-out HEAD must equal `MERGE_SHA`, and the selected script must be an ordinary non-symlink file. This binds the workflow invocation and publisher to the same immutable revision without probing CLI support or falling back to base/head Python. API errors, malformed metadata, source mismatches, and workflow-SHA drift stop before Python. Neither `github.sha` nor Actions run `head_sha` substitutes for `github.workflow_sha`.
...
## Repair integration and publication readiness

A reviewed release-automation repair normally reaches `dev` through a conventional feature PR. Verified merge-only publication removes the old privileged base-script selection obstacle; it does not waive safe read-only validation tooling or any release prerequisite. A direct maintenance PR to main is not a compliant shortcut under the release-only validator. Integration still requires explicit authorization and a route that preserves protection. Do not disable publication, weaken validation or the release skill, bypass protection, or publish with unsafe tooling to seed the repair.

Before any later publication authorization is exercised, freshly inspect exact protected-main workflows/tooling and proposed incoming release workflow/script changes, active workflow IDs, refs, protection, and checks using the complete publish skill preflight. Reject mismatches; source-policy approval and repair integration success are not release readiness. An incomplete earlier merged publication still blocks starting another release and requires operator disposition.
...
A failed historical run remains pinned to its original workflow and event inputs; rerunning it does not load a later source-selection fix. A parser failure before client construction proves no writes by that invocation, not global absence of tags, Releases, or images. Before deciding recovery, inspect fresh refs/protection/checks, exact PR metadata/changelog and run correlation, immutable records, both versioned images, and both latest aliases read-only. Default disposition is operator-blocked: source repair prepared, failed-publication disposition unresolved. Later options require separate approval: remain blocked; design a complete recovery bound to the original PR/merge, with explicit authorization for any currently prohibited execution path; or approve a documented failed-attempt disposition before a subsequent normal forward release. None is implicitly authorized by the source-policy change. Advancing main invalidates original-PR recovery under the retained `main == original MERGE_SHA` guard; resolve ordering and feasibility before any mutation, without weakening that guard or resetting main.
