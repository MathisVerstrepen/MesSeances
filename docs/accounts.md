...
- Google uses `openid email` only. A validated verified email can begin username onboarding without SES. An unverified email requires mailbox proof and the same Google-authenticated session. Email equality never merges accounts or links identities; Google issuer/subject is identity, not its observed email.
