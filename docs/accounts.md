...
### One-time PNG to WebP conversion
...
Migration 047 introduces a WebP-only CHECK. Existing PNG rows survive initially because the constraint is NOT VALID, but PostgreSQL enforces it on subsequent inserts and updates. Stop all account writers before upgrading a database with PNG avatars. Enabled API startup acquires the media-root lock before migrations and refuses to start with `account avatar conversion required` until the constraint is validated. Fresh installations without PNG references validate automatically. There is no PNG-serving fallback or automatic startup conversion.

Conversion is a separately authorized maintenance operation, not a routine validation command:

1. Stop the old API and all writers/collectors; verify the exact database/media-root pair and existing API UID. Take a consistent, access-restricted paired database/media backup and ensure temporary disk headroom.
2. Use the reviewed tagged binary and matching frontend. Inject only `DATABASE_URL` and `ACCOUNT_AVATAR_DIR` explicitly, then run `messeances-api migrate-account-avatars` as the existing media owner with the same persistent volume. The command does not read `deploy/.env` or start HTTP, OAuth, mail or cleanup workers. Local source equivalent, from `api/`: `go run -tags=nodynamic ./cmd/api migrate-account-avatars`.
3. The command locks the root before schema changes, processes one referenced image at a time, preserves source/removal intent and advances each converted avatar revision once. Already-WebP rows are validated, not recompressed. Missing, corrupt or insecure files cause a nonzero exit without clearing references or refetching Google photos. Partial completion is resumable; ambiguous commits retain both files for safe reconciliation.
4. On failure, keep enabled API stopped. Resolve the issue through authorized operator work and rerun; never bypass the CHECK, edit the migration ledger, force-unlock a running writer or reset photos to continue. On success, require zero exit and validated completion marker before starting the matching API/frontend. Old unreferenced PNGs can remain until normal bounded cleanup completes.

Rollback requires a compatible corrective build or a separately authorized consistent paired restore while quiesced. Never restore only database or media. Feature implementation does not itself authorize conversion of development or production data. Benchmark savings depend on content: a representative normalized photograph measured 74,641-byte PNG versus 5,388-byte Q80 WebP; a simple alpha gradient grew from 863 to 944 bytes.
