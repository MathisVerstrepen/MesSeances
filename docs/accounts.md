...
- Username is fixed: 3-30 ASCII characters, leading letter, letters/digits/underscore, uppercase normalized. Both supplied lists plus application reservations are blocked; see [provenance](../api/internal/accounts/reserved_usernames.md). A private profile picture appears only in settings, not the header or a public profile. Complete accounts synchronize selected theaters across devices on refresh, focus or reconnection; tabs in one browser receive change notifications. Continuously foregrounded devices do not receive live push updates.
...
- Deletion requires recent proof and typed `SUPPRIMER`. Account data, selected theaters and authority are deleted transactionally. Only username remains permanently reserved without account ID, email, subject, identity hash or deletion timestamp. Independent suppression/security data follows its retention. Browser-local cinema preferences remain unchanged.

### Selected theaters

Signed-out and incomplete-account browsing retains the independent browser-local `messeances.favoriteTheaterIds.v1` selection. On completing sign-in, an existing server selection takes precedence without modifying that local key. An account with no saved selection imports a valid stored device selection once; if the device has no valid stored selection, Paris/first-theater defaults are provisional until the user explicitly saves. Shared theater links remain temporary overrides and never import into account preferences. Logout restores the browser-local selection. Account-bound theater IDs are personal data, stored in PostgreSQL until replacement or account deletion, subject to backup retention policy.

`GET /api/v1/account/theaters` reads the complete-session owner's selection. An absent preference row reports revision `"0"` and an empty list; an initialized empty list has a positive revision. `POST /api/v1/account/theaters` requires exact same-origin Origin/CSRF protections, a complete session, expected username and expected revision. Account identity always comes from the session, never the request. Writes compare revisions under an account lock and reject stale changes with `theater_selection_changed` rather than silently replacing another device's edit. The client reloads the committed selection and asks for a fresh explicit edit. Uncertain writes are reconciled with a read, never automatically replayed. Account data is not persisted to browser storage, exposed in public SSR payloads or queued for offline writes.
...
The selected-theater resource is an account API path with the same private headers and no account CORS. Preference POST alone accepts a bounded 1 MiB strict JSON body to accommodate up to 4,096 theater IDs; legacy account JSON retains its 8 KiB bound. A separate in-memory IP limiter allows 120 preference writes per minute. Valid IDs absent from a schedule remain saved; schedule refresh does not erase the account selection.
...
Build the API with `-tags=nodynamic` so pinned `github.com/gen2brain/webp v0.6.4` uses bundled transpiled libwebp 1.6.0 rather than a host library. CGO remains disabled for deployment; Makefile, Air, CI, lint and Docker already supply this tag. Direct Go commands need it too. Input decoding remains explicitly routed through the existing JPEG/PNG/x-image decoders.

Stored avatars must use generated WebP filenames. Migration 048 unconditionally validates the WebP-only CHECK introduced by 047; there is no conversion command, legacy stored-PNG reader or conversion-readiness startup state. JPEG, PNG and static WebP remain supported upload inputs and are normalized to WebP.

Old development databases containing PNG avatar references are unsupported: migration rejects them transactionally, without modifying account references or deleting image files. Resolve or reset such disposable development data only with explicit operator authorization. Feature disablement does not bypass migrations. Normal garbage collection handles generated WebP/tmp files only and leaves unsupported PNG files untouched. Paired database/media backups remain required; do not restore only one side or edit migration history.

Benchmark savings depend on content: a representative normalized photograph measured 74,641-byte PNG versus 5,388-byte Q80 WebP; a simple alpha gradient grew from 863 to 944 bytes.
...
| Account theater selection | Until replaced or account deletion; deletion cascades in the active database. Browser-local selection is separate and survives account deletion; backups follow operator retention policy. |
...
```sh
go test -tags=nodynamic ./internal/accounts ./internal/accountmail ./internal/database ./internal/httpapi ./cmd/api -run 'Integration$' -count=1
go test -tags=nodynamic -race ./internal/accounts ./internal/accountmail ./internal/httpapi
```
...
ACCOUNT_BROWSER_HARNESS=1 go test -tags=nodynamic ./internal/httpapi -run '^TestAccountBrowserHarness$' -count=1 -timeout=0 -v
...
API startup applies migrations even when feature disabled. After installing through 049, do not roll back to a binary embedding an earlier migration history. Use a compatible corrective build retaining migration history. The additive theater-preference migration does not backfill account selections. Deploy a compatible API before or together with its frontend; do not substitute a browser-storage account fallback if the resource is unavailable. Pending email registrations created before browser binding must restart registration to obtain a new bound attempt. Emergency disable must keep public/admin app available; revoke account sessions/tokens/flows before re-enabling. Never drop username reservations, rewrite migration ledger or restore old queued tokens. Deletion fences invalidate outstanding anonymous Google flows globally because their subjects are unknown before exchange; other in-flight logins may need restart.
