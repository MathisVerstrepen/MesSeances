...
When production uses `TRUSTED_PROXY_CIDRS=127.0.0.1/32`, Nginx must be API's immediate IPv4 loopback peer. Apply the same `X-Forwarded-For` overwrite to every API proxy location, not only admin login. Leave `TRUSTED_PROXY_CIDRS` empty until this topology and header behavior are confirmed. Do not use `$proxy_add_x_forwarded_for` with untrusted inbound headers.
